<?php
class AutosearchHelper extends AppHelper {
       ...
       /*  $fields= "";
        ...
        $this->output.=$this->Html->scriptBlock('
            ...
            function query_'.$idInput.'(txt) {
			        if(txt.length >= 2) {
                    $.post("'.SITE_URL.$fun_name.'", {query: txt,rand: "'.$idInput.'"}					
					, function(data){
                        $("#autoresult_'.$idInput.'").html("<ul id=\'ul_'.$idInput.'\' class=\'autocomplete_live autoserachAdd\'>"+data+"</ul>");
                        $("#ul_'.$idInput.'").width(480);
