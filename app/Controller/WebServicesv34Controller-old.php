...
class WebServicesv34Controller extends AppController{
   ...
   /************************************************************************************************************************************
    ...
    public function testmail(){
        ...
        // die('test');
        App::import('Vendor', 'Mailgun', array('file' => 'Mailgun.php'));

       
        # First, instantiate the SDK with your API credentials
        $mg = Mailgun::create('pubkey-c03ee05e8165d99ee287a78a13b9acb3');
        pr($mg);die;
        ...
        $to = 'mahen.zed123@gmail.com';
        $subject = 'new mail';
        $message = 'send mail on cloud.';
        ...
        $mail = $this->Email->send($to, $subject, $message, $headers);
         pr($mail);die;
 ...
 function subscription_status($test_data = null){
     ...
     *************************************************************************************************************************************/
     ...
     function signup_new($test_data = null){
        ...
        $mail->Username   = "mahen.zed123@gmail.com";  
        $mail->Password   = "mahen@123";
        
        $mail->From = "mahen.zed123@gmail.com";
        $mail->FromName = "From Me";
    
        $mail->AddAddress("zedtech.cloud@gmail.com");
    
        $mail->IsHTML(true);
        $mail->CreateHeader($header);
    
        $mail->Subject = "test subject";
        $mail->Body    = nl2br("all testmail");
        $mail->AltBody = nl2br($message);
    
        // return an array with two keys: error & message
        if(!$mail->Send()) {
        $data =  array('error' => true, 'message' => 'Mailer Error: ' . $mail->ErrorInfo);
        } else {
    ...
    function signup($test_data = null){
        ...
        if($this->User->saveAll($user)){
            ...
            $from = 'JTSBoard  <do.not.reply@zedinternational.net>';
...
/****************************************************************************************************************************************
    ...
    function upload_note_image(){
        ...
        if(!empty($_FILES)){
                ...
                // echo WWW_ROOT . NOTE_IMG_THUMB_DIR;die;
                // move_uploaded_file($_FILES["note_image"]["tmp_name"], WWW_ROOT . NOTE_IMG_THUMB_DIR);
...
/****************************************************************************************************************************************
     ...
     *************************************************************************************************************************************/
     ...
     function add_note_ticket_old($test_data = null){
        ...
        if($this->NoteTicket->saveAll($noteTicket)){
            ...
            if(!empty($ticket_id)){
                ...
                if($ticket_price >= $ticket_amount ){
                    ...
                    $noteRemainingTicket['NoteRemainingTicket']['customer_history_id'] = $customer_history_id ;
                    $noteRemainingTicket['NoteRemainingTicket']['note_ticket_id'] = isset($decoded['ticket_id']) ? $decoded['ticket_id'] : '';
                    $noteRemainingTicket['NoteRemainingTicket']['remaining_amount'] = number_format($ticket_amount).'円';
                    $this->NoteRemainingTicket->saveAll($noteRemainingTicket);
                }
            }
    ...
    function calender_reservation($testData = null){
        ...
        if(!empty($user_id)){
            ...
            if(!$userData){
                ...
                $reservationDataFind = $this->Reservation->find('all',array('conditions'=>array( 'Reservation.user_id'=>$user_id, 'Reservation.reservation_type !='=>'3', 'Reservation.customer_id !='=>'', 'Reservation.status !='=>'4'),  'order' => array('Reservation.start_date' => 'DESC')));
                ...
                if(!empty($reservationDataFind)){
                    ...
                    foreach ($reservationDateArr as $dateKey => $dateValue) {
                        ...
                        foreach ($reservationDateDataFind as $key => $value) {
                            $date = $dateValue;
                            if(!empty($value['Customer']['last_name']) || !empty($value['Customer']['first_name'])){
...
/**************************************************************************
    ...
    function get_date_reservation_list_olds($testData = null){
        ...
        if(!empty($user_id)){
            ...
            if(!$userData){
                ...
                $conditions["Reservation.user_id"] = $user_id;
                $conditions["Reservation.customer_id != "] = '';
                ...
                $array_id_count = $staff_count = $staff = $event = $employee = 0; 
                $empIds = array();
                // pr($reservationDataFind);die;
                if(!empty($reservationDataFind)){
                    foreach ($reservationDataFind as $key => $value) {
     ...
     function get_date_reservation_list($testData = null){
        ...
        if(!empty($user_id)){
            ...
            if(!$userData){
                ...
                $reservationDataFind = $this->Reservation->find('all',array('conditions'=>$conditions,  'order' => array('Reservation.start_date' => 'DESC', 'Reservation.start_time' => 'ASC', 'Reservation.status !='=>'4')));
...
function upload_gallery_image(){
        if(!empty($_FILES)){
                ...
                $newName = md5(time()*rand()).'.'.$path_info['extension'];
                // echo $newName;
                echo WWW_ROOT . GALLERY_IMG_THUMB_DIR;die;
                move_uploaded_file($_FILES["note_image"]["tmp_name"], WWW_ROOT . GALLERY_IMG_THUMB_DIR);

                // if () {
                //     echo "The file ". basename( $_FILES["note_image"]["name"]). " has been uploaded.";
                // }
