...
*
*****************************************************************************/
class WebServicesv39Controller extends AppController{
 ...
 /**************************************************************************
    ...
    function attendance_info($testData = null){
     ...
     function attendance_info_download($testData = null){
        ...
        $mail->addStringAttachment($string, 'attendance_sheet.xls');
        pr($mail);
        $this->loadModel('Template');
    ...
    function add_reservation(){
       ...
       // print_r($decoded);die;
        ...
        if(!empty($reservation_type) && ($reservation_type =='1')){
            ...
            // $conditions["Reservation.user_id"] = $user_id;
            // $conditions["Reservation.employee_ids"] = $employee_ids;
            // $conditions['Reservation.start_time <='] = $start_time;
            // $conditions['Reservation.start_time >='] = $end_time;
            // $conditions['Reservation.end_time <='] = $start_time;
            // $conditions['Reservation.end_time <='] = $end_time;
            // pr($conditions);die;
        ...
        if(!empty($reservation_type) && ($reservation_type =='1')){
            ...
            if(isset($existData['Reservation']['id'])){
        ...
        if($this->Reservation->saveAll($reservation)){
            ...
            if(!empty($customer_name)){
                ...
                // $reservationNotificationMessage = $staff_name.' 様が '.$start_date.' ('.$dys[$dy].') '.$start_time.' ～ '.$end_time. ' 所要時間[ '.$duration.' ]  に予約しました';
                $reservationNotificationMessage = $staff_name.' が '.$start_date.' ('.$dys[$dy].') のシフトを更新しました';
     ...
     function get_reservation_ipad($testData = null){
        ...
        if(!empty($user_id)){
            ...
            if(!$userData){
                ...
                $this->Reservation->bindModel(array('belongsTo' => array('Customer', 'Service')));
                $reservationDataFind = $this->Reservation->find('all',array('conditions'=>array( 'Reservation.user_id'=>$user_id, 'Reservation.start_date <=' => $date, 'Reservation.end_date >=' => $date),  'order' => array('Reservation.start_date' => 'DESC', 'Reservation.start_time' => 'ASC')));
    ...
    function calender_reservation_old($testData = null){
        ...
        if(!empty($user_id)){
            ...
            if(!$userData){
                ...
                $reservationDataFind = $this->Reservation->find('all',array('conditions'=>array( 'Reservation.user_id'=>$user_id, 'Reservation.reservation_type !='=>'3', 'Reservation.status !='=>'4'),  'order' => array('Reservation.start_date' => 'DESC')));
...
/**************************************************************************
    ...
    function get_date_reservation_list_azad($testData = null){
        ...
        if(!empty($user_id)){
            ...
            if(!$userData){
                ...
                $reservationDataFind = $this->Reservation->find('all',array('conditions'=>$conditions,  'order' => array('Reservation.start_date' => 'DESC', 'Reservation.start_time' => 'ASC')));
                 // pr($reservationDataFind);die;
                $empReservationData = $reservationData = array();
                $staff_count = $staff = $event = $employee = 0; 
                $array_id_emp = array();
                $array_id_count = 0;
                if(!empty($reservationDataFind)){
                    foreach ($reservationDataFind as $key => $value) {
                            ...
                            if($value['Reservation']['reservation_type']== '3'){
                                ...
                                if(!empty($allEmp)){
                                    // $reservationData[$sectionType]['count'] = count($allEmp);
                                    foreach ($allEmp as $empKey => $empValue) {
                                        if(!in_array($empValue['Employee']['id'], $array_id_emp)){
                                          $reservationData[$sectionType][$staff]['Employee'][$staff_count]['id']= $empValue['Employee']['id'];
                                          $reservationData[$sectionType][$staff]['Employee'][$staff_count]['name']= $empValue['Employee']['name'];
                                          $reservationData[$sectionType][$staff]['Employee'][$staff_count]['emp_code']= $empValue['Employee']['emp_code'];
                                          $reservationData[$sectionType][$staff]['Employee'][$staff_count]['image']= $empValue['Employee']['image'];
                                           $staff_count++;
                                           $array_id_emp[$array_id_count] = $empValue['Employee']['id'];
                                           $array_id_count++;
                                        }
                                        
                                    }
                                }else{
                                     $reservationData[$sectionType]['Employee'][$staff]= array();
                                 }
                                ...
                                if(!empty($allEmp)){
                                    foreach ($allEmp as $empKey => $empValue) {
                                        ...
                                        if(!empty($serviceColor['ServiceColor']['color_code'])){
                                            $empReservationData[$emp_name][$employee]['service_color']= $serviceColor['ServiceColor']['color_code'];
                                        }else{
                                            $empReservationData[$emp_name][$employee]['service_color']= '';
                                        }
...
/************************ Push Notification ***********************/
    ...
    public function IOSPushNotification($deviceToken=null,$message=null,$user_id=null,$employee_id=null,$customer_id=null,  $reservation_id=null, $device_type=null, $member_type=null, $notification_type=null) {
        ...
        if($notification_type == 'add_reservation'){
            $reservationData = $this->Reservation->find('first',array('conditions'=>array('Reservation.id'=>$reservation_id, 'Reservation.user_id'=>$user_id)));
