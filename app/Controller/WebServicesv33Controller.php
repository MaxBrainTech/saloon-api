...
*
*****************************************************************************/
class WebServicesv33Controller extends AppController{
        ...
        if(!empty($user_id)){
            ...
            if(isset($data['User']['stripe_payment_status']) && ($data['User']['stripe_payment_status'] == '1')){
                ...
                if($current_date>$subscription_current_period_end){
                    $responseArr = array('notification_count' => $notification_count, 'subscription_status' => '0', 'status' => 'success' );
                    $jsonEncode = json_encode($responseArr);
                }else{
                    $responseArr = array('notification_count' => $notification_count, 'subscription_status' => '1', 'status' => 'success' );
                    $jsonEncode = json_encode($responseArr);
                }
            }else{
                $responseArr = array('notification_count' => $notification_count, 'subscription_status' => '0', 'status' => 'success' );
    ...
    function add_note($test_data = null){
        ...
        if(!empty($note_image_id)){
            ...
            foreach ($employeeData as $empKey => $empValue) {
                ...
                $noteNotificationMessage = $empName.' コメントを追加しました';
                ...
                if(!empty($reservationDataFind)){
                    ...
                    foreach ($reservationDateArr as $dateKey => $dateValue) {
                        ...
                        foreach ($reservationDateDataFind as $key => $value) {
                            ...
                            // pr($reservationDateStatusDataFind);die;
