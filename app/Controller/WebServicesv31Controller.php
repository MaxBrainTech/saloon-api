...
*
*****************************************************************************/
class WebServicesv31Controller extends AppController{
        ...
        if(!empty($user_id)){
            ...
            if(isset($data['User']['stripe_payment_status']) && ($data['User']['stripe_payment_status'] == '1')){
                ...
                if($current_date>$subscription_current_period_end){
                    $responseArr = array('subscription_status' => '0', 'status' => 'success' );
                    $jsonEncode = json_encode($responseArr);
                }else{
                    $responseArr = array('subscription_status' => '1', 'status' => 'success' );
                    $jsonEncode = json_encode($responseArr);
                }
            }else{
                $responseArr = array('subscription_status' => '0', 'status' => 'success' );
    ...
    function employee_notification_list($test_data = null){
        ...
        if(!empty($data)){
            foreach ($data as $key => $value) {
                ...
                $pushNotificationData['PushNotification'][$i]['created'] = ($value['PushNotification']['created'] != null) ? $value['PushNotification']['created'] : '';
                $pushNotificationData['PushNotification'][$i]['modified'] = ($value['PushNotification']['modified'] != null) ? $value['PushNotification']['modified'] : '';
                $pushNotificationData['PushNotification'][$i]['read_status'] = ($value['PushNotification']['read_status'] != null) ? $value['PushNotification']['read_status'] : '';
                $i++;
            }
...
/************************ Push Notification ***********************/
    ...
    function notitifation_read_status($test_data = null){
        ...
        if(!empty($id)){
            ...
            if($this->PushNotification->saveField('read_status' , 1 )){
        ...
        $log = $this->PushNotification->getDataSource()->getLog(false, false);
        $recordData['RecordData']['name'] = "notitifation_read_status";
        $recordData['RecordData']['query'] = json_encode($log);
        $this->RecordData->saveAll($recordData);
        echo  $jsonEncode;exit();
