<?php
	 ...
	 */
	 ...
	 class TwitterComponent extends Component {
		...
		private $consumer_key = "zK0cNwxMnPaGvpTQ1vxPEg";
		private $consumer_secret = "aDjtcGZmX5rCU6RJrt1sxm2LbLSHGHNZfbhmHwU0";
		 ...
		 */		
		 ...
		 public function connectApp($callback_url, $action='authorize'){
			$request = array(
			  ...
			  //echo "Location: http://api.twitter.com/oauth/$action?oauth_token={$response['oauth_token']}";die;
		...
		public function loginTwitterUser($oauth_token, $oauth_token_secret){
			//pr(CakeSession::write('Twitter.User'));die;
			//if(CakeSession::check('Twitter.User')){
				//Update class vars
		...
		public function accountVerifyCredentials() {
			//Request & return
			return json_decode($this->apiRequest('get', '/1.1/account/verify_credentials.json', ''), true);
		}
		...
		public function accountRateLimitStatus() {
			//Request & return
			return json_decode($this->apiRequest('get', '/1.1/account/rate_limit_status.json', ''), true);
		}
		...
		public function getDirectMessages() {
			//Return 
			return json_decode($this->apiRequest('get', '/1.1/direct_messages.json', ''), true);
		}
		...
		public function getDirectMessagesSent() {
			//Return 
			return json_decode($this->apiRequest('get','/1.1/direct_messages/sent.json', ''), true);
		}
		...
		public function newDirectMessage($screen_name, $text) {
			...
			//Return and request
			return json_decode($this->apiRequest('post', '/1.1/direct_messages/new.json', $body), true);
		}
		...
		public function destroyDirectMessage($id) {
			//Return 
			return json_decode($this->apiRequest('delete', '/1.1/direct_messages/destroy/'.$id.'.json', ''), true);
		}
		...
		public function getFriendsIds($screen_name) {
			...
			$body['screen_name'] = strtolower($screen_name);
			//Return and request
			return json_decode($this->apiRequest('get', '/1.1/friends/ids.json', $body), true);
		}
		...
		public function getFollowersIds($screen_name) {
			...
			$body['screen_name'] = strtolower($screen_name);
			//Return and request
			return json_decode($this->apiRequest('get', '/1.1/followers/ids.json', $body), true);	
		}
		...
		public function createFriendship($screen_name) {
			...
			if(!is_numeric($screen_name)) { 
				...
				$body['screen_name'] = strtolower($screen_name);
				//Return and request
				return json_decode($this->apiRequest('post', '/1.1/friendships/create.json', $body), true);
			}
		}
		/*
		...
		public function destroyFriendship($screen_name) {
			...
			if(!is_numeric($screen_name)) {
				...
				$body['screen_name'] = strtolower($screen_name);
				//Return and request
				return json_decode($this->apiRequest('post', '/1.1/friendships/destroy.json', $body), true);	
			}	
		}
		/*
		...
		public function friendshipExists($user_a, $user_b) {
			...
			$body['user_b'] = $user_b;
			//Return and request 
			return json_decode($this->apiRequest('get', '/1.1/friendships/exists.json', $body), true);	
		}
		...
		public function showStatus($id) {
			if(is_numeric($id)) {
				//Return and request
				return json_decode($this->apiRequest('get', '/1.1/statuses/show/'.$id.'.json', ''), true);
			}
		}
		...
		public function updateStatus($status) {
			if($status != null || $status != '') {
				...
				//Return and request
				return json_decode($this->apiRequest('post', '/1.1/statuses/update.json', $body), true);
			}
		}
		...
		public function destroyStatus($id) {
			if(is_numeric($id)) {
				//Return and request
				return json_decode($this->apiRequest('post', '/1.1/statuses/destroy/'.$id.'.json', ''), true);
			}
		}
		...
		public function publicTimeline() {
			//Request & Return
			return json_decode($this->apiRequest('get', '/1.1/statuses/public_timeline.json', ''), true);	
		}
		...
		public function friendsTimeline() {
			//Return and request
			return json_decode($this->apiRequest('get', '/1.1/statuses/friends_timeline.json', ''), true);
		}
		...
		public function homeTimeline() {
			//Return and request
			return json_decode($this->apiRequest('get', '/1.1/statuses/home_timeline.json', ''), true);
		}
		...
		public function userTimeline($param = null) {
			...
			//Return and request
			return json_decode($this->apiRequest('get', '/1.1/statuses/user_timeline.json', $body), true);
		}
		...
		public function mentionsTimeline() {
			//Return and request
			return json_decode($this->apiRequest('get', '/1.1/statuses/mentions.json', ''), true);
		}
		...
		public function showUser($param){
			...
			//Return and request
			return json_decode($this->apiRequest('get', '/1.1/users/show.json', $body), true);
		}
		//-----
	 }
?>
