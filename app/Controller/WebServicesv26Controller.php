...
*
*****************************************************************************/
class WebServicesv26Controller extends AppController{
...
/****************************************************************************************************************************************
    ...
    function employee_logout($test_data = null){
        ...
        if(!empty($employee_id)){
            $employeeCode = $this->Employee->find('first', array('conditions'=>array('Employee.id'=>$employee_id, 'Employee.status' =>Configure::read('App.Status.active')),'fields'=>array('Employee.emp_code','Employee.user_id')));
            if(isset($employeeCode['Employee']['user_id']) && !empty($employeeCode['Employee']['user_id']) ){
                $this->Employee->id = $employeeCode['Employee']['id'];
...
/****************************************************************************************************************************************
    ...
    function add_employee_chat($test_data = null){
        ...
        if($this->EmployeeChat->saveAll($employeeChat)){
            ...
            if($chat_type == '2'){
                $chatNotificationMessage = $emp_name.' 送信する '.$chat_text;
            }else{
                 $chatNotificationMessage = $emp_name.' 画像を送る';
            }
