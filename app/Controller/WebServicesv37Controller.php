...
*
*****************************************************************************/
class WebServicesv37Controller extends AppController{
   ...
   /************************************************************************************************************************************
        ...
        if(!empty($user_id)){
            ...
            if(isset($data['User']['stripe_payment_status']) && ($data['User']['stripe_payment_status'] == '1')){
                ...
                if($current_date>$subscription_current_period_end){
                    ...
                    $responseArr = array('notification_count' => $notification_count, 'service_name' => $service_name, 'service_id' => $service_id, 'subscription_status' => '1', 'employee_status' => $employee_status, 'status' => 'success' , 'msg' => 'Subscription plan working successfully.' );
                    $jsonEncode = json_encode($responseArr);
                }
            }else{
                if($employee_status ==1){
                      $responseArr = array('notification_count' => $notification_count, 'service_name' => $service_name, 'service_id' => $service_id, 'subscription_status' => '1', 'employee_status' => $employee_status, 'status' => 'success'  , 'msg' => 'Employee login successfully.');
                }else{
                    $responseArr = array('notification_count' => $notification_count, 'service_name' => $service_name, 'service_id' => $service_id, 'subscription_status' => '1', 'employee_status' => $employee_status, 'status' => 'success'  , 'msg' => 'Your account has been deactiveted.');
                }
    ...
    function signup($test_data = null){
        ...
        if($this->User->saveAll($user)){
            ...
            // $activation_url = Router::url(array('controller' => 'users','action' => 'email_confirm', $user['User']['email'], $verification_code,), true);
            // echo $activation_url;die;
            $activation_url = 'http://api.jtsboard.com/users/email_confirm/'.$user_id.'/'.$activationCode;
            // $activation_url = 'jtsboard://?user_id='.$user_id;
            // $activation_url = Router::url(array('controller' => 'users', 'action' => 'email_confirm', $user_id));
            // $activation_url = 'http://api.jtsboard.com/users/email_confirm/'.$verification_code;
            ...
            $mail_message = str_replace(array('{NAME}', '{EMAIL}','{PASSWORD}','{ACTIVATION_LINK}', '{activation_code}',  '{SITE}'), array($user['User']['salon_name'], $email, $enter_password,$activation_link, $activationCode, Configure::read('App.SITENAME')), $registrationMail['Template']['content']);
            $template = 'default';
     ...
     *************************************************************************************************************************************/
     function signup_step2($test_data = null){
        ...
        $defult_services = isset($decoded['defult_services']) ? $decoded['defult_services'] : '';
            ...
            if(!empty($user_id)){
                ...
                if(!empty($defult_services)){
                    ...
                    $this->loadModel('MainService');
                     /* Load Model End*/
                    $mainServiceData =$this->MainService->find('all',array('conditions'=>array( 'MainService.id'=>$defult_services)));
...
/************************ Push Notification ***********************/
    ...
    function reservation_status($test_data = null){
        ...
        if(!empty($user_id) && !empty($date) && !empty($employee_id)){
            ...
            $responseArr = array('status' => 'error', 'msg' => '予約がありません', 'msg1' => 'Reservation does not exist.'  );
            $jsonEncode = json_encode($responseArr);
        }
   ...
   function get_salon_profile($testData = null){
     ...
     *********************************************************************/
     ...
     function add_cancel_plan($test_data = null){
        ...
        $cancel_plan['PlanCancelRequest']['expensive'] = (isset($decoded['expensive']) && !empty($decoded['expensive'])) ? $decoded['expensive'] : '';
        $cancel_plan['PlanCancelRequest']['not_fil_my_need'] = (isset($decoded['not_fil_my_need']) && !empty($decoded['not_fil_my_need'])) ? $decoded['not_fil_my_need'] : '';
        $cancel_plan['PlanCancelRequest']['found_better'] = (isset($decoded['found_better']) && !empty($decoded['found_better'])) ? $decoded['found_better'] : '';
        $cancel_plan['PlanCancelRequest']['closing_salon'] = (isset($decoded['closing_salon']) && !empty($decoded['closing_salon'])) ? $decoded['closing_salon'] : '';
        $cancel_plan['PlanCancelRequest']['hard_to_use'] = (isset($decoded['hard_to_use']) && !empty($decoded['hard_to_use'])) ? $decoded['hard_to_use'] : '';
        $cancel_plan['PlanCancelRequest']['reason'] = (isset($decoded['reason']) && !empty($decoded['reason'])) ? $decoded['reason'] : '';
    ...
    function add_employee_image($test_data = null){
        ...
        if(isset($_FILES['image']) && !empty($_FILES['image'])){
            ...
            $responseArr = array('user_id' => $user_id, 'employee_id' => $employee_id, 'status' => 'error', 'msg'=>'Employee does not exist.');
            $jsonEncode = json_encode($responseArr);
        }
        echo  $jsonEncode;exit();
    }
