...
*
*****************************************************************************/
class WebServicesv7Controller extends AppController{
     ...
     *********************************************************************/
      ...
      function customer_service_list($testData = null){
        ...
        if(!empty($customer_id)){
            $this->Customer->bindModel(array('hasMany' => array('Esthe','Eyelush', 'Body', 'HairRemoval', 'PhotoFacial', 'Facial' )));
            ...
            if(!$data){
                ...
                if(isset($data['Facial']) && !empty($data['Facial'])){
                     $photoFacialService = $this->Service->find('first',array('conditions'=> array( 'Service.id'=>6)));
                    foreach ($data['Facial'] as $facialkey => $facialvalue) {
                        $serviceData['Service'][$i]['service_id'] = "6";
                        $serviceData['Service'][$i]['customer_service_id'] = $facialvalue['id'];
                        $serviceData['Service'][$i]['service_name'] = $this->get_service_name("6");
                        $serviceData['Service'][$i]['date'] = $facialvalue['created'];
                        $i++;
                    }
                }
