        ...
        if(!empty($email) && !empty($password)){
            ...
            if(isset($data['User']['id']) && !empty($data['User']['id'])){
                ...
                if(isset($data['User']['company_name']) && !empty($data['User']['company_name'])){
                    $responseArr = array('user_id' => $data['User']['id'], 'msg' => 'Login Sccuessfully.', 'employee_pin_number'=>$employee_pin_number,  'customer_pin_number'=>$customer_pin_number, 'is_admin' => strval($data['User']['is_admin']), 'is_form' => '2', 'status' => 'success' );
                }else{
                    $responseArr = array('user_id' => $data['User']['id'], 'msg' => 'Login Sccuessfully.', 'employee_pin_number'=>$employee_pin_number,  'customer_pin_number'=>$customer_pin_number, 'is_admin' => strval($data['User']['is_admin']), 'is_form' => '1', 'status' => 'success' );
                }
    ...
    function edit_profile($test_data = null){
       ...
       // $user['User']['has_branch'] = isset($decoded['has_branch']) ? $decoded['has_branch'] : '';
        ...
        if($this->User->saveAll($user)){
            ...
            $user_id = $this->User->id;
            $responseArr = array('user_id' => $user_id, 'status' => 'success', 'msg' => 'User profile edit successfully.' );
            $jsonEncode = json_encode($responseArr);
            ...
            if(!$data){
                ...
                if(isset($data['Esthe']) && !empty($data['Esthe'])){
                    ...
                    foreach ($data['Esthe'] as $esthekey => $estheValue) {
                        ...
                        $serviceData['Service'][$i]['service_name'] = "Esthe";
                ...
                if(isset($data['Eyelush']) && !empty($data['Eyelush'])){
                    ...
                    foreach ($data['Eyelush'] as $eyelushkey => $eyelushvalue) {
                        ...
                        $serviceData['Service'][$i]['service_name'] = "Eyelush";
                ...
                if(isset($data['Body']) && !empty($data['Body'])){
                    ...
                    foreach ($data['Body'] as $bodykey => $bodyvalue) {
                        ...
                        $serviceData['Service'][$i]['service_name'] = $bodyService['Service']['name'];
                ...
                if(isset($data['HairRemoval']) && !empty($data['HairRemoval'])){
                    ...
                    foreach ($data['HairRemoval'] as $hairRemovalkey => $hairRemovalvalue) {
                        ...
                        $serviceData['Service'][$i]['service_name'] = $hairRemovalService['Service']['name'];
                ...
                if(isset($data['PhotoFacial']) && !empty($data['PhotoFacial'])){
                    ...
                    foreach ($data['PhotoFacial'] as $photoFacialkey => $photoFacialvalue) {
                        ...
                        $serviceData['Service'][$i]['service_name'] = $photoFacialService['Service']['name'];
...
/* New V1 data 08-08-2018 */
    ...
    function customer_analysis($test_data = null){
        ...
        if($this->CustomerHistory->saveAll($customerHistory)){
            ...
            $responseArr = array('customer_analysis_id' => $customer_analysis_id, 'msg'=>'顧客情報が正常に追加されました。', 'status' => 'success' );
            $jsonEncode = json_encode($responseArr);
     ...
     function add_employee($test_data =null){
        ...
        if(isset($employeeExist['Employee']['id']) && !empty($employeeExist['Employee']['id'])){
            $employee['Employee']['id'] = $employeeExist['Employee']['id'];
             ...
             $employee['Employee']['emp_code'] = $this->RandomString();
             $employeeCodeExist = $this->Employee->find('first', array('conditions'=>array('Employee.email'=>$email)));
        }
        ...
        if(isset($employeeCodeExist['Employee']['id']) && !empty($employeeCodeExist['Employee']['id'])){
            ...
            if($this->Employee->saveAll($employee)){
                ...
                $user_id = $decoded['user_id'];
                $responseArr = array('user_id' => $user_id, 'employee_id' => $employee_id, 'status' => 'success' );
                $jsonEncode = json_encode($responseArr);
 ...
 /****************************************************************************************************************************************
     ...
     *********************************************************************/
     ...
     function add_attendance($test_data =null){
        ...
        $emp_time = isset($decoded['date']) ? $decoded['date'] : '';
        if(!empty($emp_code)){
            ...
            if(isset($employeeCode['Employee']['id']) && !empty($employeeCode['Employee']['id'])){
                $attendanceTouchId = $this->Attendance->find('first', array('conditions'=>array('Attendance.emp_code'=>$emp_code, 'Attendance.status'=>1, 'Attendance.date'=>date("Y-m-d")),'fields'=>array('Attendance.emp_code')));
                if(isset($attendanceTouchId['Attendance']['emp_code']) && !empty($attendanceTouchId['Attendance']['emp_code'])){
                    $responseArr = array('status' => 'success',  'msg' => 'あなたはすでに今日チェックアウトしました。' );
                    $jsonEncode = json_encode($responseArr);
                }else{

                    $attendanceTouchIdcheckIn = $this->Attendance->find('first', array('conditions'=>array('Attendance.emp_code'=>$emp_code, 'Attendance.status'=>0, 'Attendance.date'=>date("Y-m-d"))));
                    //$timestamp = date('Y-m-d G:i:s' , $emp_time);
                    $date = date('Y-m-d');
      ...
      function attendance_info($testData = null){
        ...
        if(!empty($user_id) && !empty($employee_id)){
            ...
            if(!$data){
                ...
                if(!empty($data)){
                    foreach ($data as $key => $value) {
                        $customerData['Attendance'][$i]['date'] = $value['Attendance']['date'];
                        if($value['Attendance']['checkin_time']=='NULL' || $value['Attendance']['checkin_time']=='0000-00-00 00:00:00' ||  empty($value['Attendance']['checkin_time']) ){
                            ...
                            $customerData['Attendance'][$i]['checkin_time'] = $value['Attendance']['checkin_time'];
                        }
                        if($value['Attendance']['checkout_time']=='NULL' || $value['Attendance']['checkout_time']=='0000-00-00 00:00:00' || empty($value['Attendance']['checkout_time']) ){
                            ...
                            $customerData['Attendance'][$i]['checkout_time'] = $value['Attendance']['checkout_time'];
                        }
...
/****************************************************************************************************************************************
    ...
    function monthly_attendance($test_data = null){
        ...
        $allDays  = round($datediff / (60 * 60 * 24));
    ...
    function get_user_id($test_data = null){
        ...
        if(!empty($emp_code)){
            $employeeCode = $this->Employee->find('first', array('conditions'=>array('Employee.emp_code'=>$emp_code),'fields'=>array('Employee.emp_code','Employee.user_id')));
