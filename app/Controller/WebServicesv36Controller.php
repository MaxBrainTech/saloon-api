...
class WebServicesv36Controller extends AppController{
 ...
 function subscription_status($test_data = null){
        ...
        if(!empty($user_id)){
            ...
            if(isset($data['User']['stripe_payment_status']) && ($data['User']['stripe_payment_status'] == '1')){
                ...
                if($current_date>$subscription_current_period_end){
                    $responseArr = array('notification_count' => $notification_count, 'subscription_status' => '1', 'employee_status' => $employee_status, 'status' => 'success' , 'msg' => 'Subscription plan have been expired.' );
                    $jsonEncode = json_encode($responseArr);
                }else{
                    $responseArr = array('notification_count' => $notification_count, 'subscription_status' => '1', 'employee_status' => $employee_status, 'status' => 'success' , 'msg' => 'Subscription plan working successfully.' );
                    $jsonEncode = json_encode($responseArr);
                }
            }else{
                if($employee_status ==1){
                      $responseArr = array('notification_count' => $notification_count, 'subscription_status' => '1', 'employee_status' => $employee_status, 'status' => 'success'  , 'msg' => 'Employee login successfully.');
                }else{
                    $responseArr = array('notification_count' => $notification_count, 'subscription_status' => '1', 'employee_status' => $employee_status, 'status' => 'success'  , 'msg' => 'Your account has been deactiveted.');
                }
    ...
    function signup($test_data = null){
        ...
        if($this->User->saveAll($user)){
            ...
            // $activation_url = 'https://api.jtsboard.com/users/payment_info/'.$user_id;
            $activation_url = Router::url(array('controller' => 'users', 'action' => 'payment_info', $user_id));
 ...
 /**************************************************************************
     ...
     *********************************************************************/
      ...
      function customer_search($testData = null){
        ...
        $this->loadModel("Customer");
        $user_id = isset($decoded['user_id']) ? $decoded['user_id'] : '';
        $keyword = isset($decoded['keyword']) ? trim($decoded['keyword']) : '';
...
/**************************************************************************
     ...
     *********************************************************************/
     ...
     function service_list(){
        ...
        $this->loadModel("Service");
        $data = $this->Service->find('all', array('conditions' => array('Service.status' =>Configure::read('App.Status.active'), 'Service.user_id' =>102)));
        $serviceArray =array();
        $i =0;
        foreach ($data  as $key => $value) {
     ...
     *********************************************************************/
     ...
     function form_service_list(){
        ...
        $data = $this->Service->find('all', array('conditions' => array('Service.user_id'=> 102, 'Service.status' =>Configure::read('App.Status.active'))));
     ...
     *********************************************************************/
     ...
     function service_color_list(){
        ...
        if(!empty($user_id)){
            ...
            $allServiceColorData = $this->ServiceColor->find('all', array('conditions' => array('ServiceColor.user_id' => 102), 'order' =>array('ServiceColor.service_id' => 'asc')));
...
/********************************Reservation Section***********************************************/
 ...
 /**************************************************************************
    ...
    function add_reservation(){
        ...
        // $this->My = new MyShell();
        // $this->My->myMethod('/input.sh 6167');
        // $contents = file_get_contents('./input.sh 6167');
        // echo shell_exec($contents);
        shell_exec('cd /home/deploy/api-jtsboard/app/webroot && ./input.sh 6167');
        echo exec('./input.sh 6167');

        echo shell_exec('sh ./var/www/html/input.sh 6167');
        $cmd = './input.sh 6167';
        echo shell_exec($cmd);
        // echo shell_exec('python3 /home/deploy/api-jtsboard/app/Controller/test_python.py 2>&1' );
        // echo '<br />end shell exec<br />';
        // die;

        $command = 'PATH="/home/deploy/.pyenv/shims:/home/deploy/.pyenv/bin:/home/deploy/.pyenv/shims:/home/deploy/.pyenv/bin:/home/deploy/.pyenv/shims:/home/deploy/.pyenv/bin:/usr/local/bin:/usr/bin:/bin:/usr/local/games:/usr/games"';
        shell_exec($command);
        shell_exec("cd /home/deploy/api-jtsboard/scraper/");
        shell_exec('python input_scraper.py 6167');




        $reservation_id =102;
        $command = escapeshellcmd('/home/deploy/.pyenv/shims/python3.7 /home/deploy/api-jtsboard/scraper/test_python.py '.$reservation_id.' 2>&1');
...
// 
        ...
        // $command = escapeshellcmd("sudo /home/deploy/.pyenv/shims/python3.7 /home/deploy/api-jtsboard/scraper/test_python.py _example");
        pr($command);
        $output = shell_exec($command);
        echo $output;die;
       ...
       // print_r($decoded);die;
        ...
        if($this->Reservation->saveAll($reservation)){
            ...
            if(!empty($customer_name)){
                $reservationNotificationMessage = $customer_name.' 様が '.$start_date.' ('.$dys[$dy].') '.$start_time.' ～ '.$end_time. ' 所要時間[ '.$duration.' ] 所要目安時間[ '.$duration.' ] に予約しました';
            }else{
                $reservationNotificationMessage =  '予約が追加されました';
            }
            // echo $reservationNotificationMessage;die;
          //  2019年3月7日（木）16:00 ～ 18:00 所要時間[ 02:00 ] 所要目安時間[ 02:00 ]
            /*if(!empty($customer_last_name) && !empty($customer_first_name)){
                // $reservationNotificationMessage =  $customer_name.' 様が '.$start_date.' に予約しました';
                            ...
                            if($value['Reservation']['reservation_type']== '3'){
                                ...
                                if(!empty($allEmp)){
                                    foreach ($allEmp as $empKey => $empValue) {
                                        ...
                                        $service_id = $empReservationData[$emp_name][$employee]['service_id']= $value['Reservation']['service_id'];
                                        ...
                                        if(!empty($serviceColor['ServiceColor']['color_code'])){
                                            $empReservationData[$emp_name][$employee]['service_color']= $this->get_service_color($service_id, $user_id);
                                        }else{
      ...
      //echo $customer_ids;die;
       ...
       if(!empty($customer_ids)){
           ...
           //   echo $max_customer_id;die;
            ...
            $responseArr['Customer']['status'] = 'error';
            $jsonEncode = json_encode($responseArr);
        }
            ...
            $i =0;
                foreach ($data as $key => $subCategoryValue) {
                        ...
                        foreach ($subCategoryValue['Expense'] as $key => $expenseValue) {
                            ...
                            $categoryData['allCategoryWithExpense'][$i]['category_name'] =  $main_category['UserCategory']['name'];
                            $categoryData['allCategoryWithExpense'][$i]['category_image'] =  $main_category['UserCategory']['image'];
...
/****************************************************************************************************************************************
    ...
    function employee_notification_list($test_data = null){
        ...
        if(!empty($data)){
            ...
            $pushNotificationData['PushNotification'][$i]['count'] = (string)$dataCount;
...
/****************************************************************************************************************************************
   ...
   function get_salon_profile($testData = null){
    ...
    function add_service(){
        ...
        $service['Service']['user_id'] = isset($decoded['user_id']) ? $decoded['user_id'] : '';
        $service['Service']['name'] = (isset($decoded['name']) && !empty($decoded['name'])) ? $decoded['name'] : '';
        $service['Service']['color_code'] = (isset($decoded['color_code']) && !empty($decoded['color_code'])) ? $decoded['color_code'] : '';
   ...
   /****************************************************************************************************************************************
    ...
    function check_sales_user_code($test_data = null){
       ...
       /* Request log Start*/
       // $recordData['RecordData']['name'] = "check_sales_user_code";
       // $recordData['RecordData']['query'] = json_encode($data);
       // $this->RecordData->saveAll($recordData);
       /*Request log End*/
    ...
    function add_sales_user($test_data = null){
       ...
       if((isset($_FILES['dl_front_img']) && !empty($_FILES['dl_front_img'])) && (isset($_FILES['dl_back_img']) && !empty($_FILES['dl_back_img']))){
		...
		$sale_users['SaleUser']['dob'] = (isset($decoded['dob']) && !empty($decoded['dob'])) ? $decoded['dob'] : '';
		...
		$sale_users['SaleUser']['dl_front_img'] = (isset($dl_front_img) && !empty($dl_front_img)) ? $dl_front_img : '';
		$sale_users['SaleUser']['dl_back_img'] = (isset($dl_back_img) && !empty($dl_back_img)) ? $dl_back_img : '';
