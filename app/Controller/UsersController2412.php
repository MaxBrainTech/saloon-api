            ...
            if (!empty($this->request->data)) {
                if(!empty($this->request->data['User']['image']['name'])){
                    if($this->request->data['User']['image']['size'] <  6291456){
                        ...
                        $this->request->data['User']['image']['name'] = $path_info['filename']."_".time().".".$path_info['extension'];
                        $res3 = $this->Upload->upload($this->request->data['User']['image'], WWW_ROOT ."uploads/note_image/original". DS, '', '', array('png', 'jpg', 'jpeg', 'gif'));
                ...
                if ($this->User->validates()) {
                    if ($this->User->save($this->request->data)) {
                        $this->Session->setFlash(__('情報を変更しました', true), 'admin_flash_success');
                        $this->redirect(array('controller' => 'users','action' => 'app_edit', $id));
                    } else {
                        $this->Session->setFlash(__('管理者を保存できませんでした。 やり直してください。', true), 'admin_flash_error');
                    }
                } else {
                    $this->Session->setFlash(__('管理者を保存できませんでした。 エラーを修正してください。', true), 'admin_flash_error');
                }
            }
        } else {
            $this->request->data = $this->User->read(null, $id);
