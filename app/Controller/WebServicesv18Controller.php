...
*
*****************************************************************************/
class WebServicesv18Controller extends AppController{
...
/****************************************************************************************************************************************
     ...
     *************************************************************************************************************************************/
     ...
     function add_note_ticket($test_data = null){
        ...
        $ticket_amount = $noteTicket['NoteTicket']['ticket_num_time'] = isset($decoded['ticket_num_time']) ? $decoded['ticket_num_time'] : '';
      ...
      function calender_reservation($testData = null){
        ...
        if(!empty($user_id)){
            ...
            if(!$userData){
                ...
                if(!empty($reservationDataFind)){
                    foreach ($reservationDataFind as $key => $value) {
                        $dateArray = $this->dateRange( $value['Reservation']['start_date'], $value['Reservation']['end_date']);
                      //  print_r($dateArray);die;
                        foreach ($dateArray as $dateKey => $dateValue) {
                            if(!isset($reservationData[$dateValue]) && empty($reservationData[$dateValue])){
                                $i= 0;
                                $date = $dateValue;
                            }
                       /* for ($i = $startTime; $i <= $endTime; $i = $i + 86400 ) {
...
function upload_gallery_image(){
        if(!empty($_FILES)){
            ...
            if($_FILES['note_image']['size'] <  6291456){
                ...
                $_FILES['note_image']['name'] = $path_info['filename']."_".time().".".$path_info['extension'];
                $res3 = $this->Upload->upload($_FILES['note_image'], WWW_ROOT . GALLERY_IMAGE . DS ."original". DS, '', '', array('png', 'jpg', 'jpeg', 'gif'));
    ...
    function add_gallery_image($test_data = null){
        ...
        $gallery['Gallery']['user_id'] = isset($decoded['user_id']) ? $decoded['user_id'] : '';
    ...
    function list_gallery_image($test_data = null){
        ...
        $data = $this->Gallery->find('all',array('conditions'=> array( 'Gallery.user_id'=>$user_id), 'order' =>array('Gallery.created'=>'DESC')));
