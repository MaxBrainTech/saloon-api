...
*
*****************************************************************************/
class WebServicesv14Controller extends AppController{
...
/*
    ...
    function edit_profile($test_data = null){
       ...
       // $user['User']['has_branch'] = isset($decoded['has_branch']) ? $decoded['has_branch'] : '';
        ...
        $user['User']['cash_box'] = isset($decoded['cash_box']) ? $decoded['cash_box'] : '';
            ...
            if(!empty($user_id) && !empty($customer_id)){
                ...
                if(isset($noteTicketData[0]) && !empty($noteTicketData[0])){
                    foreach ($noteTicketData as $noteTicketKey => $noteTicketValue) {
                        ...
                        if(($noteTicketAmount > 0) && empty($ticketName)){
                            $ticketName = $noteTicketValue['NoteTicket']['ticket_name'];
                        }

                        $totalNoteTicketAmount  = ($totalNoteTicketAmount  + $noteTicketAmount);
...
/**************************************************************************
    ...
    function get_payment_method($testData = null){
        ...
        $user_id = isset($decoded['user_id']) ? $decoded['user_id'] : '';
        $customer_id = isset($decoded['customer_id']) ? $decoded['customer_id'] : '';
        $service_id = isset($decoded['service_id']) ? $decoded['service_id'] : '0';
        $responseArr  = array();
        if(!empty($user_id) && !empty($customer_id)){
            $conditions["NoteTicket.user_id"] = $user_id;
            $conditions["NoteTicket.customer_id"] = $customer_id;
            $conditions['NoteTicket.service_id'] = array($service_id, "0");
      ...
      /*  $totalNoteTicketAmount = 0;
        if(!empty($user_id) && !empty($customer_id)){
            ...
            if(isset($noteTicketData[0]) && !empty($noteTicketData[0])){
                foreach ($noteTicketData as $noteTicketKey => $noteTicketValue) {
                    ...
                    $noteTicketAmount = $this->priceChangeInt($noteTicketValue['NoteTicket']['ticket_amount']); 
                    $totalNoteTicketAmount  = ($totalNoteTicketAmount  + $noteTicketAmount);
                    if($noteTicketAmount >= $service_price ){
                        $ticketAmount = $noteTicketAmount - $service_price ;
                        $this->NoteTicket->id = $noteTicketValue['NoteTicket']['id'];
        ...
        if($this->NoteService->saveAll($noteService)){
            ...
            if(!empty($ticket_id)){
                ...
                if(isset($noteTicketData['NoteTicket']['service_id']) && ($noteTicketData['NoteTicket']['service_id'] !=0) && ($noteTicketData['NoteTicket']['ticket_num_time'] > 0)){
                   ...
                   if($ticket_amount >= $service_price ){

                        $ticketAmount = ($ticket_amount - $service_price) ;
                        $this->NoteTicket->id = $noteTicketData['NoteTicket']['id'];
                        $this->NoteTicket->saveField('ticket_amount' , number_format($ticketAmount).'円' );
                    }
                }
...
/****************************************************************************************************************************************
     ...
     *************************************************************************************************************************************/
     ...
     function add_note_product($test_data = null){
        ...
        $noteProduct['NoteProduct']['customer_id'] = isset($decoded['customer_id']) ? $decoded['customer_id'] : '0';
        ...
        if($this->NoteProduct->saveAll($noteProduct)){
            ...
            if(!empty($ticket_id)){
                ...
                $ticket_price = $this->priceChangeInt($noteTicketData['NoteTicket']['ticket_price']); 
                if($ticket_price >= $sale_price ){
                    $ticketAmount = ($ticket_price - $sale_price) ;
                    $this->NoteTicket->id = $noteTicketData['NoteTicket']['id'];
                    $this->NoteTicket->saveField('ticket_amount' , number_format($ticketAmount).'円' );
                }else{
     ...
     *************************************************************************************************************************************/
     ...
     function add_note_ticket($test_data = null){
        ...
        $noteTicket['NoteTicket']['ticket_amount'] = isset($decoded['ticket_amount']) ? $decoded['ticket_amount'] : '';
     ...
     *************************************************************************************************************************************/
     ...
     function edit_note_ticket($test_data = null){
        ...
        $noteTicket['NoteTicket']['customer_id'] = isset($decoded['customer_id']) ? $decoded['customer_id'] : '';
        $noteTicket['NoteTicket']['ticket_id'] = isset($decoded['ticket_id']) ? $decoded['ticket_id'] : '0';
        $noteTicket['NoteTicket']['ticket_name'] = isset($decoded['ticket_name']) ? $decoded['ticket_name'] : '';
        $noteTicket['NoteTicket']['employee_id'] = isset($decoded['employee_id']) ? $decoded['employee_id'] : '';
        $noteTicket['NoteTicket']['employee_name'] = isset($decoded['employee_name']) ? $decoded['employee_name'] : '';
        $noteTicket['NoteTicket']['ticket_price'] = isset($decoded['ticket_price']) ? $decoded['ticket_price'] : '';
    ...
    function get_reservation_ipad($testData = null){
        ...
        if(!empty($user_id)){
            ...
            if(!$userData){
                ...
                $reservationDataFind = $this->Reservation->find('all',array('conditions'=>array( 'Reservation.user_id'=>$user_id, 'Reservation.start_date <=' => date('Y-m-d')),  'order' => array('Reservation.start_date' => 'DESC', 'Reservation.start_time' => 'ASC')));
                ...
                if(!empty($reservationDataFind)){
                    foreach ($reservationDataFind as $key => $value) {
                       ...
                       if(!empty($value['Reservation']['service_id'])){
                            ...
                            $reservationData[$date][$i]['service_id']= $value['Reservation']['service_id'];
                            $reservationData[$date][$i]['user_id']= $user_id;
                            if(isset($serviceColor['ServiceColor']['color_code']) && !empty($serviceColor['ServiceColor']['color_code'])){
                                $reservationData[$date][$i]['color']= $serviceColor['ServiceColor']['color_code'];
                            }else{
      ...
      function get_date_reservation($testData = null){
        ...
        if(!empty($user_id)){
            ...
            if(!$userData){
                ...
                $this->Reservation->bindModel(array('belongsTo' => array('Customer', 'Service')));
                $reservationDataFind = $this->Reservation->find('all',array('conditions'=>array( 'Reservation.start_date'=>$date, 'Reservation.user_id'=>$user_id)));
                //echo "<pre>";
                //print_r($reservationDataFind);die;
                $reservationData = array();
                    ...
                    foreach ($reservationDataFind as $key => $value) {
                        ...
                        if($value['Reservation']['reservation_type']== '3'){
                              $sectionType ='event';
                                ...
                                $reservationData[$date][$i][$sectionType][$event]['customer_name']= $value['Customer']['first_name'].' '.$value['Customer']['last_name'];
                                $last_visit =  isset($value['Customer']['last_visit']) ? $value['Customer']['last_visit'] : $value['Customer']['modified'];
                                if(empty($last_visit))
                                    $last_visit =  'First Time Coming';
