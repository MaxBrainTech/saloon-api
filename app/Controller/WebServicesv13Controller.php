...
*
*****************************************************************************/
class WebServicesv13Controller extends AppController{
                ...
                if(!empty($reservationDataFind)){
                    foreach ($reservationDataFind as $key => $value) {
                        ...
                        if($value['Reservation']['all_day'] =='1'){
                            ...
                            $reservationData['Reservation'][$key]['start_time']= $start_time;
    ...
    function get_today_sell($testData = null){
        ...
        if(!empty($user_id)){
            ...
            foreach ($customerAnalysisData as $customerAnalysisKey => $customerAnalysisValue) {
                if(isset($customerAnalysisValue['NoteService'][0])){
                ...
                if(isset($customerAnalysisValue['NoteProduct'][0])){
            ...
            $employeeCustomerArray =$employeeArray = $empIdArr = array();
             foreach ($customerAnalysisData as $customerAnalysisKey => $customerAnalysisValue) {
                    ...
                    if(isset($customerAnalysisValue['NoteProduct'])){
                        foreach ($customerAnalysisValue['NoteProduct'] as $productKey => $productValue) {
                            if(($productValue['product_id'] != 0) && isset($productValue['employee_id']) && ($productValue['employee_id']!=0)){    
                                if($productValue['payment_type'] == '現金'){
                                    $productCashPrice = $this->priceChangeInt($productValue['sale_price']);
                                    $productCashTotalPrice = ($productCashTotalPrice + $productCashPrice);
                                } 
...
/*
    function list_messages($userId) {
       ...
       // Refresh the token if it's expired.
        ...
        $opt_param = array();
        $opt_param['maxResults'] = array('500');
            ...
            if(!isset($reservationData['Sbreservation']['reservation_number']) && empty($reservationData['Sbreservation']['reservation_number'])){
                if(!empty($name)){
                    ...
                    if(isset($SbcustomerData['Sbcustomer']['id']) && !empty($SbcustomerData['Sbcustomer']['id'])){
                        ...
                        $customerData[$j]['Sbcustomer']['service_id'] = '1';
                ...
                $reservation[$i]['Sbreservation']['Sbcustomer_id'] = $cusromer_id;
       ...
       // Refresh the token if it's expired.
        ...
        foreach ($messages as $message) {
            ...
            if(empty($reservation_time)){
                $reservation_start_date_time =  $reservation_date.' 00:00';
                $reservation_end_date_time =  $reservation_date.' ';
                ...
                if($duration >0){
                    ...
                    $time = new DateTime($reservation_start_date_time);
                    $time->add(new DateInterval('PT' . $minutes_to_add . 'M'));

                    $reservation_end_date_time =  $time->format('Y-m-d H:i');

                    $reservation_start_time = $reservation_time;
                    $endTime = strtotime("+".$minutes_to_add." minutes", strtotime($reservation_time));
                    $reservation_end_time = date('h:i:s', $endTime);
                }else{
                    $reservation_end_date_time =  $reservation_date.' '.$reservation_time;
            ...
            if(!isset($reservationData['Reservation']['reservation_number']) && empty($reservationData['Reservation']['reservation_number'])){
               ...
               // echo $cusromer_id;die;
                ...
                $reservation[$i]['Reservation']['start_time'] = $reservation_start_time;
