...
*
*****************************************************************************/
class WebServicesv41Controller extends AppController{
   ...
   /************************************************************************************************************************************
    ...
    function login($test_data = null){
        ...
        if(!empty($email) && !empty($password)){
            ...
            if(isset($data['User']['id']) && !empty($data['User']['id'])){
                $user_id = $this->User->id = $data['User']['id'];
                ...
                $serviceCountData = $this->Service->find('count',array('conditions'=>array('Service.user_id'=>$user_id)));
                if($serviceCountData ==1){
                    $serviceData = $this->Service->find('first',array('conditions'=>array('Service.user_id'=>$user_id)));
                ...
                if($subscription_current_period_end <= $current_date){
                    $responseArr = array('user_id' => $data['User']['id'], 'service_id' => $service_id,  'service_name' => $service_name, 'stripe_payment_status' => $data['User']['stripe_payment_status'], 'msg1' => 'Login Sccuessfully.', 'msg' => 'ログインが成功しました。', 'employee_id'=>$employee_id, 'employee_name'=>$employee_name,  'registration_step'=>$registration_step,  'employee_image'=>$employee_image, 'employee_pin_number'=>$employee_pin_number,  'customer_pin_number'=>$customer_pin_number, 'is_admin' => strval($data['User']['is_admin']), 'status' => 'success' );

                }else{
                     $responseArr = array('user_id' => $data['User']['id'], 'service_id' => $service_id,  'service_name' => $service_name,  'stripe_payment_status' => '0', 'msg1' => 'Login Sccuessfully.', 'msg' => 'ログインが成功しました。', 'employee_id'=>$employee_id,  'employee_name'=>$employee_name,  'registration_step'=>$registration_step,  'employee_image'=>$employee_image, 'employee_pin_number'=>$employee_pin_number,  'customer_pin_number'=>$customer_pin_number, 'is_admin' => strval($data['User']['is_admin']), 'is_form' => '2', 'status' => 'success' );
                }
    ...
    function subscription_status($test_data = null){
        ...
        $device_token = isset($decoded['device_token']) ? $decoded['device_token'] : '';
        $device_type = isset($decoded['device_type']) ? $decoded['device_type'] : '';
        $notificationCountData = $this->NotificationCount->find('first',array('conditions'=>array('NotificationCount.employee_id'=>$employee_id)));
        ...
        $employeeCode = $this->Employee->find('first', array('conditions'=>array('Employee.id'=>$employee_id),'fields'=>array('Employee.emp_code','Employee.user_id','Employee.name','Employee.image','Employee.role_title','Employee.role_id','Employee.id','Employee.status')));
        if(isset($employeeCode['Employee']['status']) && !empty($employeeCode['Employee']['status']) ){
            $employee_status = $employeeCode['Employee']['status'];
        }elseif(empty($employee_id)){
            $employee_status = '1';
        }else{
            $employee_status = '0';
        }
        $badgeCountData = $this->BadgeCount->find('first',array('conditions'=>array('BadgeCount.employee_id'=>$employee_id)));
        if(isset($badgeCountData['BadgeCount']['count']) && !empty($badgeCountData['BadgeCount']['count'])){
            ...
            $this->BadgeCount->saveField('count' , 0 );
        }

        if(!empty($employee_id) && !empty($device_token) && !empty($device_type)){
        ...
        if(!empty($user_id)){
            ...
            if(isset($data['User']['stripe_payment_status']) && ($data['User']['stripe_payment_status'] == '1')){
                $subscription_current_period_end = isset($data['User']['subscription_current_period_end']) ? date('Y-m-d', strtotime($data['User']['subscription_current_period_end'])) : '';
                echo $current_date = date('Y-m-d');
                echo $subscription_current_period_end;die;
               
                if($current_date>$subscription_current_period_end){
                    $responseArr = array('notification_count' => $notification_count, 'service_name' => $service_name, 'service_id' => $service_id, 'subscription_status' => '0', 'employee_status' => $employee_status, 'status' => 'success' , 'msg' => 'Subscription plan have been expired.' );
                    $jsonEncode = json_encode($responseArr);
                }else{
                    $responseArr = array('notification_count' => $notification_count, 'service_name' => $service_name, 'service_id' => $service_id, 'subscription_status' => '1', 'employee_status' => $employee_status, 'status' => 'success' , 'msg' => '予定企画期間が完了しました' );
                    $jsonEncode = json_encode($responseArr);
                }
            }else{
                if($employee_status ==1){
                      $responseArr = array('notification_count' => $notification_count, 'service_name' => $service_name, 'service_id' => $service_id, 'subscription_status' => '1', 'employee_status' => $employee_status, 'status' => 'success'  , 'msg' => '従業員を登録しました');
                }else{
                    $responseArr = array('notification_count' => $notification_count, 'service_name' => $service_name, 'service_id' => $service_id, 'subscription_status' => '1', 'employee_status' => $employee_status, 'status' => 'success'  , 'msg' => '要求したことが無効しました');
                }
            ...
            $responseArr = array('notification_count' => $notification_count, 'subscription_status' => '1', 'employee_status' => $employee_status, 'status' => 'success' );
            $jsonEncode = json_encode($responseArr);
        }   
        $log = $this->User->getDataSource()->getLog(false, false);
        $recordData['RecordData']['name'] = "subscription_status";
        $recordData['RecordData']['query'] = json_encode($log);
        $this->RecordData->saveAll($recordData);
        echo  $jsonEncode;exit();
    ...
    function signup($test_data = null){
        ...
        /*For hotmail */
        $school_domains = array('hotmail.com');

        if (in_array($this->get_domain($email), $school_domains))
        {
             $responseArr = array('msg' => "あなたはHotmailのEメールを使っています。セキュリティ上の理由から私たちはこちらのシステムでHotmailsを使用できません。別のメールアドレスを選択してください。(Gmailをお勧めします)。", 'msg1' => "This email on hotmail.com.please use 'gmail.com' email for account scurity prepose.",  'status' => 'error' );
            $jsonEncode = json_encode($responseArr);
            return $jsonEncode;
        }
        ...
        $user['User']['device_type'] = isset($decoded['device_type']) ? strtolower($decoded['device_type']) : '';
        $password = isset($decoded['password']) ? $decoded['password'] : '';
        ...
        if($this->User->saveAll($user)){
            ...
            $activation_url = 'http://api.jtsboard.com/users/email_confirm/'.$user_id.'/'.$activationCode;
            $activation_link    =   '<a href="'.$activation_url.'" style="background-color: #debd4e; padding: 15px 30px; display: inline-block; color: #fff; text-decoration: none; border-radius: 5px;">今すぐ登録する</a>';
            $mail_message = str_replace(array('{NAME}', '{EMAIL}','{ACTIVATION_LINK}', '{activation_code}',  '{SITE}'), array($user['User']['salon_name'], $email,$activation_link, $activationCode, Configure::read('App.SITENAME')), $registrationMail['Template']['content']);
            $template = 'default';
     ...
     */
    function email_confirm($act_id) {
        $this->loadModel('User');
        $user = $this->User->find('first', array('conditions' => array('User.verification_code' => $act_id)));
        
        if (!empty($user) && count($user)) {
            $user_id = $user["User"]["id"];
            $this->User->updateAll(array('status' => Configure::read('App.Status.active')), array('User.id' => $user_id));
        ...
        $this->redirect('jtsboard://?user_id='.$user_id);
    }
     ...
     *
     *************************************************************************************************************************************/
     function signup_step2($test_data = null){
        ...
        $user_id = $user['User']['id'] = isset($decoded['user_id']) ? $decoded['user_id'] : '';
        ...
        $user['User']['sales_code'] = isset($decoded['sales_code']) ? $decoded['sales_code'] : '';
        $user['User']['status'] =1;
        $user['User']['stripe_payment_status'] =1;
        $user['User']['subscription_current_period_end'] = date('d-m-Y', strtotime('+1 months'));
        $user['User']['stripe_plan_status'] =1;

        
        $service = isset($decoded['service']) ? $decoded['service'] : '';
        // $other_services = isset($decoded['other_services']) ? $decoded['other_services'] : '';
        // pr($user);die;
        if($this->User->saveAll($user)){
            $i=0;
            $userSubCategoryData = $userCategoryData = array();
            if(!empty($user_id)){

                $data = $this->Category->find('all',array('conditions'=>array( 'Category.status'=>Configure::read('App.Status.active'), 'Category.parent_id'=>Configure::read('App.Status.inactive')),'order' => array('Category.id' => 'DESC')));
                if(!empty($data)){
                    foreach ($data as $key => $value) {
                        ...
                        $subCategoryData = $this->Category->find('all',array('conditions'=>array( 'Category.status'=>Configure::read('App.Status.active'), 'Category.parent_id'=>$category_id),'order' => array('Category.id' => 'DESC')));
                        if(!empty($subCategoryData)){
                            $userSubCategoryData =array();
                            foreach ($subCategoryData as $subKey => $subValue) {
                                $userSubCategoryData[$i]['UserCategory']['user_id'] = $user_id;
                                $userSubCategoryData[$i]['UserCategory']['parent_id'] = $user_category_id;
                                $userSubCategoryData[$i]['UserCategory']['name'] = $subValue['Category']['name'];
                                $userSubCategoryData[$i]['UserCategory']['japanese_name'] = $subValue['Category']['japanese_name'];
                                $userSubCategoryData[$i]['UserCategory']['image'] = $subValue['Category']['image'];
                                $userSubCategoryData[$i]['UserCategory']['status'] = $subValue['Category']['status'];
                                $i++;
                            } 
                ...
                if(!empty($service)){
                    ...
                    $this->loadModel('MainService');
                     /* Load Model End*/
                    $mainServiceData =$this->MainService->find('all',array('conditions'=>array( 'MainService.id'=>$service)));
                   
                    foreach ($mainServiceData as $serviceKey => $serviceValue) {

                        $ExistserviceData = $this->Service->find('first',array('conditions'=> array( 'Service.user_id'=>$user_id, 'Service.name'=>$serviceValue['MainService']['name'])));
                        // pr($ExistserviceData);die;
                        if(empty($ExistserviceData)){
                            $colorData = array();
                            $service['Service']['user_id'] = $user_id;
                            $color_id = $service['Service']['color_id'] = (isset($serviceValue['MainService']['color_id']) && !empty($serviceValue['MainService']['color_id'])) ? $serviceValue['MainService']['color_id'] : '';
                            $colorData = $this->Color->find('first',array('conditions'=> array( 'Color.id'=>$color_id)));
                            $service_name = $service['Service']['name'] = (isset($serviceValue['MainService']['name']) && !empty($serviceValue['MainService']['name'])) ? $serviceValue['MainService']['name'] : '';
                            $service['Service']['color_code'] = isset($serviceValue['MainService']['color_code']) ? $serviceValue['MainService']['color_code'] : '';
                            $service['Service']['status'] =Configure::read('App.Status.active');
                            if($this->Service->saveAll($service)){
                                $service_id = $this->Service->id;
                                $serviceColor['ServiceColor']['user_id'] = $user_id;
                                $serviceColor['ServiceColor']['service_id'] = $service_id;
                                $serviceColor['ServiceColor']['color_id'] = $color_id;
                                ...
                                $serviceColor['ServiceColor']['service_name'] = $service_name;
                                $this->ServiceColor->saveAll($serviceColor);
                            }
                        }
            ...
            $responseArr = array('user_id' => $user_id, 'service_name' => $service_name, 'service_id' => $service_id, 'status' => 'success' );
            $jsonEncode = json_encode($responseArr);
            ...
            if(!$data){
                ...
                if(!empty($data)){
                    ...
                    foreach ($data as $key => $value) {
                        ...
                        $customerData['Customer'][$i]['modified'] = date( 'Y年m月d日 H時i分', strtotime( $value['Customer']['modified']));
 ...
 /**************************************************************************
            ...
            if($sort_by == '1' || $sort_by == '2' || $sort_by == '3' || $sort_by == '4'){ 
                ...
                if($sort_by == '1' || $sort_by == '2'){
                    foreach ($data as $customerKey => $customerValue) {
                        ...
                        $date = date( 'Y年m月d日 H時i分', strtotime( $customerValue['Customer']['modified']));
                ...
                if(!empty($data)){
                    ...
                    foreach ($data as $key => $value){ 
                        ...
                        $date = date( 'Y年m月d日 H時i分', strtotime( $value['Customer']['modified']));
   ...
   /**************************************************************************
     ...
     *********************************************************************/
      ...
      function customer_samename_search($testData = null){
        ...
        if(!empty($user_id) ){
            ...
            $conditions["Customer.user_id"] = $user_id;
            $conditions["Customer.status"] = Configure::read('App.Status.active');
            $allCustomerData = $this->Customer->find('all',array('conditions' => $conditions, 'order' => array('Customer.modified' => 'DESC') ));

            if(!empty($allCustomerData)){
                $customerNameIdArr =  $customerNameDataArr = array(); 
                foreach ($allCustomerData as $key => $value) {
                   /* $id_key = '';
                    if(in_array($value['Customer']['first_name'], $customerNameDataArr)){
                        $id_key = array_search($value['Customer']['first_name'], $customerNameDataArr);
                        $id_key = str_replace('f_', '', $id_key);
                        $id_key = str_replace('l_', '', $id_key);
                        array_push($customerNameIdArr,$id_key);
                        array_push($customerNameIdArr,$value['Customer']['id']);
                    }
                    if(in_array($value['Customer']['last_name'], $customerNameDataArr)){
                        $id_key = array_search($value['Customer']['last_name'], $customerNameDataArr);
                        $id_key = str_replace('f_', '', $id_key);
                        $id_key = str_replace('l_', '', $id_key);
                        array_push($customerNameIdArr,$id_key);
                        array_push($customerNameIdArr,$value['Customer']['id']);
                    }*/
                    $customerNameDataArr[] = $value['Customer']['first_name'];
                    $customerNameDataArr[] = $value['Customer']['last_name'];

                }

                // $customerNaDataArr = array_unique($customerNameDataArr);
               // $customerAssocArr = array_diff_assoc($customerNameDataArr, array_unique($customerNameDataArr));
               $customerAssocArr = array_filter(array_unique( array_diff_assoc( $customerNameDataArr, array_unique( $customerNameDataArr ) ) ) );
                // pr($customerAssocArr);die;
               //  pr($customerAssocArr);die;
                $conditions = array (
                            'OR' => array(
                                'Customer.first_name' => $customerAssocArr,
                                'Customer.last_name' => $customerAssocArr
                            ),
                            'Customer.status' => Configure::read('App.Status.active')
                        );
                $customerRecords = $this->Customer->find('all', array('conditions' => $conditions, 'order' => array('Customer.first_name' => 'ASC', 'Customer.last_name' => 'ASC')));
               
                foreach ($customerRecords as $key => $value) {
                    $customerData['Customer'][$i]['id'] = $value['Customer']['id'];
                    $customerData['Customer'][$i]['name'] = $value['Customer']['last_name']." ".$value['Customer']['first_name'];
                    $customerData['Customer'][$i]['first_name'] = $value['Customer']['first_name'];
                    $customerData['Customer'][$i]['last_name'] = $value['Customer']['last_name'];
                       // $customerData['Customer'][$i] = $value['Customer'];
                       // $customerData['Customer'][$i]['salon_name'] = $value['Salon']['salon_name'];
                     //   $customerData['Customer'][$i]['service_name'] = $this->get_service_name($value['Customer']['service_id']);
                        $i++;
        ...
        $log = $this->Customer->getDataSource()->getLog(false, false);
        $recordData['RecordData']['name'] = "customer_samename_search";
        $recordData['RecordData']['query'] = json_encode($log);
        $this->RecordData->saveAll($recordData);
        echo  $jsonEncode;exit();
    } 
...
/**************************************************************************
    ...
    function form_service_detail($testData = null){
        ...
        if(!empty($customer_id) &&  !empty($created_date) && !empty($service_id) ){
            ...
            $customerData[$i]['Service']['msg'] = 'お客様は存在しません。';
            $customerData[$i]['Service']['msg1'] = 'Please add customer id.';
            $customerData[$i]['Service']['status'] = 'error';
            $jsonEncode = json_encode($customerData);
        }
       ...
       /* echo $activation_url = Router::url(array(
        ...
        if(!empty($userDetail)){
            ...
            if($this->User->saveField('verification_code' , $verification_code ))
            {
                ...
                if($this->sendMail($to, $subject, $mail_message, $from, $template)){
     ...
     *********************************************************************/
     ...
     function service_list(){
        ...
        $data = $this->Service->find('all', array('conditions' => array('Service.status' =>Configure::read('App.Status.active'), 'Service.user_id' =>$user_id)));
        $serviceArray =array();
        $i =0;
        foreach ($data  as $key => $value) {
...
/**************************************************************************
     ...
     *********************************************************************/
     ...
     function main_service_list(){
        $this->loadModel("MainService");

        $data =$this->MainService->find('all', array('conditions' => array('MainService.status' =>Configure::read('App.Status.active'))));
        $serviceArray['MainService'] =array();
        $i =0;
        foreach ($data  as $key => $value) {
            $serviceArray['MainService'][$i]['id'] = $value['MainService']['id'];
            $serviceArray['MainService'][$i]['name'] = $value['MainService']['name'];
            $serviceArray['MainService'][$i]['color_id'] = $value['MainService']['color_id'];
            $serviceArray['MainService'][$i]['color_name'] = $value['MainService']['color_name'];
            $serviceArray['MainService'][$i]['color_code'] = $value['MainService']['color_code'];
            $serviceArray['MainService'][$i]['status'] = $value['MainService']['status'];
            $serviceArray['MainService'][$i]['created'] = $value['MainService']['created'];
            $i++;
        }
        $jsonEncode = json_encode($serviceArray);
        echo  $jsonEncode;exit();
    ...
    function salon_service_list(){
        ...
        $this->loadModel("ServiceColor");
        $user_id = isset($decoded['user_id']) ? $decoded['user_id'] : '';
        $selectedServices = $responseArr = $serviceColorData['ServiceColor'] = array();
        $i =0;
        if(!empty($user_id)){
            ...
            if(isset($allServiceColorData[0]['ServiceColor']) && !empty($allServiceColorData[0]['ServiceColor'])){
                foreach ($allServiceColorData as $key => $value) {
                   $serviceColorData['ServiceColor'][$i]['id'] = $value['Service']['id'];
                   $serviceColorData['ServiceColor'][$i]['service_id'] = $value['ServiceColor']['service_id'];
                   $service_name = $serviceColorData['ServiceColor'][$i]['service_name'] = $value['Service']['name'];
                   ...
                   $serviceColorData['ServiceColor'][$i]['is_selected'] = (string)$value['Service']['status'];
                   array_push($selectedServices, $service_name);
                   $i++;
                }
        ...
        $mainServiceData =$this->MainService->find('all', array('conditions' => array('MainService.status' =>Configure::read('App.Status.active'))));
        foreach ($mainServiceData  as $mainKey => $mainValue) {
            if(!in_array($mainValue['MainService']['name'], $selectedServices)){
                $serviceColorData['ServiceColor'][$i]['id'] = $mainValue['MainService']['id'];
                $serviceColorData['ServiceColor'][$i]['service_id'] = $mainValue['MainService']['id'];
                $serviceColorData['ServiceColor'][$i]['service_name'] = $mainValue['MainService']['name'];
                $serviceColorData['ServiceColor'][$i]['color_code'] = $mainValue['MainService']['color_code'];
                $serviceColorData['ServiceColor'][$i]['color_id'] = $mainValue['MainService']['color_id'];
                $serviceColorData['ServiceColor'][$i]['color_name'] = $mainValue['MainService']['color_name'];
                $serviceColorData['ServiceColor'][$i]['is_selected'] = "0";
                $i++;
            }   
        }
...
/**************************************************************************
     ...
     *********************************************************************/
     ...
     function service_color_list(){
       ...
       $serviceColorData['ServiceColor'] = array();
        if(!empty($user_id)){
            $this->ServiceColor->bindModel(array('belongsTo' => array('Service','Color')));
...
/**************************************************************************
    ...
    function get_employee_id($name = '', $user_id=''){
        $this->loadModel("Employee");
    ...
    function add_note($test_data = null){
        ...
        if(!empty($note_image_id)){
            ...
            foreach ($employeeData as $empKey => $empValue) {
                $empName = $this->get_employee_name($employee_id);
                $noteNotificationMessage = $empName.' がコメントを追加しました。';
                $empy_id = isset($empValue['Employee']['id']) ? $empValue['Employee']['id'] : '';
                ...
                if(!empty($empDevicetoken) && ($empy_id  != $employee_id) && !in_array($empDevicetoken, $deviceTokenArr) ){
                    if($emp_device_type == 'Android'){
                        $this->androidPushNotification($empDevicetoken, $noteNotificationMessage, $user_id, $empValue['Employee']['id'],$customer_id, $customerAnalysisData['CustomerHistory']['id'],  $emp_device_type, 'employee', 'add_note', Configure::read('App.Firebase.apikey'));
           ...
           // print_r($customerAnalysisData);die;
            ...
            if(isset($customerAnalysisData['NoteService'])){
                foreach ($customerAnalysisData['NoteService'] as $serviceKey => $serviceValue) {
                   $note_service_id = $serviceValue['id'];
                   $count = $this->ServiceDetail->find('count', array('conditions' => array('ServiceDetail.user_id' => $user_id, 'ServiceDetail.note_service_id' => $note_service_id )));
                   if($count > 0){
                        $customerAnalysisData['NoteService'][$serviceKey]['is_form'] = '1';
                   }else{
                        $customerAnalysisData['NoteService'][$serviceKey]['is_form'] = '0';
                   }
                   $servicePrice = $this->priceChangeInt($serviceValue['service_price']); 
                   $serviceTotalPrice = ($serviceTotalPrice + $servicePrice);
        ...
        if(!empty($user_id) && !empty($customer_id)){
           ...
           // echo "<pre>";
            ...
            if(isset($noteTicketData[0]) && !empty($noteTicketData[0])){
                foreach ($noteTicketData as $noteTicketKey => $noteTicketValue) {
                    if(!empty($service_id)){
                        $noteTicketList['TicketList'][$i]['id'] = $noteTicketValue['NoteTicket']['id'];
                        $noteTicketList['TicketList'][$i]['name'] = $noteTicketValue['NoteTicket']['ticket_name'];
                        $i++;
...
/****************************************************************************************************************************************
    ...
    function upload_note_image(){
         if(!empty($_FILES)){
                ...
                // $res3 = $this->Upload->upload($_FILES['note_image'], WWW_ROOT . NOTE_IMG_VERTICAL_DIR, $newName, '', array('png', 'jpg', 'jpeg', 'gif'));
                // $res3 = $this->Upload->upload($_FILES['note_image'], WWW_ROOT . NOTE_IMG_MEDIUM_DIR, $newName, '', array('png', 'jpg', 'jpeg', 'gif'));
                // $res3 = $this->Upload->upload($_FILES['note_image'], WWW_ROOT . NOTE_IMG_THUMB_DIR, $newName, '', array('png', 'jpg', 'jpeg', 'gif'));
...
/****************************************************************************************************************************************
     ...
     *************************************************************************************************************************************/
     ...
     function edit_note_service($test_data = null){
        ...
        $staff_id = $noteService['NoteService']['staff_id'] = isset($decoded['staff_id']) ? $decoded['staff_id'] : '';
        
        $noteService['NoteService']['service_id'] = isset($decoded['service_id']) ? $decoded['service_id'] : '';
        $ticket_id = $noteService['NoteService']['ticket_id'] = isset($decoded['ticket_id']) ? $decoded['ticket_id'] : '0';
        $noteService['NoteService']['service_name'] = isset($decoded['service_name']) ? $decoded['service_name'] : '';
       
        $employee_id = $noteService['NoteService']['employee_id'] = isset($decoded['employee_id']) ? $decoded['employee_id'] : '';
        $noteService['NoteService']['employee_name'] = isset($decoded['employee_name']) ? $decoded['employee_name'] : '';
        $service_price = $noteService['NoteService']['service_price'] = isset($decoded['service_price']) ? $decoded['service_price'] : '';
        $noteService['NoteService']['payment_type'] = isset($decoded['payment_type']) ? $decoded['payment_type'] : '';
        $noteService['NoteService']['status'] = Configure::read('App.Status.active');
        ...
        if($this->NoteService->saveAll($noteService)){
            ...
            $deviceTokenArr =array();
            foreach ($employeeData as $empKey => $empValue) {
                $empName = $this->get_employee_name($staff_id);
                $emp_id = isset($empValue['Employee']['id']) ? $empValue['Employee']['id'] : '';
                $noteNotificationMessage = $empName.' がコメントを編集しました。';
                $empDevicetoken = isset($empValue['Employee']['device_token']) ? $empValue['Employee']['device_token'] : '';
                $emp_device_type = isset($empValue['Employee']['device_type']) ? $empValue['Employee']['device_type'] : '';

                if(!empty($empDevicetoken) && ($emp_id  != $employee_id) && !in_array($empDevicetoken, $deviceTokenArr) ){
                    if($emp_device_type == 'Android'){
                        $this->androidPushNotification($empDevicetoken, $noteNotificationMessage, $user_id, $empValue['Employee']['id'],$customer_id, $customer_history_id,  $emp_device_type, 'employee', 'add_note', Configure::read('App.Firebase.apikey'));
                        array_push($deviceTokenArr, $empDevicetoken);
                    }else{
...
/****************************************************************************************************************************************
     ...
     *************************************************************************************************************************************/
     ...
     function edit_note_product($test_data = null){
        ...
        $ticket_id = $noteProduct['NoteProduct']['ticket_id'] = isset($decoded['ticket_id']) ? $decoded['ticket_id'] : '0';
        $noteProduct['NoteProduct']['product_name'] = isset($decoded['product_name']) ? $decoded['product_name'] : '';
        $employee_id = $noteProduct['NoteProduct']['employee_id'] = isset($decoded['employee_id']) ? $decoded['employee_id'] : '';
        $noteProduct['NoteProduct']['employee_name'] = isset($decoded['employee_name']) ? $decoded['employee_name'] : '';
        $sale_price = $noteProduct['NoteProduct']['sale_price'] = isset($decoded['sale_price']) ? $decoded['sale_price'] : '';
        $product_quantity = $noteProduct['NoteProduct']['product_quantity'] = isset($decoded['product_stock']) ? $decoded['product_stock'] : '';
        $noteProduct['NoteProduct']['payment_type'] = isset($decoded['payment_type']) ? $decoded['payment_type'] : '';
       ...
       // print_r($noteProduct);die;
        if($this->NoteProduct->saveAll($noteProduct)){
            ...
            foreach ($employeeData as $empKey => $empValue) {
                $empName = $this->get_employee_name($staff_id);
                $emp_id = isset($empValue['Employee']['id']) ? $empValue['Employee']['id'] : '';
                $noteNotificationMessage = $empName.' が商品の内容を更新しました。';
                $empDevicetoken = isset($empValue['Employee']['device_token']) ? $empValue['Employee']['device_token'] : '';
                $emp_device_type = isset($empValue['Employee']['device_type']) ? $empValue['Employee']['device_type'] : '';
                if(!empty($empDevicetoken) && ($emp_id  != $employee_id) && !in_array($empDevicetoken, $deviceTokenArr) ){
                    if($emp_device_type == 'Android'){
        ...
        if(!empty($id)){
            if($this->NoteProduct->delete($id, true)){
               ...
               $noteOldTicketData=$this->NoteTicket->find('first', array('conditions'=>array('NoteTicket.id'=>$noteProductData['NoteProduct']['ticket_id'])));
                ...
                $responseArr = array('status' => 'success', 'msg' => '注製品が正常に削除されました。', 'msg1' => 'Note 製品が削除しました' );
                $jsonEncode = json_encode($responseArr);
            }else{
            ...
            $responseArr = array('status' => 'error', 'msg' => '注製品は存在しません。', 'msg1' => 'Note 製品がありません'  );
            $jsonEncode = json_encode($responseArr);
        }
        $log = $this->NoteProduct->getDataSource()->getLog(false, false);
     ...
     *************************************************************************************************************************************/
     ...
     function add_note_ticket($test_data = null){
        ...
        if(isset($ticketData['Ticket']['service_id']) && !empty($ticketData['Ticket']['service_id'])){
            ...
            $ticket_amount = $noteTicket['NoteTicket']['ticket_num_time'] = $ticketData['Ticket']['ticket_num_time'];
        }
     ...
     *************************************************************************************************************************************/
     ...
     function edit_note_ticket($test_data = null){
        ...
        $ticket_id = $noteTicket['NoteTicket']['ticket_id'] = isset($decoded['ticket_id']) ? $decoded['ticket_id'] : '0';
        $noteTicket['NoteTicket']['ticket_name'] = isset($decoded['ticket_name']) ? $decoded['ticket_name'] : '';
        $employee_id = $noteTicket['NoteTicket']['employee_id'] = isset($decoded['employee_id']) ? $decoded['employee_id'] : '';
        $noteTicket['NoteTicket']['employee_name'] = isset($decoded['employee_name']) ? $decoded['employee_name'] : '';
        $noteTicket['NoteTicket']['ticket_price'] = isset($decoded['ticket_price']) ? $decoded['ticket_price'] : '';
        ...
        if($this->NoteTicket->saveAll($noteTicket)){
           ...
           /* $employeeData = $this->Employee->find('all', array('conditions'=>array('Employee.user_id'=>$user_id)));
            ...
            foreach ($employeeData as $empKey => $empValue) {
                $empName = $this->get_employee_name($staff_id);
                $emp_id = isset($empValue['Employee']['id']) ? $empValue['Employee']['id'] : '';
                $noteNotificationMessage =$empName. ' がチケットの内容を編集しました。';
                $empDevicetoken = isset($empValue['Employee']['device_token']) ? $empValue['Employee']['device_token'] : '';
                $emp_device_type = isset($empValue['Employee']['device_type']) ? $empValue['Employee']['device_type'] : '';
                if(!empty($empDevicetoken) && ($emp_id  != $employee_id) && !in_array($empDevicetoken, $deviceTokenArr) ){
    ...
    function delete_note_image($test_data = null){
        ...
        if(!empty($id)){
            ...
            if($this->NoteImage->saveAll($noteImages)){
                $responseArr = array('status' => 'success', 'msg' => 'ノートイメージを削除しました' );
                $jsonEncode = json_encode($responseArr);
            }else{
            ...
            $responseArr = array('status' => 'error', 'msg' => 'ノートイメージを保存しませんでした'  );
            $jsonEncode = json_encode($responseArr);
        }
        $log = $this->NoteImage->getDataSource()->getLog(false, false);
     ...
     *********************************************************************/
     ...
     function add_employee($test_data =null){
        ...
        if(isset($employeeEmailExist['Employee']['id']) && !empty($employeeEmailExist['Employee']['id'])){
            $responseArr = array('status' => 'error', 'msg' => 'このメールが他の従業員を登録してした' , 'msg1' => 'This email is already exist.'  );
            $jsonEncode = json_encode($responseArr);
        }else{
            // echo 'test'.$decoded['service_id'].'tesws';die;
            $responseArr =array();
            $employee['Employee']['user_id'] = isset($decoded['user_id']) ? $decoded['user_id'] : '';
            $emp_name = isset($decoded['name']) ? $decoded['name'] : '';
            ...
            if($employee['Employee']['is_technician'] == '0'){
                $employee['Employee']['service_id'] = '0';
            }
            // pr($employee);die;
...
/*
    ...
    function delete_employee($test_data = null){
        ...
        if(!empty($id)){
            if($this->Employee->delete($id, true)){
                $responseArr = array('status' => 'success', 'msg' => '従業員が削除しました' );
                $jsonEncode = json_encode($responseArr);
            }else{
            ...
            $responseArr = array('status' => 'error', 'msg' => '従業員が存在していませんでした'  );
            $jsonEncode = json_encode($responseArr);
        }
        $log = $this->Employee->getDataSource()->getLog(false, false);
 ...
 /****************************************************************************************************************************************
    ...
    function deactive_employee($test_data = null){
        ...
        if(!empty($id)){
            if($this->Employee->saveField('status' , Configure::read('App.Status.inactive') )){
                $responseArr = array('status' => 'success', 'msg' => '従業員が非アクティブに成功しました' );
                $jsonEncode = json_encode($responseArr);
            }else{
    ...
    function add_attendance($test_data =null){
        ...
        $recordData['RecordData']['name'] = "checkout";
        $recordData['RecordData']['query'] = json_encode($data);
        $this->RecordData->saveAll($recordData); 
        ...
        $date = isset($decoded['date']) ? $decoded['date'] : '';
        if(!empty($emp_code)){
            $emp_time = date("Y-m-d H:i:s", strtotime($emp_time));
 ...
 /**************************************************************************
                ...
                if(!empty($data)){
                    ...
                    foreach ($data as $key => $value) {
                        ...
                        if($value['Attendance']['checkin_time']=='NULL' || $value['Attendance']['checkin_time']=='0000-00-00 00:00:00' ||  empty($value['Attendance']['checkin_time']) ){
                            ...
                            $dateObject = new DateTime($checkInDateTime);
                            $customerData['Attendance'][$i]['checkin_time'] = $dateObject->format('h:i A');
                        ...
                        if($value['Attendance']['checkout_time']=='NULL' || $value['Attendance']['checkout_time']=='0000-00-00 00:00:00' || empty($value['Attendance']['checkout_time']) ){
                            ...
                            $checkuotTime = $value['Attendance']['checkout_time'];
                            $dateObject = new DateTime($checkuotTime);
                            $customerData['Attendance'][$i]['checkout_time'] = $dateObject->format('h:i A');
                        ...
                        if(!empty($customerData['Attendance'][$i]['checkin_time']) && !empty($customerData['Attendance'][$i]['checkout_time'])){
                            ...
                            $to_time = strtotime($endCheckInTime);
                            $from_time = strtotime($endCheckOutTime);
                            $starting_total_time = round(abs($to_time  - $from_time) / 60,2);
                            $after_end_time = 0;
                             if(($starting_total_time > $salonWorkingTime) && ($salonWorkingTime >0)){
                                $total_over_time = $this->add_over_time($to_time, $from_time, $startTime, $endTime, $over_time);
                                $let_time_time = 0;
                                if($to_time >= $startTime){
                                    $let_time_time = round(abs($startTime  - $to_time) / 60,2);
                                    $after_end_time = floor(($let_time_time + $total_over_time)/$over_time)*$over_time;
                                    // $customerData['Attendance'][$i]['let_time_time'] = $let_time_time;
                                }
                            }else{
                                $total_over_time = 0;
                            }
                            // echo 'total_over_time : '.$total_over_time.'<br />';
                            if($startTime >= $to_time){
                                $to_time =  $startTime;
                            }
                            if(($endTime <= $from_time) && ($endTime!=$startTime)){
                                $from_time =  $endTime;
                            }

                            if($total_over_time >=  $over_time){
                               $over_time_hours   = floor(($total_over_time) / 60);
                                if($over_time_hours > 0){
                                    $over_time_minutes =  floor(($total_over_time - ($over_time_hours * 60)));

                                }else{
                                    $over_time_minutes = $over_time;

                                }
                                $customerData['Attendance'][$i]['count_over_time'] = $over_time_hours." Hours ".$over_time_minutes." Minutes";

                            }else{
                                 $customerData['Attendance'][$i]['count_over_time'] = "0 Minutes";
                            }

                            $total_time = round(abs($to_time  - $from_time) / 60,2);
                            // $customerData['Attendance'][$i]['total_time1'] = $total_time;
                            if(($total_over_time > 0)){
                                $total_time = ($total_time + $total_over_time + $after_end_time);
                                $total_time = floor($total_time/$over_time)*$over_time;
                            }else{
                                 $total_time = ($total_time +  $after_end_time);
                                 if(($starting_total_time >= $salonWorkingTime)){
                                    $overT = ($starting_total_time - $salonWorkingTime);
                                    if($overT < $over_time){
                                        $total_time =$salonWorkingTime;
                                    }
                                 }
                            ...
                            $customerData['Attendance'][$i]['total_over_time'] = $total_over_time;
                            $customerData['Attendance'][$i]['after_end_time'] = $after_end_time;
    ...
    function add_over_time($to_time, $from_time, $start_time, $end_time, $over_time){
        $total_time = round(abs($from_time  - $to_time) / 60,2);
        $total_salon_time = round(abs($start_time  - $end_time) / 60,2);
        $let_time_time = 0;
        if($total_time > $total_salon_time){

            if($to_time <= $start_time){
                $to_time = $start_time;
            }else{
                $let_time_time = round(abs($start_time  - $to_time) / 60,2);
            }

            if($from_time >= $end_time){
                $staff_over_time = round(abs($from_time  - $end_time) / 60,2);
                if($let_time_time > 0){
                    // echo 'staff_over_time : '.$staff_over_time.'<br/ >';
                     $staff_over_time =  ($staff_over_time - $let_time_time);
                     // echo 'let_time_time : '.$let_time_time.'<br/ >';
                     // echo 'staff_over_time : '.$staff_over_time.'<br/ >';
                }
                $total_over_time =  ((floor($staff_over_time/$over_time)*$over_time));
            }else{
                $total_over_time = 0;
            }
            return $total_over_time;

            // $salon_working_time = round(abs($to_time  - $form_time) / 60,2);

            // $total_time  = ($salon_working_time + $total_over_time);
        ...
        if(!empty($emp_code)){
            ...
            if(isset($employeeCode['Employee']['id']) && !empty($employeeCode['Employee']['id'])){
                ...
                if(isset($attendanceTouchId['Attendance']['emp_code']) && !empty($attendanceTouchId['Attendance']['emp_code'])){
                    ...
                    if(isset($attendanceTouchIdcheckIn['Attendance']['emp_code']) && !empty($attendanceTouchIdcheckIn['Attendance']['emp_code'])){
                        $responseArr = array('status' => 'success',  'msg' => '従業員が退勤',  'attendance_status' => '2', 'emp_code' => $emp_code);
                        $jsonEncode = json_encode($responseArr);
                    
                    }else{
                        $responseArr = array('status' => 'success',  'msg' => '従業員が出勤',  'attendance_status' => '1', 'emp_code' => $emp_code);
                        $jsonEncode = json_encode($responseArr);
                    }
...
/****************************************************************************************************************************************
     ...
     *
     *************************************************************************************************************************************/
    function monthly_attendance_olddd($test_data = null){
        ...
        $monthStartDate = $this->User->find('first', array('conditions'=>array('User.id'=>$user_id),'fields'=>array('User.month_end_datemonth_end_date','User.id','User.weekend')));
        if(isset($monthStartDate['User']['month_end_date']) && !empty($monthStartDate['User']['month_end_date'])){
            $day = $monthStartDate['User']['month_end_date'];
        ...
        */
        ...
        if(!empty($employee_id)){
            ...
            $responseArr = array('status' => 'error',  'msg' => '存在しない従業員を入れてください' );
            $jsonEncode = json_encode($responseArr);
        }
    ...
    function monthly_attendance_olds($test_data = null){
        ...
        $monthStartDate = $this->User->find('first', array('conditions'=>array('User.id'=>$user_id),'fields'=>array('User.month_end_date','User.id','User.weekend')));
        if(isset($monthStartDate['User']['month_end_date']) && !empty($monthStartDate['User']['month_end_date'])){
            $day = $monthStartDate['User']['month_end_date'];
            $weekendDay = $monthStartDate['User']['weekend'];
        }else{
    ...
    function monthly_attendance($test_data = null){
        ...
        if(!empty($employee_id)){
            ...
            if(isset($employeeCode['Employee']['id']) && !empty($employeeCode['Employee']['id'])){
                ...
                $start_time =  isset($getUserData['User']['start_time']) ? $getUserData['User']['start_time'] : 0;
                $end_time =  isset($getUserData['User']['end_time']) ? $getUserData['User']['end_time'] : 0;
                $office_end_time = strtotime($end_time);  
                $startTime = strtotime($start_time);
                $endTime = strtotime($end_time);
                if(!empty($start_time) && !empty($end_time)  && !empty($over_time) )  {
                ...
                $interval_lunch_time_total = $total_time = 0;
                 ...
                 if(!empty($data)){
                    foreach ($data as $key => $value) {
                        ...
                        if($interval_lunch_time == 0){
                         ...
                         if(!empty($customerData['Attendance']['checkin_time']) && !empty($customerData['Attendance']['checkout_time'])){
                                ...
                                $endCheckInTimes = date("H:i",strtotime($endCheckInTime));
                                $endCheckInTimes = strtotime($endCheckInTimes);
                                $endCheckOutTimes = date("H:i",strtotime($endCheckOutTime));
                                $endCheckOutTimes = strtotime($endCheckOutTimes);
                                // echo $endCheckOutTimes.'<br />';
                                // echo $office_end_time.'<br />';
                                $starting_total_time = round(abs($endCheckOutTimes  - $endCheckInTimes) / 60,2);
                            
                                if($starting_total_time > $salonWorkingTime){
                                    $total_over_time = $this->add_over_time($endCheckInTimes, $endCheckOutTimes, $startTime, $endTime, $over_time);
                                }else{
                                    $total_over_time = 0;
                                }
                                // echo 'total_over_time : '.$total_over_time.'<br />';
                                if($startTime >= $endCheckInTimes){
                                    $endCheckInTimes =  $startTime;
                                }
                                if($endTime <= $endCheckOutTimes){
                                    $endCheckOutTimes =  $endTime;
                                }
       
                                if(!empty($interval_lunch_time) && ($start_lunch_time < strtotime($endCheckOutTimes)) && ($end_lunch_time > strtotime($endCheckInTimes))) {
                                    $total_time +=  ($endCheckOutTimes - $endCheckInTimes + $total_over_time);
                                    // $i++;
                                }else{
                                    $total_time += ($endCheckOutTimes - $endCheckInTimes + $total_over_time);
                                }
                                $totalOverTime += $total_over_time; 
                                if(!empty($interval_lunch_time)) {
                                    $i++;
                                }
                    ...
                    // echo $totalOverTime;die;

                    $total_over_time_hours   = floor(($totalOverTime) / 60);

                    if($total_over_time_hours >= 1){
                        $total_over_time_minutes = floor(($totalOverTime - ($total_over_time_hours * 60)));
                        $attendanceRecord['Attendance']['count_over_time'] = $total_over_time_hours." Hours ".$total_over_time_minutes." Minutes";

                    }else{
                        if($totalOverTime  > 0 ){
                            $total_over_time_minutes = floor(($totalOverTime));
                            $attendanceRecord['Attendance']['count_over_time'] = $total_over_time_minutes." Minutes";
                        }else{
                            $attendanceRecord['Attendance']['count_over_time'] = "0 Minutes";
                        }    

                    }

                    // $attendanceRecord['Attendance']['count_over_time'] = (string)($totalOverTime/60).' Minutes' ;
                    //echo $i;die;
                    $interval_lunch_time = $interval_lunch_time*$i*60;
                    if($interval_lunch_time > 0){
                        $interval_lunch_time_hours   = floor(($interval_lunch_time) / 3600);
    ...
    function add_holiday($test_data =null){
        ...
        $user_id = $holiday['Holiday']['user_id'] =isset($decoded['user_id']) ? $decoded['user_id'] : '';
        $holiday['Holiday']['title'] =isset($decoded['title']) ? $decoded['title'] : '';
        $date = $holiday['Holiday']['date'] =isset($decoded['date']) ? $decoded['date'] : '';
        $employeeData = $this->Employee->find('all', array('conditions'=>array('Employee.user_id'=>$user_id), 'fields'=>array('Employee.id')));
        $employee_ids = '0';
        foreach ($employeeData as $empKeys => $empValues) {
            if(empty($employee_ids) || ($employee_ids ==  '0')){
                 $employee_ids .= ','.
                 $employee_ids = $empValues['Employee']['id'];
            }else{
                 $employee_ids .= ','.$empValues['Employee']['id'];
            }
        ...
        if($this->Holiday->saveAll($holiday)){
            if(!empty($id)){
                $conditions = array('Reservation.user_id'=>$user_id,'Reservation.all_day'=>'1','Reservation.reservation_type'=>'3','Reservation.start_date'=>date("Y-m-d H:i:s", strtotime($date)));
            ...
            $reservation_type = $reservation['Reservation']['reservation_type'] = '3';
            $start_time = $reservation['Reservation']['start_time'] = '00:00:00';
            $end_time = $reservation['Reservation']['end_time'] = '00:00:00';
           
            $employee_ids = $reservation['Reservation']['employee_ids'] = $employee_ids;
            $staff_name = $reservation['Reservation']['staff_name'] = '定休日';
            
            
            $reservation['Reservation']['all_day'] = isset($decoded['all_day']) ? $decoded['all_day'] : '1';
            
            $reservation['Reservation']['start_date'] = date("Y-m-d H:i:s", strtotime($date));
            $date = date("Y-m-d", strtotime($date));
            $reservation['Reservation']['end_date'] = date("Y-m-d H:i:s", strtotime($date));
            $reservation['Reservation']['extra_start_date'] = date("Y-m-d H:i:s", strtotime($date));
            $reservation['Reservation']['extra_end_date'] = date("Y-m-d H:i:s", strtotime($date));
            ...
            $user_id = isset($decoded['user_id']) ? $decoded['user_id'] : '';;
            $responseArr = array('user_id' => $user_id, 'holiday_id' => $holiday_id, 'status' => 'success', 'msg' =>'定休日を追加しました' );
            $jsonEncode = json_encode($responseArr);
...
/****************************************************************************************************************************************
    ...
    function delete_holiday($test_data = null){
        ...
        $this->loadModel('Holiday');
        $id = isset($decoded['id']) ? $decoded['id'] : '';
        $holidayData = $this->Holiday->find('first', array('conditions'=>array('Holiday.id'=>$id)));
        $user_id = isset($holidayData['Holiday']['user_id']) ? $holidayData['Holiday']['user_id'] : '0';
        $date = isset($holidayData['Holiday']['date']) ? $holidayData['Holiday']['date'] : '0';
        if(!empty($id)){
            if($this->Holiday->delete($id, true)){
                $conditions = array('Reservation.user_id'=>$user_id,'Reservation.all_day'=>'1','Reservation.reservation_type'=>'3','Reservation.start_date'=>date("Y-m-d H:i:s", strtotime($date)));
                $this->Reservation->deleteAll($conditions);

                $responseArr = array('status' => 'success', 'msg' => '祝日を削除しました' );
                $jsonEncode = json_encode($responseArr);
            }else{
                $responseArr = array('status' => 'error', 'msg' => '休日削除エラー。'  );
                $jsonEncode = json_encode($responseArr);
            }
        }else{
            $responseArr = array('status' => 'error', 'msg' => '休日は未定義です。'  );
            $jsonEncode = json_encode($responseArr);
        }
        $log = $this->Holiday->getDataSource()->getLog(false, false);
    ...
    function get_user_id($test_data = null){
        ...
        $recordData['RecordData']['query'] = json_encode($decoded);
        $this->RecordData->saveAll($recordData);
        ...
        if(!empty($emp_code)){
           ...
           // print_r($employeeCode );die;
            if(isset($employeeCode['Employee']['user_id']) && !empty($employeeCode['Employee']['user_id']) ){
                $employee_id = $this->Employee->id = $employeeCode['Employee']['id'];
                $this->Employee->saveField('device_token' , $device_token );
                $this->Employee->saveField('device_type' , $device_type );

                $notificationCountData = $this->NotificationCount->find('first',array('conditions'=>array('NotificationCount.employee_id'=>$employee_id)));
                if(isset($notificationCountData['NotificationCount']['count']) && !empty($notificationCountData['NotificationCount']['count'])){
                    $responseArr['Employee']['notification_count'] = $notificationCountData['NotificationCount']['count'];
                }else{
                    $responseArr['Employee']['notification_count'] = '0';
                }
                ...
                $subscription_current_period_end = isset($userData['User']['stripe_payment_status']) ? date('Y-m-d', strtotime($userData['User']['stripe_payment_status'])) : '';
                $badgeCountData = $this->BadgeCount->find('first',array('conditions'=>array('BadgeCount.employee_id'=>$employee_id)));
                if(isset($badgeCountData['BadgeCount']['count']) && !empty($badgeCountData['BadgeCount']['count'])){
                    $this->BadgeCount->id = $badgeCountData['BadgeCount']['id'];
                    $this->BadgeCount->saveField('count' , 0 );
                ...
                if($subscription_current_period_end <= $current_date){
                    ...
                    $responseArr['Employee']['subscription_status'] = '1';
                }else{
                    $responseArr['Employee']['subscription_status'] = '0';
                }    
                ...
                $responseArr['Employee']['msg'] = '従業員は存在しません';
                $responseArr['Employee']['notification_count'] = '0';
            }
            $jsonEncode = json_encode($responseArr);
            ...
            $responseArr['Employee']['status'] = 'error';
            $responseArr['Employee']['msg'] = 'スタッフコードを追加してください。';
            $responseArr['Employee']['notification_count'] = '0';
            $jsonEncode = json_encode($responseArr);
        }
     ...
     *
     *************************************************************************************************************************************/
    function employee_logout($test_data = null){
        ...
        if(!empty($employee_id)){
            $employeeCode = $this->Employee->find('first', array('conditions'=>array('Employee.id'=>$employee_id, 'Employee.status' =>Configure::read('App.Status.active')),'fields'=>array('Employee.id','Employee.emp_code','Employee.user_id')));
            if(isset($employeeCode['Employee']['user_id']) && !empty($employeeCode['Employee']['user_id']) ){
                $this->Employee->id = $employeeCode['Employee']['id'];
                $this->Employee->saveField('device_token' , '');
                
                $responseArr['Employee']['status'] = 'success';
                $responseArr['Employee']['msg'] = 'Employee logout successfully.';
            ...
            $responseArr['Employee']['status'] = 'error';
            $responseArr['Employee']['msg'] = 'Please add employee id.';
            $jsonEncode = json_encode($responseArr);
        }
...
/********************************Reservation Section***********************************************/
 ...
 /**************************************************************************
    ...
    function add_reservation(){
        ...
        $employee_id = isset($decoded['employee_id']) ? strtolower($decoded['employee_id']) : '';
        ...
        if(!empty($reservation_type) && ($reservation_type =='1')){
            ...
            $staff_name = $reservation['Reservation']['event_name'] = isset($decoded['event_name']) ? $decoded['event_name'] : '';
        }elseif(!empty($reservation_type) && ($reservation_type =='3')){
            $staff_name = $reservation['Reservation']['staff_name'] = isset($decoded['staff_name']) ? $decoded['staff_name'] : '';
        }
        $conditions = array('Reservation.status' =>5, 'Reservation.user_id'=>$user_id);
        $this->Reservation->deleteAll($conditions);
        
        // print_r($reservation);die;
        
        $reservation['Reservation']['all_day'] = isset($decoded['all_day']) ? $decoded['all_day'] : '0';
        $start_date = isset($decoded['start_date']) ? $decoded['start_date'] : '';
        ...
        $reservation['Reservation']['start_date'] = date("Y-m-d H:i:s", strtotime($extra_start_date));
        $date = date("Y-m-d", strtotime($extra_start_date));
        $reservation['Reservation']['end_date'] = date("Y-m-d H:i:s", strtotime($extra_end_date));
        ...
        $reservation['Reservation']['note'] = (isset($decoded['note']) && $decoded['note'] !='メモ')? $decoded['note'] : '';
        ...
        // print_r($reservation);die;
         ...
         if(!empty($reservation_type) && ($reservation_type =='1')){
           ...
           $conditions = array (
                                'OR' => array(
                                    'AND' => array(
                                        'Reservation.start_time >' => date('H:i:s', strtotime($start_time)),
                                        'Reservation.start_time <' => date('H:i:s', strtotime($end_time)),
                                        'Reservation.end_time >' => date('H:i:s', strtotime($start_time)),  
                                        'Reservation.end_time >' => date('H:i:s', strtotime($end_time)), 
                                    )
                                    
                                ),
                                'Reservation.user_id' => $user_id,
                                'Reservation.employee_ids' => $employee_ids,
                                'Reservation.start_date' => $date,
                                'Reservation.reservation_type' => '1',
                                'Reservation.status != ' => 5,
                                'Reservation.status != ' => 3,
                                'Reservation.all_day' => '0'
                            );
            $conditions_2 = array (
                                'OR' => array(
                                    'AND' => array(
            ...
            if(isset($decoded['id']) && !empty($decoded['id'])){
                $conditions['Reservation.id !='] = $decoded['id'];
                $conditions_2['Reservation.id !='] = $decoded['id'];
                $conditions_3['Reservation.id !='] = $decoded['id'];
            } 
            $existData = $this->Reservation->find('first',array('conditions'=> $conditions, 'order'=>array('Reservation.id' => 'DESC')));
            $existData_2 = $this->Reservation->find('first',array('conditions'=> $conditions_2, 'order'=>array('Reservation.id' => 'DESC')));
            $existData_3 = $this->Reservation->find('first',array('conditions'=> $conditions_3, 'order'=>array('Reservation.id' => 'DESC')));
            // pr($existData);
            // pr($existData_2);
            // pr($existData_3);die;
            if((isset($existData['Reservation']['id']) && !empty($existData['Reservation']['id']) ) || (isset($existData_2['Reservation']['id']) && !empty($existData_2['Reservation']['id']) ) || (isset($existData_3['Reservation']['id']) && !empty($existData_3['Reservation']['id']) )){
                $responseArr = array('msg' => 'このスタッフはすでにご予約済みです', 'msg1' => 'This Staff already have a reservation.',  'status' => 'error' );
                $jsonEncode = json_encode($responseArr);
                echo  $jsonEncode;
                exit();
            ...
            $conditions1 = array (
                               'OR' => array(
                                ...
                                'Reservation.user_id' => $user_id,
                                'Reservation.start_date' => $date,
                                'Reservation.reservation_type' => '3',
                                'Reservation.status !=' => '5',
                                'Reservation.status !=' => '3',
                                'Reservation.all_day' => '0'
                            );
            if(isset($decoded['id']) && !empty($decoded['id'])){
                $conditions1['Reservation.id !='] = $decoded['id'];
               
            } 
            // $conditions['Reservation.reservation_type'] = '1';
            $existData1 = $this->Reservation->find('first',array('conditions'=> $conditions1, 'order'=>array('Reservation.id' => 'DESC')));
            // pr($conditions1);
            // pr($existData);die;
            $employees = isset($existData1['Reservation']['employee_ids']) ? explode(',', $existData1['Reservation']['employee_ids']) : '';
            $employee_idsss = explode(',', $employee_ids);
            if(!empty($employees)){
                foreach ($employee_idsss as $epKey => $epValue) {
                    if(in_array($epValue, $employees)){
                        $responseArr = array('msg' => 'このスタッフは現在募集していません', 'msg1' => 'This Staff is not available at this time.',  'status' => 'error' );
                        $jsonEncode = json_encode($responseArr);
                        echo  $jsonEncode;
                        exit();
            ...
            $conditions2 = array (
                                ...
                                'Reservation.status != ' => '5',
                                'Reservation.status != ' => '3',
                                'Reservation.all_day' => '1'
                            );
            if(isset($decoded['id']) && !empty($decoded['id'])){
                $conditions2['Reservation.id !='] = $decoded['id'];
               
            } 

            $existData2 = $this->Reservation->find('first',array('conditions'=> $conditions2, 'order'=>array('Reservation.id' => 'DESC')));
            // pr($conditions1);
             // pr($existData2);die;
            $employeess = isset($existData2['Reservation']['employee_ids']) ? explode(',', $existData2['Reservation']['employee_ids']) : '';
            if(!empty($employeess)){
                foreach ($employee_idsss as $epKey => $epValue) {
                    if(in_array($epValue, $employeess)){
                        $responseArr = array('msg' => '休みの日', 'msg1' => 'day off.',  'status' => 'error' );
                        $jsonEncode = json_encode($responseArr);
                        echo  $jsonEncode;
                        exit();
             ...
             /* STOP RESERVATION WHEN STAFF HAVE EVENT */
            ...
            $eventConditions = array (
                                ...
                                'Reservation.start_date <=' => $date,
                                'Reservation.reservation_type' => '2',
                                'Reservation.status !=' => '5',
                                'Reservation.status !=' => '3',
                                'Reservation.all_day' => '0'
                            );
            $eventConditions2 = array (
                                'OR' => array(
                                    'AND' => array(
                                        'Reservation.start_time <=' => date('H:i:s', strtotime($start_time)),
                                        'Reservation.end_time >=' => date('H:i:s', strtotime($end_time)) 
            ...
            if(isset($decoded['id']) && !empty($decoded['id'])){
                $eventConditions['Reservation.id !='] = $decoded['id'];
                $eventConditions2['Reservation.id !='] = $decoded['id'];
                $eventConditions3['Reservation.id !='] = $decoded['id'];
                $eventConditions4['Reservation.id !='] = $decoded['id'];
            }
            $existEventData = $this->Reservation->find('first',array('conditions'=> $eventConditions, 'order'=>array('Reservation.id' => 'DESC')));
            $existEventData1 = $this->Reservation->find('first',array('conditions'=> $eventConditions2, 'order'=>array('Reservation.id' => 'DESC')));
            $existEventData2 = $this->Reservation->find('first',array('conditions'=> $eventConditions3, 'order'=>array('Reservation.id' => 'DESC')));
            $existEventData3 = $this->Reservation->find('first',array('conditions'=> $eventConditions4, 'order'=>array('Reservation.id' => 'DESC')));
            // pr($existEventData);
            // pr($existEventData1);
            // pr($existEventData3);die;
            $eventEmployees = isset($existEventData['Reservation']['employee_ids']) ? explode(',', $existEventData['Reservation']['employee_ids']) : '';
            $eventEmployees1 = isset($existEventData1['Reservation']['employee_ids']) ? explode(',', $existEventData1['Reservation']['employee_ids']) : '';
            $eventEmployees2 = isset($existEventData2['Reservation']['employee_ids']) ? explode(',', $existEventData2['Reservation']['employee_ids']) : '';
            $eventEmployees3 = isset($existEventData3['Reservation']['employee_ids']) ? explode(',', $existEventData3['Reservation']['employee_ids']) : '';
            // pr($eventEmployees);
            // pr($eventEmployees1);
            // pr($eventEmployees2);
            // pr($eventEmployees3);die;
            $employee_idsss = explode(',', $employee_ids);
            if(!empty($eventEmployees) || !empty($eventEmployees1) || !empty($eventEmployees2) || !empty($eventEmployees3)){
                if(!empty($eventEmployees)){
                    $eventemployees = $eventEmployees;
                }elseif(!empty($eventEmployees1)){
                    $eventemployees = $eventEmployees1;
                }elseif(!empty($eventEmployees2)){
                    $eventemployees = $eventEmployees2;
                }elseif(!empty($eventEmployees3)){
                    $eventemployees = $eventEmployees3;
                }


                foreach ($employee_idsss as $eventEmpKey => $eventEmpValue) {

                    if(in_array($eventEmpValue, $eventemployees)){
                        $responseArr = array('msg' => 'スタッフがイベントを開催します。', 'msg1' => 'Staff have a event now.',  'status' => 'error' );
                        $jsonEncode = json_encode($responseArr);
                        echo  $jsonEncode;
                        exit();
        ...
        if(empty($employee_ids)){
            ...
            if($this->Reservation->saveAll($reservation)){
                ...
                // $item='example'; 
                // $command = escapeshellcmd("python /var/www/html/test/test_python.py $item");
                // $output = shell_exec($command);
                // echo $output;

                $time =  ((strtotime($end_time) - strtotime($start_time))/60);

                $duration = date('H:i', mktime(0,$time));    
                $dy = date('w', strtotime($extra_start_date));
                $dys = array("日","月","火","水","木","金","土");
                

                $emps_name = '';
                if(($reservation_type =='2') || ($reservation_type =='3')){
                    $employee_idss = explode(',', $employee_ids);
                    $allEmp = $this->Employee->find('all' ,array('conditions'=>array('Employee.id'=>$employee_idss,'Employee.status'=>1), 'fields' =>array('Employee.name')));
                    foreach ($allEmp as $empsKey => $empsValue) {
                        if(empty($emps_name)){
                            $emps_name = $empsValue['Employee']['name'];
                        }else{
                            $emps_name .= 'と'.$empsValue['Employee']['name'];
                        }
                    }
                }
                ...
                if(!empty($customer_name) && ($reservation_type =='1')){
                    ...
                    $reservationNotificationMessage = $emps_name.' が '.$start_date.' のシフトを更新しました';
                }
                ...
                foreach ($employeeData as $empKey => $empValue) {
                    ...
                    $reservationReadData = $this->ReservationRead->find('first',array('conditions'=>array('ReservationRead.date'=>$date, 'ReservationRead.employee_id'=>$emp_id, 'ReservationRead.user_id'=>$user_id)));
                    // pr($notificationReadData);
                    if(!empty($reservation_type) && ($reservation_type !='3')){
                        if(isset($reservationReadData['ReservationRead']['id']) && !empty($reservationReadData['ReservationRead']['id'])){
                             $this->ReservationRead->id = $reservationReadData['ReservationRead']['id'];
                             $this->ReservationRead->saveField("status","0");
                        }else{
                            ...
                            $reservationStatusReadData['ReservationStatusRead']['employee_id'] =  $emp_id;
    ...
    function reservation_detail($testData = null){
        ...
        if(!empty($id)){
            ...
            $data = $this->Reservation->find('first',array('conditions'=> array('Reservation.id'=>$id, 'Reservation.status !='=>'5')));
            if(!empty($data)){
    ...
    function add_reservation_status(){
        ...
        $recordData['RecordData']['name'] = "add_reservation_status";
        $recordData['RecordData']['query'] = json_encode($data);
        $this->RecordData->saveAll($recordData);
        ...
        if($this->Reservation->saveField('status', $status)){
            $reservationData = $this->Reservation->find('first', array('conditions'=>array('Reservation.id'=>$reservation_id, 'Reservation.status !='=>'5')));
            $user_id = $reservationData['Reservation']['user_id'];
            $customer_id = $reservationData['Reservation']['customer_id'];
            $statusMessage = 'added';
            if($status=='2'){
                $reservationStatusNotificationMessage = '施術をスタートしました';
            }elseif($status=='3'){
                $reservationStatusNotificationMessage = '施術が終了しました';
            }elseif ($status=='4') {
                $reservationStatusNotificationMessage = '予約がキャンセルされました';
            }
            ...
            foreach ($employeeData as $empKey => $empValue) {
                ...
                if(!empty($empDevicetoken) && ($emp_id  != $employee_id) && !in_array($empDevicetoken, $deviceTokenArr) ){
                    ...
                    if($emp_device_type == 'Android'){
                        $this->androidPushNotification($empDevicetoken, $reservationStatusNotificationMessage, $user_id, $empValue['Employee']['id'],$customer_id, $reservation_id,  $emp_device_type, 'employee', 'reservation_status', Configure::read('App.Firebase.apikey'));
                        array_push($deviceTokenArr, $empDevicetoken);
                    }else{
...
/****************************************************************************************************************************************
    ...
    function delete_reservation($test_data = null){
        ...
        if(!empty($id)){
            ...
            if($this->Reservation->saveField('status' , '5' )){
                $responseArr = array('status' => 'success', 'msg' => '要約の削除を成功しました' );
                $jsonEncode = json_encode($responseArr);
            }else{
            ...
            $responseArr = array('status' => 'error', 'msg' => '要約がありません'  );
            $jsonEncode = json_encode($responseArr);
        }
        $log = $this->Reservation->getDataSource()->getLog(false, false);
        ...
        if(!empty($user_id)){
            ...
            if(!$userData){
                ...
                if(!empty($reservationDataFind)){
                    $afternoon =  $morning = $all_day = $staff = $event = 0;
                    foreach ($reservationDataFind as $key => $value) {
                        ...
                        $start_time = strtotime($value['Reservation']['start_time']);
                            ...
                            if($value['Reservation']['reservation_type']== '3'){
                                ...
                                $sectionType =Configure::read('App.Reservation.staff');
                                if(empty($staff_name)){
                                    $staff_name = $value['Reservation']['staff_name'];
                                }
                                $reservationData[$sectionType][$staff]['id']= $value['Reservation']['id'];
                                $reservationData[$sectionType][$staff]['customer_name']= $value['Reservation']['name'];    
                                $reservationData[$sectionType][$staff]['all_day']= $value['Reservation']['all_day'];
                                if($value['Reservation']['all_day'] =='1'){
                                     $reservationData[$sectionType][$staff]['start_date']= $date;
                                }else{
                                ...
                                $reservationData[$sectionType][$staff]['is_event']= $value['Reservation']['is_event'];
                                $reservationData[$sectionType][$staff]['reservation_type']= $value['Reservation']['reservation_type'];
                                    if(!empty($serviceColor['ServiceColor']['color_code'])){
                                        $reservationData[$sectionType][$staff]['service_color']= $serviceColor['ServiceColor']['color_code'];
                                    }
                                    $reservationData[$sectionType][$staff]['customer_id']= $value['Customer']['id'];
                                    
                                    $reservationData[$sectionType][$staff]['customer_name']= $value['Customer']['last_name'].' '.$value['Customer']['first_name'];
                                    $reservationData[$sectionType][$staff]['tel']= isset($value['Customer']['tel']) ? $value['Customer']['tel'] : '';
                                    $reservationData[$sectionType][$staff]['customer_status']= $value['Customer']['status'];
                                    
                                    if(!empty($allEmp)){
                                        foreach ($allEmp as $empKey => $empValue) {
                                            $holiday_emp_id = $empValue['Employee']['id'];
                                            if(!in_array($holiday_emp_id, $empIds)){
                                                array_push($empIds,$holiday_emp_id);
                                            

                                                $reservationData[$sectionType][$staff]['Employee'][$ij]['id']= $empValue['Employee']['id'];
                                                $reservationData[$sectionType][$staff]['Employee'][$ij]['name']= $empValue['Employee']['name'];
                                                $reservationData[$sectionType][$staff]['Employee'][$ij]['emp_code']= $empValue['Employee']['emp_code'];
                                                $reservationData[$sectionType][$staff]['Employee'][$ij]['image']= $empValue['Employee']['image'];
                                                $ij++;
                                            }    
                                        }
                                    }else{
                                ...
                                $reservationData[$sectionType][$staff]['last_visit']= date('Y-m-d', strtotime($last_visit));
                                $reservationData[$sectionType][$staff]['ongoing']= $value['Reservation']['status'];
                                $reservationData[$sectionType][$staff]['reservation_type']= $value['Reservation']['reservation_type'];
                                $reservationData[$sectionType][$staff]['event_name']= $value['Reservation']['event_name'];
                                $reservationData[$sectionType][$staff]['staff_name']= $staff_name;
                                $reservationData[$sectionType][$staff]['is_gmail']= $value['Reservation']['is_gmail'];
                                ...
                                $reservationData[$sectionType][$event]['customer_name']= $value['Reservation']['name'];    
                                ...
                                $reservationData[$sectionType][$event]['channel']= $value['Reservation']['channel'];
                                if($value['Reservation']['payment_total']!='null')
                                    $reservationData[$sectionType][$event]['price']= $value['Reservation']['payment_total'];
                                else
                                    $reservationData[$sectionType][$event]['price']= '';
                                ...
                                if(empty($last_visit))
                                    $last_visit =  isset($value['Reservation']['modified']) ? $value['Reservation']['modified'] : '';
                                $reservationData[$sectionType][$event]['last_visit']= date('Y-m-d', strtotime($last_visit));
                                ...
                                $reservationData[$sectionType][$event]['staff_name']= $value['Reservation']['staff_name'];
                                $reservationData[$sectionType][$event]['is_gmail']= $value['Reservation']['is_gmail'];
                                ...
                                $serviceIds = $this->CustomerForm->find('list',array('conditions'=>array('CustomerForm.user_id'=> $user_id), 'fields'=>array('CustomerForm.service_id')));
                                $count = 0;
                                $count = $this->ServiceDetail->find('count', array('conditions' => array('ServiceDetail.user_id' => $user_id, 'ServiceDetail.reservation_id' => $value['Reservation']['id'] )));
                                if($value['Reservation']['all_day']== '1'){
                                    $sectionType ='all_day';


                                    $reservationData[$sectionType][$all_day]['id']= $value['Reservation']['id'];
                                    $reservationData[$sectionType][$all_day]['customer_name']= $value['Reservation']['name'];    
                                    ...
                                    if($value['Reservation']['payment_total']!='null')
                                        ...
                                        $reservationData[$sectionType][$all_day]['price']= '';
                                    ...
                                    $service_id = $value['Reservation']['service_id'];
                                    if(in_array($service_id, $serviceIds) && ($count < 1)){
                                       $reservationData[$sectionType][$all_day]['is_form']= "1";
                                    }else{
                                       $reservationData[$sectionType][$all_day]['is_form']= "0";  
                                    }
                                    if(!empty($service_id)){
                                        $serviceColor = $this->ServiceColor->find('first' ,array('conditions'=>array('ServiceColor.service_id'=>$service_id, 'ServiceColor.user_id' => $user_id)));
                                        //print_r($serviceColor);die;
                                    ...
                                    $reservationData[$sectionType][$all_day]['reservation_type']= $value['Reservation']['reservation_type'];
                                        ...
                                        $reservationData[$sectionType][$all_day]['customer_id']= $value['Customer']['id'];
                                        
                                        $reservationData[$sectionType][$all_day]['customer_name']= $value['Customer']['last_name'].' '.$value['Customer']['first_name'];
                                        $reservationData[$sectionType][$all_day]['tel']= isset($value['Customer']['tel']) ? $value['Customer']['tel'] : '';
                                        $reservationData[$sectionType][$all_day]['customer_status']= $value['Customer']['status'];
                                    ...
                                    if(empty($last_visit))
                                        $last_visit =  isset($value['Reservation']['modified']) ? $value['Reservation']['modified'] : '';
                                    $reservationData[$sectionType][$all_day]['last_visit']= date('Y-m-d', strtotime($last_visit));
                                    ...
                                    $reservationData[$sectionType][$all_day]['staff_name']= $value['Reservation']['staff_name'];
                                    ...
                                    $all_day++;
                                     ...
                                     $sectionType = Configure::read('App.Reservation.morning');
                                    $reservationData[$sectionType][$morning]['id']= $value['Reservation']['id'];
                                    $reservationData[$sectionType][$morning]['customer_name']= $value['Reservation']['name'];    
                                    ...
                                    if($value['Reservation']['payment_total']!='null')
                                        ...
                                        $reservationData[$sectionType][$morning]['price']= '';
                                    ...
                                    $service_id = $value['Reservation']['service_id'];
                                    if(in_array($service_id, $serviceIds) && ($count < 1)){
                                       $reservationData[$sectionType][$morning]['is_form']= "1";
                                    }else{
                                       $reservationData[$sectionType][$morning]['is_form']= "0";  
                                    }
                                    if(!empty($service_id)){
                                        $serviceColor = $this->ServiceColor->find('first' ,array('conditions'=>array('ServiceColor.service_id'=>$service_id, 'ServiceColor.user_id' => $user_id)));
                                        //print_r($serviceColor);die;
                                    ...
                                    $reservationData[$sectionType][$morning]['reservation_type']= $value['Reservation']['reservation_type'];
                                        ...
                                        $reservationData[$sectionType][$morning]['customer_id']= $value['Customer']['id'];
                                        
                                        $reservationData[$sectionType][$morning]['customer_name']= $value['Customer']['last_name'].' '.$value['Customer']['first_name'];
                                        $reservationData[$sectionType][$morning]['tel']= isset($value['Customer']['tel']) ? $value['Customer']['tel'] : '';
                                        $reservationData[$sectionType][$morning]['customer_status']= $value['Customer']['status'];
                                    ...
                                    if(empty($last_visit))
                                        $last_visit =  isset($value['Reservation']['modified']) ? $value['Reservation']['modified'] : '';
                                    $reservationData[$sectionType][$morning]['last_visit']= date('Y-m-d', strtotime($last_visit));
                                    ...
                                    $reservationData[$sectionType][$morning]['staff_name']= $value['Reservation']['staff_name'];
                                    ...
                                    $morning++;
                                     ...
                                     $sectionType = Configure::read('App.Reservation.afternoon');



                                    $reservationData[$sectionType][$afternoon]['id']= $value['Reservation']['id'];
                                    $reservationData[$sectionType][$afternoon]['customer_name']= $value['Reservation']['name'];    
                                    ...
                                    if($value['Reservation']['payment_total']!='null')
                                        ...
                                        $reservationData[$sectionType][$afternoon]['price']= '';
                                    ...
                                    $service_id = $value['Reservation']['service_id'];
                                    if(in_array($service_id, $serviceIds) && ($count < 1)){
                                       $reservationData[$sectionType][$afternoon]['is_form']= "1";
                                    }else{
                                       $reservationData[$sectionType][$afternoon]['is_form']= "0";  
                                    }
                                    if(!empty($service_id)){
                                        $serviceColor = $this->ServiceColor->find('first' ,array('conditions'=>array('ServiceColor.service_id'=>$service_id, 'ServiceColor.user_id' => $user_id)));
                                        //print_r($serviceColor);die;
                                    ...
                                    $reservationData[$sectionType][$afternoon]['reservation_type']= $value['Reservation']['reservation_type'];
                                        ...
                                        $reservationData[$sectionType][$afternoon]['customer_id']= $value['Customer']['id'];
                                        
                                        $reservationData[$sectionType][$afternoon]['customer_name']= $value['Customer']['last_name'].' '.$value['Customer']['first_name'];
                                        $reservationData[$sectionType][$afternoon]['tel']= isset($value['Customer']['tel']) ? $value['Customer']['tel'] : '';
                                        $reservationData[$sectionType][$afternoon]['customer_status']= $value['Customer']['status'];
                                    ...
                                    if(empty($last_visit))
                                        $last_visit =  isset($value['Reservation']['modified']) ? $value['Reservation']['modified'] : '';
                                    $reservationData[$sectionType][$afternoon]['last_visit']= date('Y-m-d', strtotime($last_visit));
                                    ...
                                    $reservationData[$sectionType][$afternoon]['staff_name']= $value['Reservation']['staff_name'];
...
/**************************************************************************
    ...
    function get_reservation_ipad_old($testData = null){
        ...
        if(!empty($user_id)){
            ...
            if(!$userData){
                ...
                $this->Reservation->bindModel(array('belongsTo' => array('Customer', 'Service')));
                $reservationDataFind = $this->Reservation->find('all',array('conditions'=>array( 'Reservation.user_id'=>$user_id, 'Reservation.start_date <=' => date('Y-m-d'), 'Reservation.end_date >=' => date('Y-m-d'),  'Reservation.status !='=>'5'),  'order' => array('Reservation.start_date' => 'DESC', 'Reservation.start_time' => 'ASC')));
...
/**************************************************************************
                    ...
                    foreach ($reservationDataFind as $key => $value) {
                            ...
                            if($value['Reservation']['reservation_type']== '3'){
                                ...
                                if($value['Reservation']['all_day']== '1'){
                                    ...
                                    $reservationData[$sectionType][$all_day]['is_gmail']= $value['Reservation']['is_gmail'];
                                    $reservationData[$sectionType][$all_day]['reservation_type']= $value['Reservation']['reservation_type'];
                                    $reservationData[$sectionType][$all_day]['reservation_number']= $value['Reservation']['reservation_number'];
                                    ...
                                    $reservationData[$sectionType][$morning]['extra_end_date']= $value['Reservation']['extra_end_date'];
                                     ...
                                     $reservationData[$sectionType][$morning]['event_name']= $value['Reservation']['event_name'];
                                     $reservationData[$sectionType][$morning]['reservation_type']= $value['Reservation']['reservation_type'];
                                     $reservationData[$sectionType][$morning]['reservation_number']= $value['Reservation']['reservation_number'];
                                    ...
                                    $reservationData[$sectionType][$afternoon]['extra_end_date']= $value['Reservation']['extra_end_date'];
                                     ...
                                     $reservationData[$sectionType][$afternoon]['event_name']= $value['Reservation']['event_name'];
                                     $reservationData[$sectionType][$afternoon]['reservation_type']= $value['Reservation']['reservation_type'];
                                     $reservationData[$sectionType][$afternoon]['reservation_number']= $value['Reservation']['reservation_number'];
                                     $reservationData[$sectionType][$afternoon]['price']= $value['Reservation']['payment_total'];
                                      if($value['Reservation']['channel'] != 'null')
                                        $reservationData[$sectionType][$afternoon]['channel']= $value['Reservation']['channel'];
                                    else
    ...
    function get_date_reservation_list_old($testData = null){
            ...
            if(!$userData){
                ...
                if(!empty($reservationDataFind)){
                    foreach ($reservationDataFind as $key => $value) {
                            ...
                            $reservationStatusReadData = $this->ReservationStatusRead->find('first',array('conditions'=>array('ReservationStatusRead.reservation_id'=>$reservation_id, 'ReservationStatusRead.employee_id'=>$employee_id, 'ReservationStatusRead.user_id'=>$user_id)));
                            if(isset($reservationStatusReadData['ReservationStatusRead']['status']) && ($reservationStatusReadData['ReservationStatusRead']['status'] == '1')){

                                $reservationStatusRead = '1';
                            }else{
                                $reservationStatusRead = '0';
                            }
                            ...
                            if($value['Reservation']['reservation_type']== '3'){
                                ...
                                $reservationData[$sectionType][$staff]['service_id']= $value['Reservation']['service_id'];
                                $reservationData[$sectionType][$staff]['service_name']= $value['Service']['name'];
                                $reservationData[$sectionType][$staff]['read_status']= $reservationStatusRead;
                                ...
                                $reservationData[$sectionType][$event]['service_id']= $value['Reservation']['service_id'];
                                $reservationData[$sectionType][$event]['service_name']= $value['Service']['name'];
                                $reservationData[$sectionType][$event]['read_status']= $reservationStatusRead;
                                ...
                                if(!empty($allEmp)){
                                    foreach ($allEmp as $empKey => $empValue) {
                                        $emp_name= $empValue['Employee']['name'];
                                        $emp_service_id= $empValue['Employee']['service_id'];
                                        $empReservationData[$emp_name][$employee]['id']= $value['Reservation']['id'];
                                        $empReservationData[$emp_name][$employee]['user_id']= $value['Reservation']['user_id'];
                                        $empReservationData[$emp_name][$employee]['start_date']= $value['Reservation']['start_date'];
                                        $empReservationData[$emp_name][$employee]['end_date']= $value['Reservation']['end_date'];
                                        $empReservationData[$emp_name][$employee]['extra_start_date']= $value['Reservation']['extra_start_date'];
                                        $empReservationData[$emp_name][$employee]['extra_end_date']= $value['Reservation']['extra_end_date'];
                                        $empReservationData[$emp_name][$employee]['start_time']= $value['Reservation']['start_time'];
                                        $empReservationData[$emp_name][$employee]['end_time']= $value['Reservation']['end_time'];
                                        $empReservationData[$emp_name][$employee]['service_id']= $value['Reservation']['service_id'];
                                        $empReservationData[$emp_name][$employee]['service_name']= $value['Service']['name'];
                                        $empReservationData[$emp_name][$employee]['read_status']= $reservationStatusRead;
                                        $empReservationData[$emp_name][$employee]['customer_name']= $value['Customer']['last_name'].' '.$value['Customer']['first_name'];

                                        $empReservationData[$emp_name][$employee]['customer_id']= $value['Reservation']['customer_id'];
                                        $empReservationData[$emp_name][$employee]['event_name']= $value['Reservation']['event_name'];
                                        $empReservationData[$emp_name][$employee]['is_event']= $value['Reservation']['is_event'];
                                      
                                        $empReservationData[$emp_name][$employee]['is_gmail']= $value['Reservation']['is_gmail'];
                                     
                                        $empReservationData[$emp_name][$employee]['all_day']= $value['Reservation']['all_day'];
                                        $empReservationData[$emp_name][$employee]['ongoing']= $value['Reservation']['status'];
                                        $empReservationData[$emp_name][$employee]['reservation_type']= $value['Reservation']['reservation_type'];
                                        $empReservationData[$emp_name][$employee]['reservation_number']= $value['Reservation']['reservation_number'];
                                        $empReservationData[$emp_name][$employee]['used_points']= ($value['Reservation']['used_points'] != 'null')  ? (string)$value['Reservation']['used_points'] : '0';

                                        $empReservationData[$emp_name][$employee]['note']= $value['Reservation']['note'];
                                        $empReservationData[$emp_name][$employee]['price']= $value['Reservation']['payment_total'];
                                        if($value['Reservation']['channel'] != 'null')
                                            $empReservationData[$emp_name][$employee]['channel']= $value['Reservation']['channel'];
                                        else
                                            $empReservationData[$emp_name][$employee]['channel']= '';

                                        if(!empty($serviceColor['ServiceColor']['color_code'])){
                                            $empReservationData[$emp_name][$employee]['service_color']= $this->get_service_color($emp_service_id, $user_id);
                                        }else{
                                            $empReservationData[$emp_name][$employee]['service_color']= '';
                                        }
                                     
                                        $inner_employee_ids = $value['Reservation']['employee_ids'];
                                        if(!empty($employee_ids)){
                                        $inner_employee_ids =  explode(",", $inner_employee_ids);
                                        $innerAllEmp = $this->Employee->find('all' ,array('conditions'=>array('Employee.id'=>$inner_employee_ids)));
                                        // print_r($allEmp);die;
                                        }

                                        if(!empty($innerAllEmp)){
                                        foreach ($innerAllEmp as $innerEmpKey => $innerEmpValue) {
                                            $empReservationData[$emp_name][$employee]['Employee'][$innerEmpKey]['id']= $innerEmpValue['Employee']['id'];
                                            $empReservationData[$emp_name][$employee]['Employee'][$innerEmpKey]['name']= $innerEmpValue['Employee']['name'];
                                            $empReservationData[$emp_name][$employee]['Employee'][$innerEmpKey]['emp_code']= $innerEmpValue['Employee']['emp_code'];
                    ...
                    $reservationData['User']['salon_start_time']= $salon_start_time;
                    $reservationData['User']['salon_end_time']= $salon_end_time;    
                    if(!empty($user_id) && !empty($date) && !empty($employee_id)){
                        $reservationReadData = $this->ReservationRead->find('first',array('conditions'=>array('ReservationRead.date'=>$date, 'ReservationRead.employee_id'=>$employee_id, 'ReservationRead.user_id'=>$user_id)));
                        // pr($notificationReadData);
                        if(isset($reservationReadData['ReservationRead']['id']) && !empty($reservationReadData['ReservationRead']['id'])){
                             $this->ReservationRead->id = $reservationReadData['ReservationRead']['id'];
                             $this->ReservationRead->saveField("status","1");
                        }else{
                            $reservationStatusData['ReservationRead']['user_id'] =  $user_id;
                            $reservationStatusData['ReservationRead']['employee_id'] =  $employee_id;
                            $reservationStatusData['ReservationRead']['date'] =  $date;
                            $reservationStatusData['ReservationRead']['status'] =  '1';
                            $this->ReservationRead->saveAll($reservationStatusData);
                        }
        ...
        $log = $this->Reservation->getDataSource()->getLog(false, false);
        $recordData['RecordData']['name'] = "get_date_reservation_list";
        $recordData['RecordData']['query'] = json_encode($log);
        $this->RecordData->saveAll($recordData);
        echo  $jsonEncode;exit();
    }

    function get_date_reservation_list($testData = null){
            ...
            if(!$userData){
                ...
                if(!empty($reservationDataFind)){
                    foreach ($reservationDataFind as $key => $value) {
                            ...
                            if(($value['Reservation']['reservation_type']== '3') && ($value['Reservation']['all_day']== '1')){
                                $sectionType =Configure::read('App.Reservation.staff');
                                $reservationData[$sectionType][$staff]['id']= $value['Reservation']['id'];
                                $reservationData[$sectionType][$staff]['user_id']= $value['Reservation']['user_id'];
                                $reservationData[$sectionType][$staff]['customer_id']= $value['Reservation']['customer_id'];
    ...
    function get_reservation($testData = null){
        ...
        if(!empty($user_id)){
            ...
            if(!$userData){
                ...
                $this->Reservation->bindModel(array('belongsTo' => array('Customer', 'Service')));
                $reservationDataFind = $this->Reservation->find('all',array('conditions'=>array( 'Reservation.user_id'=>$user_id,  'Reservation.status !='=>'5'),  'order' => array('Reservation.start_date' => 'DESC')));
    ...
    function customer_marge(){
       ...
       if(!empty($customer_ids)){
            ...
            $max_customer_id = $customer_ids[0];
           ...
           //   echo $max_customer_id;die;
            if($count_customer_ids != 1){
                foreach ($customer_ids as  $value) {
                    if($max_customer_id != $value){
                        ...
                        $this->NoteService->updateAll(array('NoteService.customer_id' => $max_customer_id),array('NoteService.customer_id' => $value));
                        $this->NoteProduct->updateAll(array('NoteProduct.customer_id' => $max_customer_id),array('NoteProduct.customer_id' => $value));
                        $this->NoteTicket->updateAll(array('NoteTicket.customer_id' => $max_customer_id),array('NoteTicket.customer_id' => $value));
                        $this->NoteRemainingTicket->updateAll(array('NoteRemainingTicket.customer_id' => $max_customer_id),array('NoteRemainingTicket.customer_id' => $value));
            ...
            $data['Customer']['name'] = $data['Customer']['last_name']." ".$data['Customer']['first_name'];
            // print_r($data);die;
        ...
        $log = $this->Customer->getDataSource()->getLog(false, false);
        $recordData['RecordData']['name'] = "customer_marge";
        $recordData['RecordData']['query'] = json_encode($log);
        $this->RecordData->saveAll($recordData);
        echo  $jsonEncode;exit();
      ...
      function product_list($testData = null){
        ...
        if(!empty($user_id)){
            ...
            $customerData[$i]['Product']['msg1'] = '商品は存在しません.';
             $customerData[$i]['Product']['msg'] = '製品がありません';
            $customerData[$i]['Product']['status'] = 'error';
            $jsonEncode = json_encode($customerData);
        }
        $log = $this->Product->getDataSource()->getLog(false, false);
    ...
    function delete_product($test_data = null){
        ...
        if(!empty($id)){
            if($this->Product->delete($id, true)){
                $responseArr = array('status' => 'success', 'msg' => '製品が削除しました' );
                $jsonEncode = json_encode($responseArr);
            }else{
            ...
            $responseArr = array('status' => 'error', 'msg' => '製品がありません'  );
            $jsonEncode = json_encode($responseArr);
        }
        $log = $this->Product->getDataSource()->getLog(false, false);
...
/**************************Category Section***********************************/
 ...
 /**************************************************************************
    ...
    function add_category($testData = null){
        ...
        $user_id = $userCategoryData['UserCategory']['user_id'] = isset($decoded['user_id']) ? $decoded['user_id'] : ''; 
        $parent_id = $userCategoryData['UserCategory']['parent_id'] = isset($decoded['parent_id']) ? $decoded['parent_id'] : '0'; 
        $name = $userCategoryData['UserCategory']['name'] = isset($decoded['name']) ? $decoded['name'] : '';
        $japanese_name = $userCategoryData['UserCategory']['japanese_name'] = isset($decoded['name']) ? $decoded['name'] : '';

        $categoryCountData = $this->UserCategory->find('count', array('conditions'=>array('UserCategory.name'=>$name, 'UserCategory.parent_id'=>$parent_id,'UserCategory.user_id'=>$user_id )));
        if($categoryCountData > 0){
            $UserCategory['msg'] = '実際の売上'; // please enter user detail
            $UserCategory['msg1'] = 'This category name is already exist.'; // please enter user detail
        ...
        if($this->UserCategory->saveAll($userCategoryData)){
            ...
            $UserCategory['msg'] = 'カテゴリは正常に追加されました。'; // category have added successfully
            $UserCategory['status'] = 'success';
            $jsonEncode = json_encode($UserCategory);
            
        }else{
            $UserCategory['msg'] = 'ユーザー詳細を入力してください。'; // please enter user detail
            $UserCategory['status'] = 'error';
            $jsonEncode = json_encode($UserCategory);
        }
        $log = $this->UserCategory->getDataSource()->getLog(false, false);
    ...
    function delete_budget($test_data = null){
        ...
        if(!empty($id)){
            if($this->Budget->delete($id, true)){
                $responseArr = array('status' => 'success', 'msg' => 'カテゴリー予算を削除完了しました' );
                $jsonEncode = json_encode($responseArr);
            }else{
            ...
            $responseArr = array('status' => 'error', 'msg' => 'カテゴリー予算を存在しません'  );
            $jsonEncode = json_encode($responseArr);
        }
        $log = $this->Budget->getDataSource()->getLog(false, false);
    ...
    function delete_category($test_data = null){
        ...
        if(!empty($id)){
            if($this->UserCategory->delete($id, true)){
                $responseArr = array('status' => 'success', 'msg' => 'カテゴリーを削除完了しました' );
                $jsonEncode = json_encode($responseArr);
            }else{
            ...
            $responseArr = array('status' => 'error', 'msg' => 'カテゴリーを存在しません'  );
            $jsonEncode = json_encode($responseArr);
        }
        $log = $this->UserCategory->getDataSource()->getLog(false, false);
...
/**************************Expenses Section***********************************/
    ...
    function add_expense(){
        ...
        if($this->Expense->saveAll($expense)){
            ...
            $responseArr['user_id'] = $decoded['user_id'];
            $responseArr['status'] = 'success';
            $responseArr['msg'] = $msg;
            ...
            $responseArr['expense_id'] = $decoded['id'];
            $responseArr['user_id'] = $decoded['user_id'];
            $responseArr = array('status' => 'error' );
            $jsonEncode = json_encode($responseArr);
        }
    ...
    /*
      ...
      function expense_list($testData = null){
        ...
        if(!empty($user_id)){
            ...
            $expenseData[$i]['Expense']['msg'] = '支出を存在しません';
            $expenseData[$i]['Expense']['status'] = 'error';
            $jsonEncode = json_encode($expenseData);
        }
        $log = $this->Expense->getDataSource()->getLog(false, false);
                ...
                if(!empty($data)){
                    ...
                    foreach ($data as $key => $value) {
                        ...
                        if(isset($value['Expense'][0]['user_category_id']) || isset($value['Expense']['user_category_id']) || isset($value['Budget']['user_category_id'])){
                            ...
                            $expenseData['Expense'][$i]['category_id'] = $parent_id;
                            $expenseData['Expense'][$i]['name'] = $value['UserCategory']['name'];
                            $expenseData['Expense'][$i]['japanese_name'] = $value['UserCategory']['japanese_name'];
                            $expenseData['Expense'][$i]['parent_category_name'] = $this->get_category_name($parent_id);
                            $expenseData['Expense'][$i]['parent_category_japanese_name'] = $this->get_category_japanese_name($parent_id);
...
/**************************************************************************
     * NAME: sub_category_all_expense_list
     * Description: Sub Category All Expense List .
     *
     * Subroutines Called:
     *
     * Parameters: Input / Output
     ...
     *********************************************************************/
      ...
      function sub_category_all_expense_list($testData = null){
        ...
        $categoryData['subCategoryWithExpense'] = array();
        if(!empty($user_id)){
           ...
           $this->UserCategory->bindModel(
            ...
            $data = $this->UserCategory->find('all',array('conditions'=> array( 'UserCategory.user_id'=>$user_id,'UserCategory.parent_id'=>$category_id), 'order' =>array('UserCategory.modified'=>'DESC')));
            // pr($data);die;
            $main_category = $this->UserCategory->find('first',array('conditions'=> array( 'UserCategory.id'=>$category_id,'UserCategory.user_id'=>$user_id)));

            $i =0;
                foreach ($data as $key => $subCategoryValue) {                   
                    if(isset($subCategoryValue['Expense']) && !empty($subCategoryValue['Expense'])){
                        foreach ($subCategoryValue['Expense'] as $key => $expenseValue) {

                            $categoryData['subCategoryWithExpense'][$i] =  $expenseValue;
                            $categoryData['subCategoryWithExpense'][$i]['user_sub_category_id'] =  $expenseValue['user_category_id'];
                            $categoryData['subCategoryWithExpense'][$i]['created'] =  date('Y-m-d', strtotime($expenseValue['created']));
                            $categoryData['subCategoryWithExpense'][$i]['modified'] =  date('Y-m-d', strtotime($expenseValue['modified']));
                            $categoryData['subCategoryWithExpense'][$i]['name'] =  $main_category['UserCategory']['name'];
                            $categoryData['subCategoryWithExpense'][$i]['user_category_id'] =  $main_category['UserCategory']['id'];
                            $categoryData['subCategoryWithExpense'][$i]['japanese_name'] =  $main_category['UserCategory']['japanese_name'];
                            $categoryData['subCategoryWithExpense'][$i]['image'] =  $main_category['UserCategory']['image'];
                            $categoryData['subCategoryWithExpense'][$i]['sub_category_name'] =  $subCategoryValue['UserCategory']['name'];
                            $categoryData['subCategoryWithExpense'][$i]['sub_category_japanese_name'] =  $subCategoryValue['UserCategory']['japanese_name'];
                        ...
                        $categoryData['date'] = $month_date;
                        $categoryData['msg'] = 'レコードが見つかりませんでした。';
                        $categoryData['msg1'] = 'No Record Found.';
                        $categoryData['status'] = 'error';
                    }
                }

                $jsonEncode = json_encode($categoryData);
                echo  $jsonEncode;exit();           
        }
    ...
    /**************************************************************************
     * NAME: category_all_expense_list
     * Description: Category All Expense List .
     *
     * Subroutines Called:
     *
     * Parameters: Input / Output
     ...
     *********************************************************************/
      ...
      function category_all_expense_list($testData = null){
        ...
        $categoryData['allCategoryWithExpense'] = array();
        if(!empty($user_id)){
           ...
           $this->UserCategory->bindModel(
            ...
            $data = $this->UserCategory->find('all',array('conditions'=> array( 'UserCategory.user_id'=>$user_id, 'UserCategory.parent_id !='=>"0"), 'order' =>array('UserCategory.modified'=>'DESC')));
            // pr($data);die;
            $i =0;
                foreach ($data as $key => $subCategoryValue) {
                        $main_category = $this->UserCategory->find('first',array('conditions'=> array( 'UserCategory.id'=>$subCategoryValue['UserCategory']['parent_id'],'UserCategory.user_id'=>$user_id)));
                        foreach ($subCategoryValue['Expense'] as $key => $expenseValue) {
                            $categoryData['allCategoryWithExpense'][$i] =  $expenseValue;
                            $categoryData['allCategoryWithExpense'][$i]['created'] =  date('Y-m-d', strtotime($expenseValue['created']));
                            $categoryData['allCategoryWithExpense'][$i]['modified'] =  date('Y-m-d', strtotime($expenseValue['modified']));
                            $categoryData['allCategoryWithExpense'][$i]['name'] =  $main_category['UserCategory']['name'];
                            $categoryData['allCategoryWithExpense'][$i]['japanese_name'] =  $main_category['UserCategory']['japanese_name'];
                            $categoryData['allCategoryWithExpense'][$i]['image'] =  $main_category['UserCategory']['image'];
                            $categoryData['allCategoryWithExpense'][$i]['user_sub_category_id'] =  $subCategoryValue['UserCategory']['id'];
                            $categoryData['allCategoryWithExpense'][$i]['sub_category_name'] =  $subCategoryValue['UserCategory']['name'];
                            $categoryData['allCategoryWithExpense'][$i]['sub_category_japanese_name'] =  $subCategoryValue['UserCategory']['japanese_name'];
...
/****************************************************************************************************************************************
    ...
    function delete_expense($test_data = null){
        ...
        if(!empty($id)){
            if($this->Expense->delete($id, true)){
                $responseArr = array('status' => 'success', 'msg' => '支出を削除しました' );
                $jsonEncode = json_encode($responseArr);
            }else{
            ...
            $responseArr = array('status' => 'error', 'msg' => '支出を存在しません'  );
            $jsonEncode = json_encode($responseArr);
        }
        $log = $this->Expense->getDataSource()->getLog(false, false);
    ...
    function add_employee_chat($test_data = null){
        ...
        $chat_type = $employeeChat['EmployeeChat']['chat_type'] = isset($decoded['chat_type']) ? $decoded['chat_type'] : '';
        $chat_text = $employeeChat['EmployeeChat']['chat_text'] = isset($decoded['chat_text']) ? $decoded['chat_text'] : '';
        ...
        if($this->EmployeeChat->saveAll($employeeChat)){
            ...
            if($chat_type == '2'){
                $chatNotificationMessage = $chat_text;
            }else{
                 $chatNotificationMessage = '画像を送信しました';
            }
            ...
            foreach ($employeeData as $empKey => $empValue) {
                ...
                if(!empty($empDevicetoken) && ($emp_id  != $employee_id) && !in_array($empDevicetoken, $deviceTokenArr) ){
                    ...
                    if($emp_device_type == 'Android'){
                        ...
                        $this->androidPushNotification($empDevicetoken, $chatNotificationMessage, $user_id, $empValue['Employee']['id'],$employee_id,'', $emp_device_type, 'employee', 'chat', Configure::read('App.Firebase.apikey')) ;
                    }else{
                        $this->IOSPushNotification($empDevicetoken, $chatNotificationMessage, $user_id, $empValue['Employee']['id'],$employee_id,'', $emp_device_type, 'employee', 'chat') ;
                    }
                    array_push($deviceTokenArr, $empDevicetoken);
                }
           }
    ...
    function list_employee_chat($test_data = null){
        ...
        if(!empty($data)){
            ...
            foreach ($data as $key => $value) {
                ...
                $employeeChatData['EmployeeChat'][$date][$i]['chat_time'] = date('H:i',  strtotime($value['EmployeeChat']['chat_time']));
...
/*******************************************Gallery Section********************************************************/
 ...
 /****************************************************************************************************************************************
    ...
    function upload_gallery_image_old(){
        ...
        if(!empty($_FILES)){
                ...
                if (!empty($this->Upload->result)){
                    ...
                    $result['message'] = 'success';
                    $result['response_data'] = array();
                    $result['response_data']['image'] = $newName;
   ...
   function upload_gallery_image(){
        ...
        $recordData['RecordData']['query'] = json_encode($_FILES);
        $this->RecordData->saveAll($recordData);
        if(!empty($_FILES)){
        ...
        if($this->Gallery->saveAll($gallery)){
            ...
            foreach ($employeeData as $empKey => $empValue) {
                ...
                if(!empty($empDevicetoken) && ($emp_id  != $employee_id) && !in_array($empDevicetoken, $deviceTokenArr) ){
                    ...
                    if($emp_device_type == 'Android'){
                        $this->androidPushNotification($empDevicetoken, $galleryImageNotificationMessage, $user_id, $empValue['Employee']['id'],'', $album_id,  $emp_device_type, 'employee', 'gallery_image', Configure::read('App.Firebase.apikey'));
                        array_push($deviceTokenArr, $empDevicetoken);
                    }else{
                        $this->IOSPushNotification($empDevicetoken, $galleryImageNotificationMessage, $user_id, $empValue['Employee']['id'],'', $album_id,  $emp_device_type, 'employee', 'gallery_image');
                        array_push($deviceTokenArr, $empDevicetoken);
    ...
    /****************************************************************************************************************************************
     * NAME: add_multipart_gallery_image
     * Description: Register a user with .
     *
     * Subroutines Called:
     *
    ...
    function add_multipart_gallery_image($test_data = null){
        $data = file_get_contents('php://input');
        ...
        $recordData['RecordData']['name'] = "add_multipart_gallery_image";
        $recordData['RecordData']['query'] = json_encode($data);
        $this->RecordData->saveAll($recordData);
        $decoded = json_decode($data, true);
        $responseArr = array();
        ...
        $gallery['Gallery']['employee_image'] = $emp_image;
        // $gallery['Gallery']['image'] = isset($decoded['image']) ? $decoded['image'] : '';
        $gallery['Gallery']['date'] = isset($decoded['date']) ? date( 'Y-m-d H:i:s', strtotime($decoded['date'] ) ): '';
        $galleryRes = false;

        if(isset($_FILES['image']) && !empty($_FILES['image'])){
                $i = 0;
                $imageUploadArr = array();
                foreach ($_FILES['image']['error'] as $key => $value) {
                    $imageUploadArr[$key]['image']['error'] = $value;
                }
                foreach ($_FILES['image']['name'] as $key => $value) {
                    $imageUploadArr[$key]['image']['name'] = $value;
                }
                foreach ($_FILES['image']['size'] as $key => $value) {
                    $imageUploadArr[$key]['image']['size'] = $value;
                }
                foreach ($_FILES['image']['tmp_name'] as $key => $value) {
                    $imageUploadArr[$key]['image']['tmp_name'] = $value;
                }
                foreach ($_FILES['image']['type'] as $key => $value) {
                    $imageUploadArr[$key]['image']['type'] = $value;
                }
                $jsonEncode = json_encode($imageUploadArr);
                // echo  $jsonEncode;exit();
                foreach ($imageUploadArr as $fileKey => $fileValue) {
                    $gallery['Gallery']['image'] = array();   
                    $path_info = pathinfo($fileValue['image']['name']);

                    $newName = md5(time()*rand()).'.'.$path_info['extension'];

                    $thumbRules = array('size' => array(GALLERY_THUMB_WIDTH, GALLERY_THUMB_HEIGHT), 'type' => 'resizecrop');
                    $thumb = $this->Upload->upload($fileValue['image'], WWW_ROOT . GALLERY_IMG_THUMB_DIR, $newName, $thumbRules);
                    /* medium */
                     $mediumRules = array('size' => array(GALLERY_MEDIUM_WIDTH, GALLERY_MEDIUM_HEIGHT), 'type' => 'resizecrop');
                    $medium = $this->Upload->upload($fileValue['image'], WWW_ROOT . GALLERY_IMG_MEDIUM_DIR, $newName, $mediumRules);

                    $verticalRules = array('size' => array(GALLERY_VERTICAL_WIDTH, GALLERY_VERTICAL_HEIGHT), 'type' => 'resizecrop');
                    $vertical = $this->Upload->upload($fileValue['image'], WWW_ROOT . GALLERY_IMG_VERTICAL_DIR, $newName, $verticalRules);

                    $res3 = $this->Upload->upload($fileValue['image'], WWW_ROOT . GALLERY_IMG_ORIGINAL_DIR, $newName, '', array('png', 'jpg', 'jpeg', 'gif'));
                    $gallery['Gallery']['image'] = $this->Upload->result;
                    $this->Gallery->saveAll($gallery);
                    $galleryRes = true;
                }    
        }
        ...
        if($galleryRes){
           ...
           /* $employeeData = $this->Employee->find('all', array('conditions'=>array('Employee.user_id'=>$user_id)));
            ...
            $responseArr = array('album_id' => $album_id, 'msg'=>'顧客情報が正常に追加されました。',  'msg1'=>'Gallery image successfully added.', 'status' => 'success' );
            $jsonEncode = json_encode($responseArr);
        }else{
            $responseArr = array('status' => 'error' );
            $jsonEncode = json_encode($responseArr);
    ...
    function list_gallery_image($test_data = null){
        ...
        $data = $this->Gallery->find('all',array('conditions'=> array( 'Gallery.user_id'=>$user_id, 'Gallery.album_id'=>$album_id), 'order' =>array('Gallery.id'=>'DESC')));
...
/****************************************************************************************************************************************
    ...
    function user_notification_list($test_data = null){
        ...
        if(!empty($data)){
            foreach ($data as $key => $value) {
                ...
                $reservation_id = $pushNotificationData['PushNotification'][$i]['reservation_id'] = ($value['PushNotification']['reservation_id'] != null) ? $value['PushNotification']['reservation_id'] : '';
                ...
                if($notification_type == 'add_note'){
                    $customerHistoryData = $this->CustomerHistory->find('first',array('conditions'=> array( 'CustomerHistory.id'=>$reservation_id), 'fields' =>array('CustomerHistory.date')));
                ...
                if(($notification_type == 'add_reservation') || ($notification_type == 'reservation_status')){
                    $reservationData = $this->Reservation->find('first', array('conditions' =>array('Reservation.id'=>$reservation_id,  'Reservation.status !='=>'5')));
                     $pushNotificationData['PushNotification'][$i]['reservation_date'] = isset($reservationData['Reservation']['start_date']) ? $reservationData['Reservation']['start_date'] : '';
                }
    ...
    function employee_notification_list_old($test_data = null){
        $data = file_get_contents('php://input');
        ...
        $this->loadModel('NotificationCount');
        $employee_id = isset($decoded['employee_id']) ? $decoded['employee_id'] : '';
        $this->PushNotification->bindModel(array('hasOne' => array('NotificationRead'=>array('conditions'=>array('NotificationRead.employee_id' => $employee_id)))));
        // $data = $this->PushNotification->find('all',array('conditions'=> array( 'PushNotification.employee_id'=>$employee_id, 'PushNotification.member_type'=>'employee'), 'order' =>array('PushNotification.created'=>'DESC')));
         // pr($data);die;
        $data = $this->PushNotification->find('all',array('conditions'=> array( 'PushNotification.employee_id'=>$employee_id, 'PushNotification.member_type'=>'employee', 'PushNotification.notification_type !='=>'chat'), 'order' =>array('PushNotification.created'=>'DESC')));

        $notificationCountData = $this->NotificationCount->find('first',array('conditions'=>array('NotificationCount.employee_id'=>$employee_id)));
        // pr($notificationCountData);die;
        if(isset($notificationCountData['NotificationCount']['id']) && !empty($notificationCountData['NotificationCount']['id'])){
            ...
            $this->NotificationCount->saveField('count' ,  0);
        }
        // pr($data);die;
        ...
        if(!empty($data)){
            foreach ($data as $key => $value) {
                $push_notification_id = $pushNotificationData['PushNotification'][$i]['id'] = ($value['PushNotification']['id'] != null) ? $value['PushNotification']['id'] : '';
                $pushNotificationData['PushNotification'][$i]['user_id'] = ($value['PushNotification']['user_id'] != null) ? $value['PushNotification']['user_id'] : '';
                $pushNotificationData['PushNotification'][$i]['employee_id'] = ($value['PushNotification']['employee_id'] != null) ? $value['PushNotification']['employee_id'] : '';
                $pushNotificationData['PushNotification'][$i]['customer_id'] = ($value['PushNotification']['customer_id'] != null) ? $value['PushNotification']['customer_id'] : '';
                $reservation_id = $pushNotificationData['PushNotification'][$i]['reservation_id'] = ($value['PushNotification']['reservation_id'] != null) ? $value['PushNotification']['reservation_id'] : '';
                ...
                $notificationCountData = array();
                $notificationCountData = $this->NotificationRead->find('first',array('conditions'=>array('NotificationRead.employee_id'=>$employee_id, 'NotificationRead.push_notification_id'=>$push_notification_id)));

                // echo $value['NotificationRead']['status'];die;
                if(isset($value['NotificationRead']['status']) && !empty($value['NotificationRead']['status'])){
                    $pushNotificationData['PushNotification'][$i]['read_status'] = '1';
                }else{
                    $pushNotificationData['PushNotification'][$i]['read_status'] = '0'; 
                }
                $pushNotificationData['PushNotification'][$i]['device_token'] = ($value['PushNotification']['device_token'] != null) ? $value['PushNotification']['device_token'] : '';
                $pushNotificationData['PushNotification'][$i]['device_type'] = ($value['PushNotification']['device_type'] != null) ? $value['PushNotification']['device_type'] : '';
                $pushNotificationData['PushNotification'][$i]['member_type'] = ($value['PushNotification']['member_type'] != null) ? $value['PushNotification']['member_type'] : '';
                $pushNotificationData['PushNotification'][$i]['status'] = ($value['PushNotification']['status'] != null) ? $value['PushNotification']['status'] : '';
                $pushNotificationData['PushNotification'][$i]['created'] = ($value['PushNotification']['created'] != null) ? date( 'Y年m月d日 H時i分', strtotime( $value['PushNotification']['created'])) : '';
                $pushNotificationData['PushNotification'][$i]['modified'] = ($value['PushNotification']['modified'] != null) ? date( 'Y年m月d日 H時i分', strtotime( $value['PushNotification']['modified'])) : '';
                // pr($pushNotificationData);die;
                $i++;
            }
    ...
    function employee_notification_list($test_data = null){
        ...
        $this->loadModel('NotificationCount');
        $employee_id = isset($decoded['employee_id']) ? $decoded['employee_id'] : '';
        $page = isset($decoded['page']) ? $decoded['page'] : 1;
        if($page == 1){
            $start = 0;
        }else{
             $start = $page * 20;
        }
        $length = Configure::read('App.Page.length');

        $this->PushNotification->bindModel(array('hasOne' => array('NotificationRead'=>array('conditions'=>array('NotificationRead.employee_id' => $employee_id)))));
        $dataCount = $this->PushNotification->find('count',array('conditions'=> array( 'PushNotification.employee_id'=>$employee_id, 'PushNotification.member_type'=>'employee', 'PushNotification.notification_type !='=>'chat')));
        $data = $this->PushNotification->find('all',array('conditions'=> array( 'PushNotification.employee_id'=>$employee_id, 'PushNotification.member_type'=>'employee', 'PushNotification.notification_type !='=>'chat'), 'order' =>array('PushNotification.created'=>'DESC'), 'limit' => $start.','.$length));
        ...
        $i =0;
        if(!empty($data)){
             $pushNotificationData['count'] = (string)$dataCount;
            foreach ($data as $key => $value) {
                $push_notification_id = $pushNotificationData['PushNotification'][$i]['id'] = ($value['PushNotification']['id'] != null) ? $value['PushNotification']['id'] : '';
                $pushNotificationData['PushNotification'][$i]['user_id'] = ($value['PushNotification']['user_id'] != null) ? $value['PushNotification']['user_id'] : '';
                $pushNotificationData['PushNotification'][$i]['employee_id'] = ($value['PushNotification']['employee_id'] != null) ? $value['PushNotification']['employee_id'] : '';
...
/************************ Push Notification ***********************/
    ...
    public function IOSPushNotification($deviceToken=null,$message=null,$user_id=null,$employee_id=null,$customer_id=null,  $reservation_id=null, $device_type=null, $member_type=null, $notification_type=null) {
     ...
     public function androidPushNotification($deviceToken="fzLq-lukLVU:APA91bE6Jyk9F7gyz4FC2Thux1plnTkQCl5iFGXd_9x58xI2I4QxJ7sraPxQTZ8woxOof0O0-Bos6bvvVwCbjENnkSCVQP4oHenZmLh9ayGA2vZ69TeGLJtefbdlJJzdPG-xcywJV_FM",$message="Jai Hanuman",$user_id="33",$employee_id="29",$customer_id="4785",  $reservation_id="7929", $device_type="Android", $member_type="employee", $notification_type="add_reservation", $firebase_api_key=null){
    ...
    public function androidNotification(){
        // API access key from Google API's Console
        define( 'API_ACCESS_KEY', 'AAAAZ7uaMes:APA91bEXH95N3_9Btg4E59Q3E6G5nk7PSXfEZQ7Qw63B9KldLCtXFDB3Hp2sdZHHVe1ROeJKXWRQ9rw0furJXxve2cdK056yAJ8rd9Oo8dwN8iyysAyxXGOk1dyhhAfbm_GckuL9Suqv' );
        $registrationIds =  array('e_xU1QJV9cU:APA91bHUDujHLC3-982xI7vHu9XYrIZFPvBbzcgucKurOQK88jxqbl9f-0utnZxzugwlfoXH3dVdSuv7qyYC-voJ4ZHmyBkg6F-nIZILjiuNT7AQOGq5yxQNLVPGIillPNgT8xoehQdB');
        // prep the bundle
        $msg = array
        (
            'message'   => 'First android message.',
            'title'     => 'JTSBoard',
            'vibrate'   => 1,
            'sound'     => 1
        );
       ...
       // Refresh the token if it's expired.
        ...
        foreach ($messages as $message) {
           ...
           $reservation_start_date_time =  $reservation_date.' '.$reservation_start_time;
            ...
            $current_date = date('Y-m-d');
            $reservationData = $this->Reservation->find('first', array('conditions'=> array( 'Reservation.reservation_number'=>$reservation_number,  'Reservation.status !='=>'5'), 'fields' =>array('Reservation.id', 'Reservation.reservation_number')));
    ...
    function notitifation_status($test_data = null){
        ...
        $this->loadModel('PushNotification');
        $this->loadModel('NotificationRead');
        $push_notification_id = isset($decoded['push_notification_id']) ? $decoded['push_notification_id'] : '';
        $employee_id = isset($decoded['employee_id']) ? $decoded['employee_id'] : '';
        if(!empty($push_notification_id)){
            $notificationReadData = $this->NotificationRead->find('first',array('conditions'=>array('NotificationRead.push_notification_id'=>$push_notification_id, 'NotificationRead.employee_id'=>$employee_id)));
            // pr($notificationReadData);
            if(isset($notificationReadData['NotificationRead']['id']) && !empty($notificationReadData['NotificationRead']['id'])){
                 $this->NotificationRead->id = $notificationReadData['NotificationRead']['id'];
                 if($this->NotificationRead->saveField("status","1")){
                    $responseArr = array('status' => 'success', 'msg' => '通知ステータス正常に更新されました。', 'msg1' => 'Notification Status Updated successfully.' );
                    $jsonEncode = json_encode($responseArr);
                }else{
                    $responseArr = array('status' => 'error', 'msg' => '通知ステータスエラー', 'msg1' => 'Notification Status error.'  );
                    $jsonEncode = json_encode($responseArr);
                }
            }else{
                // die('else');
                $pushNotificationStatusData['NotificationRead']['employee_id'] =  $employee_id;
                $pushNotificationStatusData['NotificationRead']['push_notification_id'] =  $push_notification_id;
                $pushNotificationStatusData['NotificationRead']['status'] =  '1';
                // pr($pushNotificationStatusData);die;
                if($this->NotificationRead->saveAll($pushNotificationStatusData)){
                    $responseArr = array('status' => 'success', 'msg' => '通知ステータス正常に更新されました。', 'msg1' => 'Notification Status Updated successfully.' );
                    $jsonEncode = json_encode($responseArr);
                }else{
                    $responseArr = array('status' => 'error', 'msg' => '通知ステータスエラー', 'msg1' => 'Notification Status error.'  );
        ...
        $log = $this->NotificationRead->getDataSource()->getLog(false, false);
        $recordData['RecordData']['name'] = "notitifation_status";
        $recordData['RecordData']['query'] = json_encode($log);
        $this->RecordData->saveAll($recordData);
        echo  $jsonEncode;exit();
    ...
    function reservation_status($test_data = null){
        ...
        $recordData['RecordData']['name'] = "reservation_status";
        $recordData['RecordData']['query'] = json_encode($data);
        $this->RecordData->saveAll($recordData);
        $decoded = json_decode($data, true); 
        $this->loadModel('ReservationRead');
        ...
        if(!empty($user_id) && !empty($date) && !empty($employee_id)){
            ...
            if(isset($reservationReadData['ReservationRead']['id']) && !empty($reservationReadData['ReservationRead']['id'])){
                 $this->ReservationRead->id = $reservationReadData['ReservationRead']['id'];
                 if($this->ReservationRead->saveField("status","1")){
                    $responseArr = array('status' => 'success', 'msg' => '予約状況更新に成功しました', 'msg1' => 'Reservation Status Updated successfully.' );
                    $jsonEncode = json_encode($responseArr);
                }else{
                    $responseArr = array('status' => 'error', 'msg' => '予約状況エラー。', 'msg1' => 'Reservation Status error.'  );
                    $jsonEncode = json_encode($responseArr);
                }
            }else{
                $reservationStatusData['ReservationRead']['user_id'] =  $user_id;
                ...
                $reservationStatusData['ReservationRead']['date'] =  $date;
                $reservationStatusData['ReservationRead']['status'] =  '1';
                // pr($reservationStatusData);die;
                if($this->ReservationRead->saveAll($reservationStatusData)){
                    $responseArr = array('status' => 'success', 'msg' => '予約状況更新に成功しました', 'msg1' => 'Reservation Status Updated successfully.' );
                    $jsonEncode = json_encode($responseArr);
                }else{
                    $responseArr = array('status' => 'error', 'msg' => '予約状況エラー。', 'msg1' => 'Reservation Status error.'  );
            ...
            $responseArr = array('status' => 'error', 'msg' => '予約がありません', 'msg1' => '要約がありません'  );
            $jsonEncode = json_encode($responseArr);
        }
        // $log = $this->ReservationRead->getDataSource()->getLog(false, false);
        // $recordData['RecordData']['name'] = "reservation_status";
        // $recordData['RecordData']['query'] = json_encode($log);
        // $this->RecordData->saveAll($recordData);
        echo  $jsonEncode;exit();
    ...
    function reservation_status_read($test_data = null){
        ...
        $user_id = isset($decoded['user_id']) ? $decoded['user_id'] : '';
        $employee_id = isset($decoded['employee_id']) ? $decoded['employee_id'] : '';
        $reservation_id = isset($decoded['reservation_id']) ? $decoded['reservation_id'] : '';
        if(!empty($user_id) && !empty($reservation_id) && !empty($employee_id)){
            $reservationStatusReadData = $this->ReservationStatusRead->find('first',array('conditions'=>array('ReservationStatusRead.reservation_id'=>$reservation_id, 'ReservationStatusRead.employee_id'=>$employee_id, 'ReservationStatusRead.user_id'=>$user_id)));
            // pr($notificationReadData);
            if(isset($reservationStatusReadData['ReservationStatusRead']['id']) && !empty($reservationStatusReadData['ReservationStatusRead']['id'])){
                 $this->ReservationStatusRead->id = $reservationStatusReadData['ReservationStatusRead']['id'];
                 if($this->ReservationStatusRead->saveField("status","1")){
                    $responseArr = array('status' => 'success', 'msg' => '予約状況更新に成功しました', 'msg1' => 'Reservation Status Updated successfully.' );
                    $jsonEncode = json_encode($responseArr);
                }else{
                    $responseArr = array('status' => 'error', 'msg' => '予約状況エラー。', 'msg1' => 'Reservation Status error.'  );
                ...
                $reservationStatusData['ReservationStatusRead']['user_id'] =  $user_id;
                $reservationStatusData['ReservationStatusRead']['employee_id'] =  $employee_id;
                $reservationStatusData['ReservationStatusRead']['reservation_id'] =  $reservation_id;
                $reservationStatusData['ReservationStatusRead']['status'] =  '1';
                // pr($reservationStatusData);die;
                if($this->ReservationStatusRead->saveAll($reservationStatusData)){
                    $responseArr = array('status' => 'success', 'msg' => '予約状況更新に成功しました', 'msg1' => 'Reservation Status Updated successfully.' );
                    $jsonEncode = json_encode($responseArr);
                }else{
                    $responseArr = array('status' => 'error', 'msg' => '予約状況エラー。', 'msg1' => 'Reservation Status error.'  );
        ...
        $log = $this->ReservationStatusRead->getDataSource()->getLog(false, false);
        $recordData['RecordData']['name'] = "reservation_status";
        $recordData['RecordData']['query'] = json_encode($log);
        $this->RecordData->saveAll($recordData);
        echo  $jsonEncode;exit();
    ...
    public function IOSNotification() {
        ...
        $deviceToken = 'be985f7949f59236e8ffeadac7de87a5d86279680ad563d2a479ee8a43e70927';
        $ctx = stream_context_create();
        $passphrase = '123456';
        // ck.pem is your certificate file
         $base_path = ROOT.DS.APP_DIR.DS.WEBROOT_DIR;
...
/****************************************************************************************************************************************
   ...
   function service_from_info($testData = null){
        ...
        $this->loadModel("ServiceDetail");
        $user_id = isset($decoded['user_id']) ? $decoded['user_id'] : '';
        $note_service_id = isset($decoded['note_service_id']) ? $decoded['note_service_id'] : '';
        if(!empty($user_id) && !empty($note_service_id) ){
            $data = $this->ServiceDetail->find('all',array('conditions'=>  array( 'ServiceDetail.user_id'=>$user_id ,'ServiceDetail.note_service_id'=>$note_service_id )));
            if(!$data){
                $jsonEncode =  json_encode(array('status'=>'error', 'msg'=> 'Customer does not exist.'));
            }else{
                $allData['Service']['id'] =  $data[0]['ServiceDetail']['id'];
        ...
        $log = $this->ServiceDetail->getDataSource()->getLog(false, false);
        $recordData['RecordData']['name'] = "service_from_info";
        $recordData['RecordData']['query'] = json_encode($log);
        $this->RecordData->saveAll($recordData);
        echo  $jsonEncode;exit();
    ...
    function add_salon_service(){
        ...
        $serviceColor = $responseArr = array();
        
        /* Request log Start*/
        $recordData['RecordData']['name'] = "add_salon_service";
        $recordData['RecordData']['query'] = json_encode($data);
        $this->RecordData->saveAll($recordData);
        /*Request log End*/
        ...
        $decoded = json_decode($data, true);
         // pr($decoded);die;
        $user_id =  isset($decoded['user_id']) ? $decoded['user_id'] : '';
        $default_service =  isset($decoded['service']['default_service']) ? $decoded['service']['default_service'] : '';
        $other_service =  isset($decoded['service']['other_service']) ? $decoded['service']['other_service'] : '';

        $existAllServiceData = $this->Service->find('all',array('conditions'=> array( 'Service.user_id'=>$user_id, 'Service.status'=>1)));
        $removeOldServiceIds =  $oldSeletedServiceIds = $oldServiceIds = array();
        if(!empty($existAllServiceData)){
            foreach ($existAllServiceData as $oldServiceKey => $oldServiceValue) {
                array_push($oldServiceIds, $oldServiceValue['Service']['id']);
            }
        }

        if(!empty($default_service)){
            foreach ($default_service as $newServiceKey => $newServiceValue) {
                if(!empty($newServiceValue) && !in_array($newServiceValue, $oldServiceIds)){
                    $newMainService = $this->MainService->find('first',array('conditions'=> array( 'MainService.id'=>$newServiceValue)));
                    if(!empty($newMainService)){
                            $colorData = array();
                            $service['Service']['user_id'] = $user_id;
                            $color_id = (isset($newMainService['MainService']['color_id']) && !empty($newMainService['MainService']['color_id'])) ? $newMainService['MainService']['color_id'] : '';
                            $colorData = $this->Color->find('first',array('conditions'=> array( 'Color.id'=>$color_id)));
                            $service_name = $service['Service']['name'] = (isset($newMainService['MainService']['name']) && !empty($newMainService['MainService']['name'])) ? $newMainService['MainService']['name'] : '';
                            $service['Service']['color_code'] = isset($newMainService['MainService']['color_code']) ? $newMainService['MainService']['color_code'] : '';
                            $service['Service']['status'] =Configure::read('App.Status.active');
                            $oldExistService = $this->Service->find('first',array('conditions'=> array( 'Service.name'=>$service_name, 'Service.user_id'=>$user_id)));
                            if(!isset($oldExistService['Service']['id'])){
                                if($this->Service->saveAll($service)){
                                    $service_id = $this->Service->id;
                                    $serviceColor['ServiceColor']['user_id'] = $user_id;
                                    $serviceColor['ServiceColor']['service_id'] = $service_id;
                                ...
                                $this->Service->id = $oldExistService['Service']['id'];
                                $this->Service->saveField('status', 1);
                            }
                            
                        }else{
                            $this->Service->id = $newServiceValue;
                            $this->Service->saveField('status', 1);
                        }
                }else{
                    if(!empty($newServiceValue)){
                        array_push($oldSeletedServiceIds, $newServiceValue);
                    }
                    
                }
            }
            foreach ($oldServiceIds as $oldSeletedServiceKey => $oldSeletedServiceValue) {
                if(!in_array($oldSeletedServiceValue, $oldSeletedServiceIds)){
                     array_push($removeOldServiceIds, $oldSeletedServiceValue);
                }
            }
            if(isset($removeOldServiceIds[0]) && !empty($removeOldServiceIds[0])){
                
                foreach ($removeOldServiceIds as $removeOldServiceKey => $removeOldServiceValue) {
                    $this->Service->id = $removeOldServiceValue;
                    $this->Service->saveField('status', 0);
                }
            }
            ...
            if(!empty($other_service)){
                foreach ($other_service as $otherServiceKey => $otherServiceValue) {
                    if(!empty($otherServiceValue)){
                        $existService = $this->Service->find('first',array('conditions'=> array( 'Service.name'=>$otherServiceValue, 'Service.user_id'=>$user_id)));
                        if(isset($existService['Service']['id']) && !empty($existService['Service']['id'])){
                            $this->Service->id = $existService['Service']['id'];
                            $this->Service->saveField('status', 1);
                            ...
                            $color_id = '2';
                            $colorData = $this->Color->find('first',array('conditions'=> array( 'Color.id'=>$color_id)));
                            $service_name = $service['Service']['name'] = $otherServiceValue;
                            $service['Service']['color_code'] = isset($colorData['Color']['color_code']) ? $colorData['Color']['color_code'] : '';
                            $service['Service']['status'] =Configure::read('App.Status.active');
                            if($this->Service->saveAll($service)){
                                $service_id = $this->Service->id;
                                $serviceColor['ServiceColor']['user_id'] = $user_id;
        ...
        $responseArr = array('user_id' => $user_id, 'status' => 'success', 'msg' => 'サービスの色を正常に追加。', 'msg1' => 'Services added successfully.');
        $jsonEncode = json_encode($responseArr);
...
/****************************************************************************************************************************************
   ...
   function get_salon_profile($testData = null){
        ...
        /* Request log Start*/
        $recordData['RecordData']['name'] = "get_salon_profile";
        $recordData['RecordData']['query'] = json_encode($data);
        $this->RecordData->saveAll($recordData);
        /*Request log End*/
        ...
        if(!empty($user_id)){
            ...
            $this->User->bindModel(array('hasOne' => array('StripeTransactionDetails', 'StripePlanDetails', 'StripeSubscriptionDetails','PlanCancelRequest')));
            $payment_data = $this->User->find('first',array('conditions'=>  array('User.id'=>$user_id)));
            $count_cancel = $this->PlanCancelRequest->find('count',array('conditions'=>  array('PlanCancelRequest.user_id'=>$user_id)));
            // pr($count_cancel);
            // pr($payment_data);die;

            if(!$data){
                $jsonEncode =  json_encode(array('user_id'=>$user_id, 'status'=>'error', 'msg'=> 'User does not exist.'));
            }else{
                // pr($data);die;

                /** User Details **/
                $allData['User']['id'] =  $data['User']['id'];
                ...
                $allData['User']['zip_code'] =  isset($data['User']['zip_code'])?$data['User']['zip_code']:'';
                $allData['User']['city'] =  isset($data['User']['city'])?$data['User']['city']:'';
                $allData['User']['address1'] =  isset($data['User']['address1'])?$data['User']['address1']:'';
                $allData['User']['address2'] =  isset($data['User']['address2'])?$data['User']['address2']:'';
                $allData['User']['employee_number'] =  isset($data['User']['employee_number'])?$data['User']['employee_number']:'';
                $allData['User']['advertisement'] =  isset($data['User']['advertisement'])?$data['User']['advertisement']:'';
                $allData['User']['avr_customer'] =  isset($data['User']['avr_customer'])?$data['User']['avr_customer']:'';
                $allData['User']['employee_pin_number'] =  isset($data['User']['employee_pin_number'])?$data['User']['employee_pin_number']:'';
                $allData['User']['customer_pin_number'] =  isset($data['User']['customer_pin_number'])?$data['User']['customer_pin_number']:'';
                $allData['User']['month_start_date'] =  isset($data['User']['month_start_date'])?$data['User']['month_start_date']:'';
                $allData['User']['weekend'] =  isset($data['User']['weekend'])?$data['User']['weekend']:'';
                $allData['User']['over_time'] =  isset($data['User']['over_time'])?$data['User']['over_time']:'';
                $allData['User']['start_time'] =  isset($data['User']['start_time'])?$data['User']['start_time']:'';
                $allData['User']['end_time'] =  isset($data['User']['end_time'])?$data['User']['end_time']:'';
                $allData['User']['salon_board_status'] =  isset($data['User']['salon_board_status'])?$data['User']['salon_board_status']:'';
                $allData['User']['sb_username'] =  isset($data['User']['sb_username'])?$data['User']['sb_username']:'';
                $allData['User']['sb_password'] =  isset($data['User']['sb_password'])?$data['User']['sb_password']:'';
                $allData['User']['stripe_payment_status'] =  isset($data['User']['stripe_payment_status'])?$data['User']['stripe_payment_status']:'';
                $allData['User']['subscription_current_period_end'] =  isset($data['User']['subscription_current_period_end'])?$data['User']['subscription_current_period_end']:'';
                $allData['User']['stripe_plan_status'] =  isset($data['User']['stripe_plan_status'])?$data['User']['stripe_plan_status']:'';

                /** Payment Details **/
                $allData['Payment_info']['plan_nickname'] =  isset($payment_data['StripePlanDetails']['plan_nickname'])?$payment_data['StripePlanDetails']['plan_nickname']:'';
                $allData['Payment_info']['customer_id'] =  isset($payment_data['StripeSubscriptionDetails']['customer_id'])?$payment_data['StripeSubscriptionDetails']['customer_id']:'';
                $allData['Payment_info']['transaction_id'] =  isset($payment_data['StripeTransactionDetails']['transaction_id'])?$payment_data['StripeTransactionDetails']['transaction_id']:'';
                $allData['Payment_info']['transaction_amount'] =  isset($payment_data['StripeTransactionDetails']['transaction_amount'])?$payment_data['StripeTransactionDetails']['transaction_amount']:'';
                $allData['Payment_info']['transaction_created'] =  isset($payment_data['StripeTransactionDetails']['transaction_created'])?$payment_data['StripeTransactionDetails']['transaction_created']:'';
                $allData['Payment_info']['transaction_status'] =  isset($payment_data['StripeTransactionDetails']['transaction_status'])?$payment_data['StripeTransactionDetails']['transaction_status']:'';
                $allData['Payment_info']['subscription_current_period_end'] =  isset($payment_data['StripeSubscriptionDetails']['subscription_current_period_end'])?$payment_data['StripeSubscriptionDetails']['subscription_current_period_end']:'';
                $allData['Payment_info']['stripe_plan_status'] =  isset($payment_data['User']['stripe_plan_status'])?$payment_data['User']['stripe_plan_status']:'';
                $allData['Payment_info']['PlanCancelRequest'] =  (isset($count_cancel) && ($count_cancel == '1'))?'1':'0';


                $jsonEncode =  json_encode($allData);
            }
        }else{
            $userData['User']['msg'] = 'お客様は存在しません。';
            $userData['User']['msg1'] = 'Please add User id.';
            $userData['User']['status'] = 'error';
    ...
    /****************************************************************************************************************************************
     * NAME: get_salon_payment_details
     * Description: Get Salon Profile
     *
     * Subroutines Called:
     *
   ...
   function get_salon_payment_details($testData = null){
        ...
        if(!empty($user_id)){
            ...
            /** Get User Payment Data **/
            $this->User->bindModel(array('hasOne' => array('StripeCardDetails', 'StripeTransactionDetails', 'StripePlanDetails', 'StripeSubscriptionDetails','PlanCancelRequest')));
            $payment_data = $this->User->find('first',array('conditions'=>  array('User.id'=>$user_id)));
            $count_cancel = $this->PlanCancelRequest->find('count',array('conditions'=>  array('PlanCancelRequest.user_id'=>$user_id)));
            // pr($count_cancel);
            // pr($payment_data);die;
            ...
            if(!$data){
                ...
                if(isset($payment_data['StripeCardDetails']['card_last4_digit']) && !empty($payment_data['StripeCardDetails']['card_last4_digit'])){
                    $subscription_created = $payment_data['StripeSubscriptionDetails']['subscription_created'];
                    $card_brand = $payment_data['StripeCardDetails']['card_brand'];
                    $card_last4_digit = $payment_data['StripeCardDetails']['card_last4_digit'];
                    $y = date('Y', strtotime($subscription_created));
                    $m = date('m', strtotime($subscription_created));
                    $d = date('d', strtotime($subscription_created));
                    $allData['Payment_info']['scheduled_payment_detail'] =  "次のお支払い予定日は".$y."年".$m."月".$d."日です。".$card_brand." ****".$card_last4_digit;
                }else{
                      $allData['Payment_info']['scheduled_payment_detail'] =  "無料トライアル　アプリ";

                }
                $cy = date('Y');
                $allData['Payment_info']['plan_title'] =  "JTSボード".$cy."プラン";
                ...
                $allData['Payment_info']['transaction_created'] =  isset($payment_data['StripeTransactionDetails']['transaction_created'])?$payment_data['StripeTransactionDetails']['transaction_created']:'';
                $allData['Payment_info']['transaction_status'] =  isset($payment_data['StripeTransactionDetails']['transaction_status'])?$payment_data['StripeTransactionDetails']['transaction_status']:'';
                $allData['Payment_info']['subscription_current_period_end'] =  isset($payment_data['StripeSubscriptionDetails']['subscription_current_period_end'])? date('Y年m月d日',strtotime($payment_data['StripeSubscriptionDetails']['subscription_current_period_end'])):'';
                $allData['Payment_info']['stripe_plan_status'] =  isset($payment_data['User']['stripe_plan_status'])?$payment_data['User']['stripe_plan_status']:'';
                $allData['Payment_info']['PlanCancelRequest'] =  (isset($count_cancel) && ($count_cancel == '1'))?'1':'0';
   ...
   function get_shop_profile($testData = null){
        ...
        /* Request log Start*/
        $recordData['RecordData']['name'] = "get_shop_profile";
        $recordData['RecordData']['query'] = json_encode($data);
        $this->RecordData->saveAll($recordData);
        /*Request log End*/
        ...
        if(!empty($user_id)){
            ...
            if(!$data){
                ...
                $allData['User']['address1'] =  isset($data['User']['address1'])?$data['User']['address1']:'';
                $allData['User']['address2'] =  isset($data['User']['address2'])?$data['User']['address2']:'';
                $allData['User']['prefecture'] =  isset($data['User']['prefecture'])?$data['User']['prefecture']:'';
                $allData['User']['employee_number'] =  isset($data['User']['employee_number'])?$data['User']['employee_number']:'';
                $allData['User']['advertisement'] =  isset($data['User']['advertisement'])?$data['User']['advertisement']:'';
                $allData['User']['avr_customer'] =  isset($data['User']['avr_customer'])?$data['User']['avr_customer']:'';
                $allData['User']['employee_pin_number'] =  isset($data['User']['employee_pin_number'])?$data['User']['employee_pin_number']:'';
                $allData['User']['customer_pin_number'] =  isset($data['User']['customer_pin_number'])?$data['User']['customer_pin_number']:'';
                $allData['User']['month_end_date'] =  isset($data['User']['month_end_date'])?$data['User']['month_end_date']:'';
                $allData['User']['weekend'] =  isset($data['User']['weekend'])?$data['User']['weekend']:'';
                $allData['User']['over_time'] =  isset($data['User']['over_time'])?$data['User']['over_time']:'';
                $allData['User']['start_time'] =  isset($data['User']['start_time'])?$data['User']['start_time']:'';
                $allData['User']['end_time'] =  isset($data['User']['end_time'])?$data['User']['end_time']:'';
                ...
                $salonWorkingDaysData = $this->UserWorkingDay->find('all', array('conditions'=>array( 'UserWorkingDay.user_id'=>$user_id)));
                $allData['UserWorkingDay'] = array();
                if(!empty($salonWorkingDaysData)){
                    $i =  0;

                    foreach ($salonWorkingDaysData as $sectionKey => $sectionValue) {
                       $allData['UserWorkingDay'][$i]['user_id']  = $user_id;
                       $allData['UserWorkingDay'][$i]['day']  = $sectionValue['UserWorkingDay']['day'];
                       $allData['UserWorkingDay'][$i]['start_time']  = $sectionValue['UserWorkingDay']['start_time'];
                       $allData['UserWorkingDay'][$i]['end_time']  = $sectionValue['UserWorkingDay']['end_time'];
                       $allData['UserWorkingDay'][$i]['step_type']  = $sectionValue['UserWorkingDay']['step_type'];
                       $i++;
                    }
                }
                $jsonEncode =  json_encode($allData);
    ...
    /**************************************************************************
     * NAME: edit_shop_profile
     * Description: Edit Salon Profile.
     *
     * Subroutines Called:
     *
     * Parameters: Input / Output
    ...
    function edit_shop_profile($testData = null){
        ...
        /* Request log Start*/
        $recordData['RecordData']['name'] = "edit_shop_profile";
        $recordData['RecordData']['query'] = json_encode($data);
        $this->RecordData->saveAll($recordData);
        /*Request log End*/
        ...
        if(isset($decoded['email']) && !empty($decoded['email'])){
            ...
            if($userExist){
                ...
                $userData = $this->User->find('first', array('conditions'=>array('User.id '=>$user_id)));
                $activation_url = 'http://api.jtsboard.com/users/email_confirm/'.$user_id.'/'.$verification_code;
                $activation_link    =   '<a href="'.$activation_url.'" style="background-color: #debd4e; padding: 15px 30px; display: inline-block; color: #fff; text-decoration: none; border-radius: 5px;">今すぐ登録する</a>';
                $mail_message = str_replace(array('{NAME}', '{EMAIL}','{ACTIVATION_LINK}', '{activation_code}',  '{SITE}'), array($userData['User']['salon_name'], $email, $activation_link, $verification_code, Configure::read('App.SITENAME')), $registrationMail['Template']['content']);
                $template = 'default';
                $this->set('message', $mail_message);
        ...
        if(isset($decoded['salon_name']) && !empty($decoded['salon_name'])){
            $salon_name = (isset($decoded['salon_name']) && !empty($decoded['salon_name'])) ? $decoded['salon_name'] : '';
            $this->User->saveField('salon_name' , $salon_name );
        }
        if(isset($decoded['website']) && !empty($decoded['website'])){
            $website = (isset($decoded['website']) && !empty($decoded['website'])) ? $decoded['website'] : '';
            $this->User->saveField('website' , $website );
        }
        if(isset($decoded['tel']) && !empty($decoded['tel'])){
            $tel = (isset($decoded['tel']) && !empty($decoded['tel'])) ? $decoded['tel'] : '';
            $this->User->saveField('tel' , $tel );
        }
        if(isset($decoded['employee_pin_number']) && !empty($decoded['employee_pin_number'])){
            $employee_pin_number = (isset($decoded['employee_pin_number']) && !empty($decoded['employee_pin_number'])) ? $decoded['employee_pin_number'] : '';
            $this->User->saveField('employee_pin_number' , $employee_pin_number );
        }

        if(isset($decoded['customer_pin_number']) && !empty($decoded['customer_pin_number'])){
            $customer_pin_number = (isset($decoded['customer_pin_number']) && !empty($decoded['customer_pin_number'])) ? $decoded['customer_pin_number'] : '';
            $this->User->saveField('customer_pin_number' , $customer_pin_number );
        }
        ...
        if(isset($decoded['over_time']) && !empty($decoded['over_time'])){
            $over_time = (isset($decoded['over_time']) && !empty($decoded['over_time'])) ? $decoded['over_time'] : '';
            $this->User->saveField('over_time' , $over_time );
        }
        if(isset($decoded['other_tel']) && !empty($decoded['other_tel'])){
            $other_tel = (isset($decoded['other_tel']) && !empty($decoded['other_tel'])) ? $decoded['other_tel'] : '';
            $this->User->saveField('other_tel' , $other_tel );
        }
        if(isset($decoded['month_end_date']) && !empty($decoded['month_end_date'])){
            $month_end_date = (isset($decoded['month_end_date']) && !empty($decoded['month_end_date'])) ? $decoded['month_end_date'] : '';
            $this->User->saveField('month_end_date' , $month_end_date );
        }
        if(isset($decoded['advertisement']) && !empty($decoded['advertisement'])){
            $advertisement = (isset($decoded['advertisement']) && !empty($decoded['advertisement'])) ? $decoded['advertisement'] : '';
            $this->User->saveField('advertisement' , $advertisement );
        }
         if(isset($decoded['avr_customer']) && !empty($decoded['avr_customer'])){
            $avr_customer = (isset($decoded['avr_customer']) && !empty($decoded['avr_customer'])) ? $decoded['avr_customer'] : '';
            $this->User->saveField('avr_customer' , $avr_customer );
        }

        if(isset($decoded['salon_board_status']) && isset($decoded['sb_username']) && isset($decoded['sb_password']) && ($decoded['salon_board_status'] == 1)){
            $salon_board_status = (isset($decoded['salon_board_status']) && !empty($decoded['salon_board_status'])) ? $decoded['salon_board_status'] : '0';
            $sb_username = (isset($decoded['sb_username']) && !empty($decoded['sb_username'])) ? $decoded['sb_username'] : '';
            $sb_password = (isset($decoded['sb_password']) && !empty($decoded['sb_password'])) ? $decoded['sb_password'] : '';
            $this->User->saveField('salon_board_status' , $salon_board_status );
            $this->User->saveField('sb_username' , $sb_username );
            $this->User->saveField('sb_password' , $sb_password );
        }

        $responseArr = array('user_id' => $user_id, 'status' => 'success', 'msg' => 'ユーザープロフィールは正常に編集されます。', 'msg1' => 'Shop profile edit successfully.');
        $jsonEncode = json_encode($responseArr);
        echo  $jsonEncode;exit();
    }
...
/**************************************************************************
    ...
    function edit_shop_address($testData = null){
        ...
        /* Request log Start*/
        $recordData['RecordData']['name'] = "edit_shop_address";
        $recordData['RecordData']['query'] = json_encode($data);
        $this->RecordData->saveAll($recordData);
        /*Request log End*/
        ...
        $decoded = json_decode($data, true);
        $user['User']['id'] = isset($decoded['user_id']) ? $decoded['user_id'] : '';
        $user['User']['zip_code'] = (isset($decoded['zip_code']) && !empty($decoded['zip_code'])) ? $decoded['zip_code'] : '';
        $user['User']['city'] = (isset($decoded['city']) && !empty($decoded['city'])) ? $decoded['city'] : '';
        $user['User']['prefecture'] = (isset($decoded['prefecture']) && !empty($decoded['prefecture'])) ? $decoded['prefecture'] : '';
        $user['User']['address1'] = (isset($decoded['address1']) && !empty($decoded['address1'])) ? $decoded['address1'] : '';
        $user['User']['address2'] = (isset($decoded['address2']) && !empty($decoded['address2'])) ? $decoded['address2'] : '';
        // pr($user);die;

       if($this->User->saveAll($user)){
            $user_id = $this->User->id;
            $responseArr = array('user_id' => $user_id, 'status' => 'success', 'msg' => 'ショップアドレスの編集に成功しました。', 'msg1' => 'Shop address edit successfully.');
            $jsonEncode = json_encode($responseArr);
            
        }else{
            $user_id = $this->User->id;
            $responseArr = array('user_id' => $user_id, 'status' => 'error');
            $jsonEncode = json_encode($responseArr);
        }
        echo  $jsonEncode;exit();
    }
    ...
    function salon_working_days_time($testData = null){
        ...
        /* Request log Start*/
        $recordData['RecordData']['name'] = "salon_working_days_time";
        $recordData['RecordData']['query'] = json_encode($data);
        $this->RecordData->saveAll($recordData);
        /*Request log End*/
        ...
        // pr($decoded);die;
        $user_id = isset($decoded['user_id']) ? $decoded['user_id'] : '';
        $days_section = (isset($decoded['days_section']) && !empty($decoded['days_section'])) ? $decoded['days_section'] : '';
        
        $userWorkingDayData = array();
        $i =  0;
        if(!empty($days_section)){
            $conditions = array('UserWorkingDay.user_id' =>$user_id);
            $this->UserWorkingDay->deleteAll($conditions);
            foreach ($days_section as $sectionKey => $sectionValue) {
            
                   $userWorkingDayData[$i]['UserWorkingDay']['user_id']  = $user_id;
                   $userWorkingDayData[$i]['UserWorkingDay']['day']  = $sectionValue['day'];
                   $userWorkingDayData[$i]['UserWorkingDay']['start_time']  = $sectionValue['start_time'];
                   $userWorkingDayData[$i]['UserWorkingDay']['end_time']  = $sectionValue['end_time'];
                   $userWorkingDayData[$i]['UserWorkingDay']['step_type']  = $sectionKey;
                   $i ++;
            }    
        }
        
        if($this->UserWorkingDay->saveAll($userWorkingDayData)){
            $responseArr = array( 'status' => 'success', 'msg' => 'サロンのプロフィール編集に成功しました。', 'msg1' => 'Salon working days edit successfully.');
            $jsonEncode = json_encode($responseArr);
        }else{
            $user_id = $this->User->id;
            $responseArr = array('user_id' => $user_id, 'status' => 'error');
...
/**************************************************************************
     * NAME: get_salon_working_days_time
     * Description: Edit Salon Profile.
     *
     * Subroutines Called:
     *
    ...
    function get_salon_working_days_time($testData = null){
        ...
        /* Request log Start*/
        $recordData['RecordData']['name'] = "get_salon_working_days_time";
        $recordData['RecordData']['query'] = json_encode($data);
        $this->RecordData->saveAll($recordData);
        /*Request log End*/
        ...
        $user_id = isset($decoded['user_id']) ? $decoded['user_id'] : '';
        $salonWorkingDaysData = $this->UserWorkingDay->find('all', array('conditions'=>array( 'UserWorkingDay.user_id'=>$user_id)));
        $userWorkingDayData['UserWorkingDay']= array();
        $i =  0;
        if(!empty($salonWorkingDaysData)){
           
            foreach ($salonWorkingDaysData as $sectionKey => $sectionValue) {
               $userWorkingDayData['UserWorkingDay'][$i]['user_id']  = $user_id;
               $userWorkingDayData['UserWorkingDay'][$i]['day']  = $sectionValue['UserWorkingDay']['day'];
               $userWorkingDayData['UserWorkingDay'][$i]['start_time']  = $sectionValue['UserWorkingDay']['start_time'];
               $userWorkingDayData['UserWorkingDay'][$i]['end_time']  = $sectionValue['UserWorkingDay']['end_time'];
               $userWorkingDayData['UserWorkingDay'][$i]['step_type']  = $sectionValue['UserWorkingDay']['step_type'];
               $i++;
            }
        }    
       
       $jsonEncode = json_encode($userWorkingDayData);
        echo  $jsonEncode;exit();
    ...
    function edit_salon_profile($testData = null){
        ...
        /* Request log Start*/
        $recordData['RecordData']['name'] = "edit_salon_profile";
        $recordData['RecordData']['query'] = json_encode($data);
        $this->RecordData->saveAll($recordData);
        /*Request log End*/
        ...
        $user['User']['id'] = isset($decoded['user_id']) ? $decoded['user_id'] : '';
        $user['User']['name'] = (isset($decoded['name']) && !empty($decoded['name'])) ? $decoded['name'] : '';
        $user['User']['email'] = (isset($decoded['email']) && !empty($decoded['email'])) ? $decoded['email'] : '';
        $user['User']['company_name'] = (isset($decoded['company_name']) && !empty($decoded['company_name'])) ? $decoded['company_name'] : '';
        $user['User']['salon_name'] = (isset($decoded['salon_name']) && !empty($decoded['salon_name'])) ? $decoded['salon_name'] : '';
        $user['User']['website'] = (isset($decoded['website']) && !empty($decoded['website'])) ? $decoded['website'] : '';
        $user['User']['tel'] = (isset($decoded['tel']) && !empty($decoded['tel'])) ? $decoded['tel'] : '';
        $user['User']['zip_code'] = (isset($decoded['zip_code']) && !empty($decoded['zip_code'])) ? $decoded['zip_code'] : '';
        $user['User']['city'] = (isset($decoded['city']) && !empty($decoded['city'])) ? $decoded['city'] : '';
        $user['User']['address1'] = (isset($decoded['address1']) && !empty($decoded['address1'])) ? $decoded['address1'] : '';
        $user['User']['address2'] = (isset($decoded['address2']) && !empty($decoded['address2'])) ? $decoded['address2'] : '';
        ...
        if($this->User->saveAll($user)){
            $user_id = $this->User->id;
            $responseArr = array('user_id' => $user_id, 'status' => 'success', 'msg' => 'ユーザープロフィールは正常に編集されます。', 'msg1' => 'Salon profile edit successfully.');
            $jsonEncode = json_encode($responseArr);
    ...
    /**************************************************************************
     * NAME: edit_salon_info
     * Description: Edit Salon info.
     *
     * Subroutines Called:
     *
     * Parameters: Input / Output
    ...
    function edit_salon_info($testData = null){
        ...
        /* Request log Start*/
        $recordData['RecordData']['name'] = "edit_salon_info";
        $recordData['RecordData']['query'] = json_encode($data);
        $this->RecordData->saveAll($recordData);
        /*Request log End*/
        ...
        $user['User']['id'] = isset($decoded['user_id']) ? $decoded['user_id'] : '';
        $user['User']['employee_number'] = (isset($decoded['employee_number']) && !empty($decoded['employee_number'])) ? $decoded['employee_number'] : '';
        $user['User']['advertisement'] = (isset($decoded['advertisement']) && !empty($decoded['advertisement'])) ? $decoded['advertisement'] : '';
        $user['User']['avr_customer'] = (isset($decoded['avr_customer']) && !empty($decoded['avr_customer'])) ? $decoded['avr_customer'] : '';
        $user['User']['employee_pin_number'] = (isset($decoded['employee_pin_number']) && !empty($decoded['employee_pin_number'])) ? $decoded['employee_pin_number'] : '';
        $user['User']['customer_pin_number'] = (isset($decoded['customer_pin_number']) && !empty($decoded['customer_pin_number'])) ? $decoded['customer_pin_number'] : '';
        $user['User']['month_start_date'] = (isset($decoded['month_start_date']) && !empty($decoded['month_start_date'])) ? $decoded['month_start_date'] : '1';
        $user['User']['weekend'] = (isset($decoded['weekend']) && !empty($decoded['weekend'])) ? $decoded['weekend'] : '';
        $user['User']['over_time'] = (isset($decoded['over_time']) && !empty($decoded['over_time'])) ? $decoded['over_time'] : '0';
        $user['User']['start_time'] = (isset($decoded['start_time']) && !empty($decoded['start_time'])) ? $decoded['start_time'] : '00:00:00';
        $user['User']['end_time'] = (isset($decoded['end_time']) && !empty($decoded['end_time'])) ? $decoded['end_time'] : '00:00:00';
        $user['User']['sb_username'] = (isset($decoded['sb_username']) && !empty($decoded['sb_username'])) ? $decoded['sb_username'] : '';
        $user['User']['sb_password'] = (isset($decoded['sb_password']) && !empty($decoded['sb_password'])) ? $decoded['sb_password'] : '';
        
        if($this->User->saveAll($user)){
            $user_id = $this->User->id;
            $responseArr = array('user_id' => $user_id, 'status' => 'success', 'msg' => 'ユーザープロフィールは正常に編集されます。', 'msg1' => 'Salon information edit successfully.');
            $jsonEncode = json_encode($responseArr);
    ...
    function add_service(){
        ...
        $id =  
        $user_id =  $service['Service']['user_id'] = isset($decoded['user_id']) ? $decoded['user_id'] : '';
        $color_id = $service['Service']['color_id'] = (isset($decoded['color_id']) && !empty($decoded['color_id'])) ? $decoded['color_id'] : '';
        $colorData = $this->Color->find('first',array('conditions'=> array( 'Color.id'=>$color_id)));
        $service_name = $service['Service']['name'] = (isset($decoded['service_name']) && !empty($decoded['service_name'])) ? $decoded['service_name'] : '';
        $service['Service']['color_code'] = isset($colorData['Color']['color_code']) ? $colorData['Color']['color_code'] : '';
        $service['Service']['status'] =Configure::read('App.Status.active');
        if(isset($decoded['id']) && !empty($decoded['id'])){
            $service['Service']['id'] = isset($decoded['id']) ? $decoded['id'] : '';
            $ExistserviceData =  array();
        }else{
            $ExistserviceData = $this->Service->find('first',array('conditions'=> array( 'Service.user_id'=>$user_id, 'Service.name'=>$service_name)));
        }
        if(empty($ExistserviceData)){
            if($this->Service->saveAll($service)){
                $service_id = $this->Service->id;
                ...
                $responseArr = array('service_id' => $service_id, 'user_id' => $decoded['user_id'], 'status' => 'success', 'msg' => 'サービスの色を正常に追加。', 'msg1' => 'Service add successfully.');
                $jsonEncode = json_encode($responseArr);
                
            }else{

                $responseArr = array('status' => 'error', 'msg' => 'サービスが正常に追加されていません。', 'msg1' => 'Service is not added successfully.' );
                $jsonEncode = json_encode($responseArr);
            }
        }else{
            $responseArr = array('status' =>'error', 'msg' => 'サービスはすでに追加されています。', 'msg1' => 'Service is already added.');
            $jsonEncode = json_encode($responseArr);
        }    
    ...
    function delete_service($test_data = null){
        ...
        if(!empty($id)){
            if($this->Service->delete($id, true)){
                $ServiceColorData = $this->ServiceColor->find('first',array('conditions'=> array( 'ServiceColor.service_id'=>$id)));
                if(isset($ServiceColorData['ServiceColor']['id']) && !empty($ServiceColorData['ServiceColor']['id'])){
                    $this->ServiceColor->delete($ServiceColorData['ServiceColor']['id'], true);
                }
                $responseArr = array('status' => 'success', 'msg' => 'サービスは正常に削除されました。', 'msg1' => 'Sevice deleted successfully.' );
                $jsonEncode = json_encode($responseArr);
            }else{
                $responseArr = array('status' => 'error', 'msg' => 'サービス削除エラー'  , 'msg1' => 'Sevice deleted error.'  );
                $jsonEncode = json_encode($responseArr);
            }
        }else{
            $responseArr = array('status' => 'error', 'msg' => 'サービスが存在しません。' , 'msg1' => 'Sevice does not exist.'  );
            $jsonEncode = json_encode($responseArr);
        }
        echo  $jsonEncode;exit();
    } 
    ...
    function add_user_image($test_data = null){
        ...
        /* Request log Start*/
        $recordData['RecordData']['name'] = "add_user_image";
        $recordData['RecordData']['query'] = json_encode($data);
        $this->RecordData->saveAll($recordData);
        /*Request log End*/
        ...
        $user_id = (isset($decoded['user_id']) && !empty($decoded['user_id'])) ? $decoded['user_id'] : '';
        
       
        /********Start User Images add********/
        ...
        if(isset($_FILES['image']) && !empty($_FILES['image'])){
            ...
            if(in_array($path_info['extension'], array('jpg', 'jpeg', 'png', 'gif'))){
                ...
                $medium = $this->Upload->upload($_FILES['image'], WWW_ROOT . MY_SHOP_IMG_MEDIUM_DIR, $newName, $mediumRules);

                /* Original image upload */
                $res3 = $this->Upload->upload($_FILES['image'], WWW_ROOT . MY_SHOP_IMG_ORIGINAL_DIR, $newName, '', array('png', 'jpg', 'jpeg', 'gif'));

                /** Get user image from database **/
                $user_image = $this->User->find('first',array('conditions'=>  array('User.id'=>$user_id),'fields'=>array('User.image')));

                /** Save and unlink user image in database**/
                if(!empty($this->Upload->result)){
                    $this->User->id = $user_id;
                    $this->User->saveField('image' , $this->Upload->result);
                    if(isset($user_image['User']['image']) && !empty($user_image['User']['image'])){
                        (file_exists(WWW_ROOT . MY_SHOP_IMG_ORIGINAL_DIR.$user_image['User']['image'])?unlink(WWW_ROOT . MY_SHOP_IMG_ORIGINAL_DIR.$user_image['User']['image']):'');
                        (file_exists(WWW_ROOT . MY_SHOP_IMG_MEDIUM_DIR.$user_image['User']['image'])?unlink(WWW_ROOT . MY_SHOP_IMG_MEDIUM_DIR.$user_image['User']['image']):'');
                    }
                }
            ...
            $responseArr = array('user_id' => $user_id, 'status' => 'error', 'msg'=>'My shop image does not exist.');
            $jsonEncode = json_encode($responseArr);
        }
        echo  $jsonEncode;exit();
    }
    ...
    /**************************************************************************
     * NAME: add_cancel_plan
     * Description: Add cancel plan record.
     *
     * Subroutines Called:
     *
     * Parameters: Input / Output
    ...
    function add_cancel_plan($test_data = null){
        ...
        /* Request log Start*/
        $recordData['RecordData']['name'] = "add_cancel_plan";
        $recordData['RecordData']['query'] = json_encode($data);
        $this->RecordData->saveAll($recordData);
        /*Request log End*/
        ...
        $cancel_plan['PlanCancelRequest']['user_id'] = isset($decoded['user_id']) ? $decoded['user_id'] : '';
        $cancel_plan['PlanCancelRequest']['customer_id'] = (isset($decoded['customer_id']) && !empty($decoded['customer_id'])) ? $decoded['customer_id'] : '';
        $cancel_plan['PlanCancelRequest']['reason'] = isset($decoded['reason']) ? $decoded['reason'] : '';
        $cancel_plan['PlanCancelRequest']['status'] = Configure::read('App.Status.active');


        if($this->PlanCancelRequest->saveAll($cancel_plan)){

            $cancel_plan_id = $this->PlanCancelRequest->id;
            $responseArr = array('cancel_plan_id' => $cancel_plan_id, 'user_id' => $decoded['user_id'], 'status' => 'success', 'msg' => 'サービスの色を正常に追加。', 'msg1' => 'Cancel plan request add successfully.' );
            $jsonEncode = json_encode($responseArr);
        }else{
            $responseArr = array('status' => 'error' );
            $jsonEncode = json_encode($responseArr);
     ...
     /****************************************************************************************************************************************
     * NAME: add_employee_image
     * Description: Add Employee Image.
     *
     * Subroutines Called:
     *
     * Parameters: Input / Output
    ...
    function add_employee_image($test_data = null){
        ...
        /* Request log Start*/
        $recordData['RecordData']['name'] = "add_employee_image";
        $recordData['RecordData']['query'] = json_encode($data);
        $this->RecordData->saveAll($recordData);
        /*Request log End*/
        ...
        $user_id = (isset($decoded['user_id']) && !empty($decoded['user_id'])) ? $decoded['user_id'] : '';
        $employee_id = (isset($decoded['employee_id']) && !empty($decoded['employee_id'])) ? $decoded['employee_id'] : '';
        ...
        if(isset($_FILES['image']) && !empty($_FILES['image'])){
            ...
            if(in_array($path_info['extension'], array('jpg', 'jpeg', 'png', 'gif'))){
                ...
                /* Medium image upload */
                $mediumRules = array('size' => array(NOTE_MEDIUM_WIDTH, NOTE_MEDIUM_HEIGHT), 'type' => 'resizecrop');
                $medium = $this->Upload->upload($_FILES['image'], WWW_ROOT . NOTE_IMG_MEDIUM_DIR, $newName, $mediumRules);

                /* Thumb image upload */
                $thumbRules = array('size' => array(NOTE_THUMB_WIDTH, NOTE_THUMB_HEIGHT), 'type' => 'resizecrop');
                $thumb = $this->Upload->upload($_FILES['image'], WWW_ROOT . NOTE_IMG_THUMB_DIR, $newName, $thumbRules);

                /* Original image upload */
                $res3 = $this->Upload->upload($_FILES['image'], WWW_ROOT . NOTE_IMG_ORIGINAL_DIR, $newName, '', array('png', 'jpg', 'jpeg', 'gif'));

                /** Get user image from database **/
                $employee_image = $this->Employee->find('first',array('conditions'=>  array('Employee.id'=>$employee_id, 'Employee.user_id'=>$user_id),'fields'=>array('Employee.image')));

                /** Save and unlink user image in database**/
                if(!empty($this->Upload->result)){
                    $this->Employee->id = $employee_id;
                    $this->Employee->saveField('image' , $this->Upload->result);
                    if(isset($employee_image['Employee']['image']) && !empty($employee_image['Employee']['image'])){
                        (file_exists(WWW_ROOT . NOTE_IMG_ORIGINAL_DIR.$employee_image['Employee']['image'])?unlink(WWW_ROOT . NOTE_IMG_ORIGINAL_DIR.$employee_image['Employee']['image']):'');
                        (file_exists(WWW_ROOT . NOTE_IMG_MEDIUM_DIR.$employee_image['Employee']['image'])?unlink(WWW_ROOT . NOTE_IMG_MEDIUM_DIR.$employee_image['Employee']['image']):'');
                        (file_exists(WWW_ROOT . NOTE_IMG_THUMB_DIR.$employee_image['Employee']['image'])?unlink(WWW_ROOT . NOTE_IMG_THUMB_DIR.$employee_image['Employee']['image']):'');
                    }
                }
                $responseArr = array('user_id' => $user_id, 'employee_id' => $employee_id, 'image_name'=>$this->Upload->result, 'msg'=>'顧客情報が正常に追加されました。', 'msg1'=>'Employee image upload successfully.', 'status' => 'success');
                $jsonEncode = json_encode($responseArr);
            }else{

                $responseArr = array('user_id' => $user_id, 'employee_id' => $employee_id, 'status' => 'error', 'msg'=>'顧客情報が正常に追加されました。', 'msg1'=>'Image type should be (jpg, jpeg, png,gif).');
                $jsonEncode = json_encode($responseArr);
            }
        }else{
            $responseArr = array('user_id' => $user_id, 'employee_id' => $employee_id, 'status' => 'error', 'msg'=>'従業員が存在していませんでした');
            $jsonEncode = json_encode($responseArr);
        }
        echo  $jsonEncode;exit();
    }
     ...
     /****************************************************************************************************************************************
     * NAME: add_sub_service
     * Description: Add Sub Service.
     *
     * Subroutines Called:
     *
     * Parameters: Input / Output
    ...
    public function add_sub_service(){
        ...
        if(isset($decoded['user_id']) && !empty($decoded['user_id'])){
            if(isset($decoded['service_id']) && !empty($decoded['service_id'])){
                if(isset($decoded['id']) && !empty($decoded['id'])){
                    $this->SubService->id = $decoded['id'];
                    $subServiceData['SubService']['id'] = $decoded['id'];
                ...
                $subServiceData['SubService']['price'] = isset($decoded['price']) ? $decoded['price'] : ''; 
                $subServiceData['SubService']['status'] = Configure::read('App.Status.active');

                if($this->SubService->saveAll($subServiceData)){
                    $SubService = array('id'=> $this->SubService->id, 'user_id'=>$decoded['user_id'],'status' =>'success', 'msg'=>$msg,'response_code'=>'200');
                    $jsonEncode = json_encode($SubService);
                }else{
                    $SubService = array('status' => 'error', 'msg'=>'Error: Sub service add','response_code'=>'404');
                    $jsonEncode = json_encode($SubService);
                }
            }else{
                $SubService = array('status' => 'error', 'msg'=>'Please enter service id','response_code'=>'404');
                $jsonEncode = json_encode($SubService);
            }
        }else{
    ...
    /****************************************************************************************************************************************
     * NAME: list_sub_service
     * Description: List Sub Service.
     *
     * Subroutines Called:
     *
     * Parameters: Input / Output
    ...
    public function list_sub_service($id = null){
            ...
            foreach($serviceData as $serviceValue){
                // if(isset($serviceValue['SubService']) && !empty($serviceValue['SubService'])){
                    ...
                    foreach ($serviceValue['SubService'] as $value) {
                        ...
                        $subServiceList['Service'][$serviceValue['Service']['name']]['SubServiceList'][$i]['name'] = $value['name'];
                        $subServiceList['Service'][$serviceValue['Service']['name']]['SubServiceList'][$i]['duration'] = $value['duration'];
                        $subServiceList['Service'][$serviceValue['Service']['name']]['SubServiceList'][$i]['price'] = $value['price'];
                        $subServiceList['Service'][$serviceValue['Service']['name']]['SubServiceList'][$i]['status'] = $value['status'];
                        $subServiceList['Service'][$serviceValue['Service']['name']]['SubServiceList'][$i]['created'] = $value['created'];
                        $subServiceList['Service'][$serviceValue['Service']['name']]['SubServiceList'][$i]['modified'] = $value['modified'];
                        $i++;
                    }
                // }
            }
    ...
    public function delete_sub_service(){
        ...
        if(isset($id) && !empty($id)){
            if($this->SubService->delete($id, true)){
                $SubService = array('status' => 'success', 'msg'=>'Sub service delete successfully.','response_code'=>'200');
                $jsonEncode = json_encode($SubService);
            }else{
                $SubService = array('status' => 'error', 'msg'=>'Sub service deleted error','response_code'=>'404');
                $jsonEncode = json_encode($SubService);
            }
        }else{
            $SubService = array('status' => 'error', 'msg'=>'Please enter subService id','response_code'=>'404');
            $jsonEncode = json_encode($SubService);
        }        
        echo  $jsonEncode;exit();
     }
    ...
    public function new_service_list($id = null){
        ...
        if(isset($decoded['user_id']) && !empty($decoded['user_id'])){
            $this->Service->bindModel(array('hasMany' => array('SubService'=>array('conditions'=>array('SubService.user_id'=>$decoded['user_id'])))));
            $serviceData = $this->Service->find('all', array('conditions'=>array('Service.user_id'=>$decoded['user_id'],'Service.status'=>1)));
            $subServiceList['Service'] = array();
            foreach($serviceData as $serviceValue){
                if(isset($serviceValue['SubService']) && !empty($serviceValue['SubService'])){
                    $subServiceList['Service'][$serviceValue['Service']['name']]['service_id'] = $serviceValue['Service']['id'];
                    $subServiceList['Service'][$serviceValue['Service']['name']]['color_code'] = $serviceValue['Service']['color_code'];
                    $i = 0;
                    $subServiceList['Service'][$serviceValue['Service']['name']]['SubServiceList'] = array();
                    foreach ($serviceValue['SubService'] as $value) {
                        $subServiceList['Service'][$serviceValue['Service']['name']]['SubServiceList'][$i]['id'] = $value['id'];
     ...
     /****************************************************************************************************************************************
     * NAME: print_list
     * Description: List Note Service, Note Product and Note Ticket.
     *
     * Subroutines Called:
     *
     * Parameters: Input / Output
    ...
    public function print_list($id = null){
        ...
        $this->loadModel("CustomerHistory");
        $this->loadModel("SubService");
        $this->loadModel("CustomerReceipt");
        /*** Load Model End ***/
        ...
        if(!empty($user_id)){
            if(!empty($customer_history_id)){
                ...
                $this->CustomerHistory->bindModel(array('hasMany' => array(
                        'NoteService'=>array('conditions'=>array('NoteService.print_status'=>'0')),
                        'NoteTicket'=>array('conditions'=>array('NoteTicket.print_status'=>'0')),
                        'NoteProduct'=>array('conditions'=>array('NoteProduct.print_status'=>'0'))
                    )));

                $printData = $this->CustomerHistory->find('all', array('conditions'=>array('CustomerHistory.user_id'=>$user_id,'CustomerHistory.id'=>$customer_history_id)));
                // pr($printData);die;
                $salonData = $this->User->find('first',array('conditions'=>array('User.id'=>$user_id)));
                
                 $PrintList['salon_name'] = $salon_name = $salonData['User']['salon_name'];
                 $PrintList['address'] = $address = $salonData['User']['address2'].', '.$salonData['User']['address1'].', '.$salonData['User']['city'].', '.$salonData['User']['zip_code'].', '.$salonData['User']['prefecture'];
                 $PrintList['tel'] = $tel = $salonData['User']['tel'];
                 $PrintList['date'] = $date;
                ...
                $i = 0;
                // $PrintList['print']['staff_name'] = array();
                // $PrintList['print']['item'] = array();
                // $PrintList['print']['payment_type'] = array();
                foreach($printData as $printValue){
                    // $PrintList['print']['NoteService'] = $printValue['NoteService'];
                    // $PrintList['print']['NoteProduct'] = $printValue['NoteProduct'];
                    // $PrintList['print']['NoteTicket'] = $printValue['NoteTicket'];
                    $PrintList['customer_id'] = $customer_id = $printValue['CustomerHistory']['customer_id'];
                    $PrintList['customer_history_id'] = $customer_history_id;

                    $staff_name = array();
                    $serviceList = array();
                    // $price = array();
                    $i = 0;
                    foreach ($printValue['NoteService'] as $valueNoteService) {
                        // pr($valueNoteService);die;
                        $total_service_price = $total_service_price + str_replace( array( ',','円' ), '', $valueNoteService['service_price']);
                        $staff_name_array1[$i] = $valueNoteService['employee_name'];
                        // $price .= $valueNoteService['service_name']." ".$valueNoteService['service_price']."<br>";
                        // $serviceList1[$valueNoteService['service_name']] = number_format(str_replace( array( ',','円' ), '', $valueNoteService['service_price']))."円";
                        // $serviceList1[$valueNoteService['service_name']] = str_replace( array( ',','円' ), '', $valueNoteService['service_price']);
                        $serviceList1[$valueNoteService['service_name']] = $valueNoteService['service_price'];
                        // pr($serviceList1);die;
                        // $paymentList1[$valueNoteService['payment_type']] = number_format(str_replace( array( ',','円',' 円' ), '', $valueNoteService['service_price'])).' 円';
                        $paymentList1[$valueNoteService['payment_type']] = $valueNoteService['service_price'];
                        // $this->NoteService->id = $valueNoteService['id'];
                        // $this->NoteService->saveField('print_status', true);
                        $i++;
                    }
                    // pr($serviceList1[$valueNoteService['service_name']] = number_format($serviceList1[$valueNoteService['service_name']).'円');die;
                    
                    $j=0;
                    foreach ($printValue['NoteProduct'] as $valueNoteProduct) {
                        $total_sale_price = $total_sale_price + str_replace( array( ',','円' ), '', $valueNoteProduct['sale_price']);
                        $staff_name_array2[$j] = $valueNoteProduct['employee_name'];
                        // $serviceList2[$valueNoteProduct['product_name']] = str_replace( array( ',','円' ), '', $valueNoteProduct['sale_price']);
                        $serviceList2[$valueNoteProduct['product_name']] = $valueNoteProduct['sale_price'];
                        // $paymentList2[$valueNoteProduct['payment_type']] = str_replace( array( ',','円' ), '', $valueNoteProduct['sale_price']);
                        $paymentList2[$valueNoteProduct['payment_type']] = $valueNoteProduct['sale_price'];
                        // $staff_name .= $valueNoteProduct['employee_name']." ";
                        // $price .= $valueNoteProduct['product_name']." ".$valueNoteProduct['sale_price']."<br>";
                        // $price .= $valueNoteProduct['product_name']." ".number_format(str_replace( array( ',','円' ), '', $valueNoteProduct['sale_price']))."円";
                        // $this->NoteProduct->id = $valueNoteProduct['id'];
                        // $this->NoteProduct->saveField('print_status', true);
                        $j++;
                    }
                    // pr($staff_name_array);

                    $k = 0;
                    foreach ($printValue['NoteTicket'] as $valueNoteTicket) {
                        $total_ticket_price = $total_ticket_price + str_replace( array( ',','円' ), '', $valueNoteTicket['ticket_price']);
                        $staff_name_array3[$k] = $valueNoteTicket['employee_name'];
                        // $serviceList3[$valueNoteTicket['ticket_name']] = str_replace( array( ',','円' ), '', $valueNoteTicket['ticket_price']);
                        $serviceList3[$valueNoteTicket['ticket_name']] = $valueNoteTicket['ticket_price'];
                        // $paymentList3[$valueNoteTicket['payment_type']] = str_replace( array( ',','円' ), '', $valueNoteTicket['ticket_price']);
                        $paymentList3[$valueNoteTicket['payment_type']] = $valueNoteTicket['ticket_price'];
                        // $staff_name .= $valueNoteTicket['employee_name']." ";
                        // $price .= $valueNoteTicket['ticket_name']." ".$valueNoteTicket['ticket_price']."<br>";
                        // $price .= $valueNoteTicket['ticket_name']." ".number_format(str_replace( array( ',','円' ), '', $valueNoteTicket['ticket_price']))."円";
                        // $this->NoteTicket->id = $valueNoteTicket['id'];
                        // $this->NoteTicket->saveField('print_status', true);
                        $k++;
                    }

                    $PrintList['staff_name'] = array_unique(
                        array_merge(
                            isset($staff_name_array1)? $staff_name_array1 : array(),
                            isset($staff_name_array2)? $staff_name_array2 : array(),
                            isset($staff_name_array3)? $staff_name_array3 : array()
                        )
                    );
                    $PrintList['item'] = array_merge(
                        isset($serviceList1)? $serviceList1 : array(),
                        isset($serviceList2)? $serviceList2 : array(),
                        isset($serviceList3)? $serviceList3 : array()
                    );
                    $PrintList['payment_type'] = array_merge(
                        isset($paymentList1)? $paymentList1 : array(),
                        isset($paymentList2)? $paymentList2 : array(),
                        isset($paymentList3)? $paymentList3 : array()
                    );

                    // pr($PrintList['item']);die;
                    $dataStaff = implode('|', $PrintList['staff_name']);
                    // $string_item_name = implode(',', $PrintList['item']);

                    $dataItems = array_map(function($value, $key) {
                        return $key.'="'.$value.'"';
                    }, array_values($PrintList['item']), array_keys($PrintList['item']));
                    $dataItems = implode('|', $dataItems);

                    $dataPayments = array_map(function($value, $key) {
                        return $key.'="'.$value.'"';
                    }, array_values($PrintList['payment_type']), array_keys($PrintList['payment_type']));
                    $dataPayments = implode('|', $dataPayments);
                    ...
                    // pr($PrintList);die;
                    // pr($staffArray);die;
                    // pr($service_employee_name);
                    // pr($product_employee_name);
                    // pr($ticket_employee_name);die;

                    $total_price = $total_service_price + $total_sale_price + $total_ticket_price;
                    $PrintList['print_total_price'] = $totalPrice = number_format($total_price)."円";

                    $taxRate = 5;
                    $tax_amount = round($total_price*$taxRate/100);
                    $PrintList['print_tax_amount'] = $taxPrice = number_format($tax_amount)."円";

                    $total_cost = $total_price + $tax_amount;
                    $PrintList['print_total_cost'] = $totalCost = number_format($total_cost)."円";
                    
                    // $staff_name_array['staff_name'] = explode(" ",$staff_name);
                    // pr($staff_name_array);
                    // die;
                    // $price;
                ...
                $new_randstring = $this->JTSPrintRandomCodeString();
                $PrintList['receipt_number'] = $new_randstring;
                // echo $this->JTSPrintRandomCodeString();

                $print['CustomerReceipt']['user_id'] = $user_id;
                $print['CustomerReceipt']['customer_id'] = $customer_id;
                $print['CustomerReceipt']['customer_history_id'] = $customer_history_id;
                $print['CustomerReceipt']['receipt_number'] = $new_randstring;
                $print['CustomerReceipt']['salon_name'] = $salon_name;
                $print['CustomerReceipt']['tel'] = $tel;
                $print['CustomerReceipt']['date'] = $date;
                $print['CustomerReceipt']['address'] = $address;
                $print['CustomerReceipt']['staff_names'] = $dataStaff;
                $print['CustomerReceipt']['items'] = $dataItems;
                $print['CustomerReceipt']['payment_types'] = $dataPayments;
                $print['CustomerReceipt']['total_price'] = $totalPrice;
                $print['CustomerReceipt']['tax_price'] = $taxPrice;
                $print['CustomerReceipt']['total_cost'] = $totalCost;

                // pr($print);die;
                if($this->CustomerReceipt->saveAll($print)){
                    $Print = array('printList'=>$PrintList,'status' => 'success', 'msg'=>'Print receipt save successfully.','response_code'=>'200');
                    $jsonEncode = json_encode($Print);
                }else{
                    $PrintList = array('status' => 'error', 'msg'=>'Error: Print receipt saved','response_code'=>'404');
                    $jsonEncode = json_encode($PrintList);
                }
                // $jsonEncode = json_encode($PrintList);
            }else{
                $Print = array('status' => 'error', 'msg'=>'Please enter customer history id','response_code'=>'404');
                $jsonEncode = json_encode($Print);
            }
        }else{
            $Print = array('status' => 'error', 'msg'=>'Please enter user id','response_code'=>'404');
            $jsonEncode = json_encode($Print);
        }        
        echo  $jsonEncode;exit();
    }
    ...
    function JTSPrintRandomCodeString(){
        $randstring = 'RB0000';
        $randstring .= mt_rand(1000000000, 9999999999);
       $this->loadModel("CustomerReceipt");
       $customerPrintCodeExist = $this->CustomerReceipt->find('first', array('conditions'=>array('CustomerReceipt.receipt_number'=>$randstring)));
       if(isset($customerPrintCodeExist['CustomerReceipt']['id']) && !empty($customerPrintCodeExist['CustomerReceipt']['id'])){

           $this->JTSPrintRandomCodeString();
       }
       // pr($randstring);die;
       return $randstring;
   ...
   /** QR Code Generated Function Start **/
   public function qrCode($user_id, $salon_name){
       // // Don't render a view file
       $this->autoRender = false;     
       App::import("Vendor", "phpqrcode/qrlib");
       
       $tempDir = "uploads/qr_code/"; 
       $fileName = $salon_name.'.png'; 
        
       $pngAbsoluteFilePath = $tempDir.$fileName; 
       $urlRelativeFilePath = $tempDir.$fileName; 
       QRcode::png($user_id, $pngAbsoluteFilePath,QR_ECLEVEL_L, 10);

       return $fileName;
   }
   /** QR Code Generated Function End **/

   function add_web_user_image($test_data = null){
        // pr($_FILES);
        // pr($_POST);die;
         /* Load Model Start*/
         $this->loadModel("User");
     ...
     function update_user_id(){
        $this->loadModel('Reservation');
        $this->loadModel('Employee');
        $alldata = $this->Reservation->find('all');
        foreach ($alldata as $res_key => $res_value) {
            $emp_arra_ids = explode(',', $res_value['Reservation']['employee_ids']);
            $i = 0;
            $emp_array = array('43','49','53','54','55','74','77','86','110','129');
            foreach ($emp_arra_ids as $key => $value) {
                if($i ==0){
                    $emp_id = (int)$value;
                    // echo $emp_id;die;
                    if(in_array($emp_id, $emp_array)){
                        $empUserdata = $this->Employee->find('first', array('Employee.id'=>$emp_id));
                        pr($empUserdata);
                        $this->Reservation->updateAll(array('user_id' => $empUserdata['Employee']['user_id']), array('Reservation.id' => $res_value['Reservation']['id']));
                        echo $res_value['Reservation']['id'];die;
                    }
