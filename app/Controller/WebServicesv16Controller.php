...
*
*****************************************************************************/
class WebServicesv16Controller extends AppController{
            ...
            if(!empty($user_id) && !empty($customer_id)){
                ...
                if(isset($noteTicketData[0]) && !empty($noteTicketData[0])){
                    foreach ($noteTicketData as $noteTicketKey => $noteTicketValue) {
                        ...
                        $NoteRemainingTicketData=$this->NoteRemainingTicket->find('first', array('conditions'=>array('NoteRemainingTicket.user_id'=>$user_id, 'NoteRemainingTicket.customer_id'=>$customer_id, 'NoteRemainingTicket.customer_history_id'=>$id), 'order' => array('NoteRemainingTicket.id' => 'DESC')));
...
/****************************************************************************************************************************************
     ...
     *************************************************************************************************************************************/
     ...
     function add_note_ticket($test_data = null){
        ...
        $ticket_amount = $noteTicket['NoteTicket']['ticket_amount'] = isset($decoded['ticket_amount']) ? $decoded['ticket_amount'] : '';
      ...
      function get_date_reservation($testData = null){
        ...
        if(!empty($user_id)){
            ...
            if(!$userData){
                ...
                $this->Reservation->bindModel(array('belongsTo' => array('Customer', 'Service')));
                $reservationDataFind = $this->Reservation->find('all',array('conditions'=>array( 'Reservation.start_date <=' => $date, 'Reservation.end_date >=' => $date, 'Reservation.user_id'=>$user_id)));
                //echo "<pre>";
                //print_r($reservationDataFind);die;
                $reservationData = array();
...
/****************************************************************************************************************************************
    ...
    function list_employee_chat($test_data = null){
        ...
        if(!empty($data)){
            ...
            $employeeChatData[$i]['EmployeeChat'] = '';
