            ...
            if($sort_by == '1' || $sort_by == '2' || $sort_by == '3' || $sort_by == '4'){ 
                ...
                if($sort_by == '1' || $sort_by == '2'){
                    foreach ($data as $customerKey => $customerValue) {
                        ...
                        $date = date('M jS Y g:i A',strtotime('+3 hour +30 minutes +30 seconds',strtotime($customerValue['Customer']['modified']))); 
                ...
                $i= 0;
                    foreach ($data as $key => $value){ 
                        ...
                        $date = date('M jS Y g:i A',strtotime('+3 hour +30 minutes +30 seconds',strtotime($value['Customer']['modified']))); 
      ...
      function get_reservation_ipad($testData = null){
        ...
        if(!empty($user_id)){
            ...
            if(!$userData){
                ...
                $reservationDataFind = $this->Reservation->find('all',array('conditions'=>array( 'Reservation.user_id'=>$user_id, 'Reservation.start_date <=' => date('Y-m-d')),  'order' => array('Reservation.start_time' => 'ASC', 'Reservation.start_date' => 'DESC')));
                ...
                if(!empty($reservationDataFind)){
                    foreach ($reservationDataFind as $key => $value) {
                        ...
                        $startDate = date('d M Y', strtotime($value['Reservation']['start_date']));
                        if($value['Reservation']['all_day'] =='1'){
                             $reservationData['Reservation'][$key]['start_date']= $startDate;
                        }else{
                            $startTime = date('h:i A', strtotime($value['Reservation']['start_time']));
                            $endTime = date('h:i A', strtotime($value['Reservation']['end_time']));
                            $reservationData['Reservation'][$key]['start_date']= $startDate.' '.$startTime;
                            $reservationData['Reservation'][$key]['start_time']= $startTime;
                            $reservationData['Reservation'][$key]['end_time']= $endTime;
                        }
                        ...
                        $reservationData['Reservation'][$key]['last_visit']= date('d M Y', strtotime($last_visit));
