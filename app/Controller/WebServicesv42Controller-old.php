...
*
*****************************************************************************/
class WebServicesv42Controller extends AppController{
...
/**************************************************************************
    ...
    function get_employee_id($name = ''){
        $this->loadModel("Employee");
        if(!empty($name)){
            $data = $this->Employee->find('first',array('conditions'=> array('Employee.name'=>$name )));
...
/*
 ...
 /**************************************************************************
    ...
    function attendance_info_download_old($testData = null){
                        ...
                        if(!empty($customerData['Attendance'][$i]['checkin_time']) && !empty($customerData['Attendance'][$i]['checkout_time'])){
                            ...
                            if($total_over_time >=  $over_time){
                               $over_time_hours   = floor(($total_over_time) / 60);
                                ...
                                if($over_time_hours<10){
                                    $over_time_hours = '0'.$over_time_hours;
                                }
                                if($over_time_minutes<10){
                                    $over_time_minutes = '0'.$over_time_minutes;
                                }
                                $dataxls[$i]['over_time'] = $over_time_hours.":".$over_time_minutes;
                                

                            }else{
                                $dataxls[$i]['over_time'] = '00:00';
                            }
                            ...
                            $dy  = date("w", strtotime($value['Attendance']['date']));
                            $dataxls[$i]['date'] = date('d', strtotime($value['Attendance']['date']));
                            $dataxls[$i]['day'] = $dys[$dy];
                            $dataxls[$i]['employee_type'] = '';
                            $dataxls[$i]['holiday_type'] = '';
                            $dataxls[$i]['actual_start_time'] = date("H:i",strtotime($value['Attendance']['checkin_time']));
                            $dataxls[$i]['actual_end_time'] = date("H:i",strtotime($value['Attendance']['checkout_time']));
                            $dataxls[$i]['salon_start_time'] = date("H:i",strtotime($start_time));
                            $dataxls[$i]['salon_end_time'] = date("H:i",strtotime($end_time));
                            
                            $dataxls[$i]['after_time_count'] = '';
                            $dataxls[$i]['before_time_count'] = '';
                            ...
                            $duration =  $total_hours.":".$total_minutes;
                            ...
                            if(!empty($interval_lunch_time)){
                                ...
                                if($working_time > 0){
                                    ...
                                    $working_minutes =  floor(($working_time - ($working_hours * 60)));
                                    $lunch_hours   = floor(($interval_lunch_time) / 60);
                                    $lunch_minutes =  floor(($interval_lunch_time - ($lunch_hours * 60)));
                                    if($working_minutes<10){
                                        $working_minutes = '0'.$working_minutes;
                                    }
                                    if($working_hours<10){
                                        $working_hours = '0'.$working_hours;
                                    }
                                    if($lunch_hours<10){
                                        $lunch_hours = '0'.$lunch_hours;
                                    }
                                    if($lunch_minutes<10){
                                        $lunch_minutes = '0'.$lunch_minutes;
                                    }
                                    $dataxls[$i]['total_hours_without_lunch'] = $working_hours.":".$working_minutes;
                                    $dataxls[$i]['lunch_hours'] = $lunch_hours.":".$lunch_minutes;
                               
                                }else{
                                    $start_lunch = new DateTime($value['Attendance']['start_lunch_time']);
                                    $end_lunch = new DateTime($value['Attendance']['end_lunch_time']);
                                   
                                    $interval_lunch = $start_lunch->diff($end_lunch);
                                    $interval_lunch_time =  $interval_lunch->format('%h').":".$interval_lunch->format('%i');
                                    
                                    $dataxls[$i]['total_hours_without_lunch'] = '00:00';
                                    $dataxls[$i]['lunch_hours'] = $interval_lunch_time;
                                }
                                ...
                                $start_lunch = new DateTime($value['Attendance']['start_lunch_time']);
                                    ...
                                    $interval_lunch = $start_lunch->diff($end_lunch);
                                    $interval_lunch_time =  $interval_lunch->format('%h').":".$interval_lunch->format('%i');
                                $dataxls[$i]['total_hours_without_lunch'] = $duration;
                                $dataxls[$i]['lunch_hours'] = $interval_lunch_time;
                            }    
                            $dataxls[$i]['total_hours_without_lunch'] = $duration;
                        }else{
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue('A1', '日付')
                    ->setCellValue('B1', '曜日')
                    ->setCellValue('c1', '勤務体系')
                    ->setCellValue('D1', '休暇区分')
                    ->setCellValue('E1', '出勤')
                    ->setCellValue('F1', '退勤')
                    ->setCellValue('G1', '出勤時刻')
                    ->setCellValue('H1', '退勤時刻')
                    ->setCellValue('I1', 'total hours without lunch')
                    ->setCellValue('J1', 'Lunch Hours')
                    ->setCellValue('K1', 'over time')
                    ->setCellValue('L1', '遅刻')
                    ->setCellValue('M1', '早退');
        ...
        foreach ($dataxls as $key => $value) {
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('C'.$i, $value['employee_type'])
                    ->setCellValue('D'.$i, $value['holiday_type'])
                    ->setCellValue('E'.$i, $value['actual_start_time'])
                    ->setCellValue('F'.$i, $value['actual_end_time'])
                    ->setCellValue('G'.$i, $value['salon_start_time'])
                    ->setCellValue('H'.$i, $value['salon_end_time'])
                    ->setCellValue('I'.$i, $value['total_hours_without_lunch'])
                    ->setCellValue('J'.$i, $value['lunch_hours'])
                    ->setCellValue('K'.$i, $value['over_time'])
                    ->setCellValue('L'.$i, $value['after_time_count'])
                    ->setCellValue('M'.$i, $value['before_time_count']); 
            $i++;        
        }            
    ...
    function attendance_info_download($testData = null){
        ...
        $sheetMonth = date('Y年　m月', strtotime($startDate));
        $m = $i=0;
        $dys = array("日","月","火","水","木","金","土");
        if(!empty($user_id) && !empty($employee_id)){
            $data = $this->Attendance->find('all',array('conditions'=> array( 'Attendance.user_id'=>$user_id, 'Attendance.employee_id'=>$employee_id, 'Attendance.date >= ' => $startDate,'Attendance.date <= ' => $endDate),
                                                   'order' => array('Attendance.date' => 'ASC')));
            $empData = $this->Employee->find('first',array('conditions'=> array( 'Employee.id'=>$employee_id)));  
            $userData = $this->User->find('first',array('conditions'=> array( 'User.id'=>$user_id)));
            $emp_name  = isset($empData['Employee']['name']) ? $empData['Employee']['name'] : '';
            $emp_code  = isset($empData['Employee']['emp_code']) ? $empData['Employee']['emp_code'] : '';
            ...
            if(!empty($data)){
                ...
                foreach ($data as $key => $value) {
                    ...
                    $endCheckInTime = date("H:i",strtotime($value['Attendance']['checkin_time']));
                    $endCheckOutTime = date("H:i",strtotime($value['Attendance']['checkout_time']));
                    $to_time = strtotime($endCheckInTime);
                    $from_time = strtotime($endCheckOutTime);
                    $starting_total_time = round(abs($to_time  - $from_time) / 60,2);

                    $total_hourss = round(abs($to_time  - $from_time) / 3600,2);
                    if($total_hourss>= 9){
                        $interval_lunch_time = 60;   
                    }elseif( ($total_hourss>= 6) && ($total_hourss<= 9)){
                        $interval_lunch_time = 45;   
                    }else{
                        $interval_lunch_time = 0;   
                    }
                    $after_end_time = 0;
                    if(($starting_total_time > $salonWorkingTime) && ($salonWorkingTime >0)){
                       $total_over_time = $this->add_over_time($to_time, $from_time, $startTime, $endTime, $over_time);
                        ...
                        if($to_time >= $startTime){
                            $let_time_time = round(abs($startTime  - $to_time) / 60,2);
                            $after_end_time = ($let_time_time + $total_over_time);
                        }
                    }else{
                        $total_over_time = 0;
                    }
                    ...
                    if($over_time_hours > 0){
                        ...
                        $over_time_minutes = $total_over_time;
                ...
                $dataxls[$i]['employee_type'] = '';
                $dataxls[$i]['holiday_type'] = '';
                $dataxls[$i]['actual_start_time'] = '';
                $dataxls[$i]['actual_end_time'] = '';
                $dataxls[$i]['salon_start_time'] ='';
                $dataxls[$i]['salon_end_time'] = '';
        ...
        // Add some data
        $objPHPExcel->getActiveSheet(0)->mergeCells('F1:I1');
        $objPHPExcel->getActiveSheet(0)->setCellValue('F1',$sheetMonth);
        $objPHPExcel->getActiveSheet(0)->mergeCells('A2:C2');
        $objPHPExcel->getActiveSheet(0)->setCellValue('A2',$emp_code.'and '.$emp_name.' 様' );
        $objPHPExcel->getActiveSheet(0)->mergeCells('E2:K2');
        $objPHPExcel->getActiveSheet(0)->setCellValue('E2','ゼンエルディーンインクーナショナル 株式会社');
        ...
        $objPHPExcel->getActiveSheet(0)->getStyle('A3:M3')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('FF00FF00');
        $objPHPExcel->getActiveSheet()->getRowDimension('1')->setRowHeight(40);
        $objPHPExcel->getActiveSheet()->getRowDimension('3')->setRowHeight(40);
        // $objWorksheet->getActiveSheet(0)->getRowDimension('3')->setRowHeight(40);
        // $objWorksheet->getActiveSheet(0)->getColumnDimension('A')->setWidth(100);
        $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue('A3', '日付')
                    ->setCellValue('B3', '曜日')
                    ->setCellValue('C3', '勤務体系')
                    ->setCellValue('D3', '休暇区分')
                    ->setCellValue('E3', '出勤')
                    ->setCellValue('F3', '退勤')
                    ->setCellValue('G3', '出勤時刻')
                    ->setCellValue('H3', '退勤時刻')
                    ->setCellValue('I3', '総労働時間')
                    ->setCellValue('J3', '休憩時間')
                    ->setCellValue('K3', '残業')
                    ->setCellValue('L3', '遅刻')
                    ->setCellValue('M3', '早退');
        ...
        // $string = $objWriter->getExcelData();
        // $mail->addStringAttachment($string, 'myfile.xls');

        // $this->loadModel('Template');
        // $registrationMail = $this->Template->find('first', array('conditions' => array('Template.slug' => 'employee_attendance')));
        // $email_subject = $registrationMail['Template']['subject'];
        // $subject = 'JTSボード' . $email_subject ;
        ...
        // $activation_link    =   '<a href="'.$activation_url.'" style="background-color: #debd4e; padding: 15px 30px; display: inline-block; color: #fff; text-decoration: none; border-radius: 5px;">今すぐ登録する</a>';
        // $mail_message = str_replace(array('{NAME}', '{Employee_Name}','{Month}', '{SITE}'), array($userData['User']['name'], $emp_name,$month, Configure::read('App.SITENAME')), $registrationMail['Template']['content']);
        // $template = 'default';
        // $this->set('message', $mail_message);
    ...
    function add_over_time_old($to_time, $from_time, $start_time, $end_time, $over_time){
    ...
    function add_over_time($to_time, $from_time, $start_time, $end_time, $over_time){
        ...
        if($total_time > $total_salon_time){
            ...
            $total_over_time =  ($staff_over_time);
...
/****************************************************************************************************************************************
    ...
    function monthly_attendance($test_data = null){
     ...
     function monthly_attendance_download($test_data = null){
        ...
        if(!empty($employee_id)){
            ...
            $end_lunch_time = strtotime($emp_end_lunch_time);
            $interval_lunch_time =  $end_lunch_time - $start_lunch_time; 
            $hours_lunch_time = $minutes_lunch_time = '00';
            if(!empty($interval_lunch_time)){
                $hours_lunch_time   = floor(($interval_lunch_time) / 3600);
                $minutes_lunch_time = floor(($interval_lunch_time - ($hours_lunch_time * 3600))/60);
            } 
            if(isset($employeeCode['Employee']['id']) && !empty($employeeCode['Employee']['id'])){
                ...
                foreach ($data as $key => $value) {
                    ...
                    $interval_lunch_time = strtotime($value['Attendance']['lunch_time'])*60;
                    $endCheckInTime = date("H:i",strtotime($value['Attendance']['checkin_time']));
                    $endCheckOutTime = date("H:i",strtotime($value['Attendance']['checkout_time']));
                    $to_time = strtotime($endCheckInTime);
                    $from_time = strtotime($endCheckOutTime);    
                    $starting_total_time = $to_time  - $from_time;

                    $working_time = $starting_total_time - $interval_lunch_time;
                    $working_time = date("H:i",$working_time);
                    ...
                    $start_lunch_time = strtotime($value['Attendance']['start_lunch_time']);
                    $end_lunch_time = strtotime($value['Attendance']['end_lunch_time']);
                    $interval_lunch_time =  $end_lunch_time - $start_lunch_time; 
                    $hours_lunch_time = $minutes_lunch_time = '00';
                    
                    // $interval_lunch_time = date('H:i', strtotime($interval_lunch_time));
                    // echo $interval_lunch_time;
                    if(!empty($interval_lunch_time)){
                        $hours_lunch_time   = floor($interval_lunch_time / 3600);
                        if($hours_lunch_time<10){
                            $hours_lunch_time = '0'.$hours_lunch_time;
                        }
                        $minutes_lunch_time = floor(($interval_lunch_time - ($hours_lunch_time * 3600))/60);
                        if($minutes_lunch_time<10){
                            $minutes_lunch_time = '0'.$minutes_lunch_time;
                        }
                    } 
                    ...
                    $total_over_time_hours = date("H:i",$total_over_time_hours);
                    // $interval_lunch_time = date("H:i",strtotime($interval_lunch_time));


                    $dy  = date("w", strtotime($value['Attendance']['date']));
                    $dataxls[$m][$j]['date'] = date('d', strtotime($value['Attendance']['date']));
                    $dataxls[$m][$j]['day'] = $dys[$dy];
                    $dataxls[$m][$j]['employee_type'] = '';
                    $dataxls[$m][$j]['holiday_type'] = '';
                    $dataxls[$m][$j]['actual_start_time'] = date("H:i",strtotime($value['Attendance']['checkin_time']));
                    $dataxls[$m][$j]['actual_end_time'] = date("H:i",strtotime($value['Attendance']['checkout_time']));
                    $dataxls[$m][$j]['salon_start_time'] = date("H:i",strtotime($start_time));
                    $dataxls[$m][$j]['salon_end_time'] = date("H:i",strtotime($end_time));
                    $dataxls[$m][$j]['total_hours_without_lunch'] = $working_time;
                    $dataxls[$m][$j]['lunch_hours'] = $hours_lunch_time.':'.$minutes_lunch_time;
                    $dataxls[$m][$j]['over_time'] = $total_over_time_hours;
                    $dataxls[$m][$j]['after_time_count'] = '';
                    $dataxls[$m][$j]['before_time_count'] = '';
                    $j++;
                ...
                'employee_type' => "勤務体系",
                'actual_time' => "休暇区分",
                'actual_start_time' => "出勤",
                'actual_end_time' => "退勤",
                'salon_start_time' => "出勤時刻",
                'salon_end_time' => "退勤時刻",
                'total_hours_without_lunch' => "total hours without lunch",
                'lunch_hours' => "Lunch Hours",
                'over_time' => "over time",
                'after_time_count' => "遅刻",
                'before_time_count' => "早退",
                );
                ...
                if(!empty($reservationDataFind)){
                    ...
                    if(!empty($empReservationData)){
                        ...
                        foreach ($empReservationData as $empReservationkey => $empReservationvalue) {
                            $m =0;
                            $empId ='';
                            $empId = $this->get_employee_id($empReservationkey);
...
/************************ Push Notification ***********************/
    ...
    public function IOSPushNotification($deviceToken=null,$message=null,$user_id=null,$employee_id=null,$customer_id=null,  $reservation_id=null, $device_type=null, $member_type=null, $notification_type=null) {
     ...
     public function androidPushNotification($deviceToken=null,$message=null,$user_id=null,$employee_id=null,$customer_id=null,  $reservation_id=null, $device_type=null, $member_type=null, $notification_type=null, $firebase_api_key=null){
